...
The writer persists point-cloud map artifacts with enough provenance metadata
to prove derived artifacts came from the same live mapping source. Simulation
gates may call this module, but the artifact contract lives under ``src`` so
product map and relocalization flows can reuse it.
"""
...
_SAVED_MAP_ARTIFACT_FRAMES = runtime_topic_allowed_frame_ids(None)[TOPICS.saved_map_cloud]
_SAVED_MAP_DEFAULT_FRAME_ID = topic_default_frame_id(TOPICS.saved_map_cloud)

DERIVED_MAP_ARTIFACTS = (
    "occupancy.npz",
    "octomap.ot",
    "octomap.bt",
    "esdf.npz",
...
# Tomogram generation is retired, but existing saved-map metadata must remain
# auditable so provenance drift cannot silently pass validation.
_RETIRED_ARTIFACT_REQUIRED_METADATA = {
    "tomogram": (
        "source_map_sha256",
        "source_profile",
        "data_source",
...
def invalidate_derived_map_artifacts(map_dir: Path | str) -> list[str]:
    """Remove planning artifacts that no longer match ``map.pcd``."""
    root = Path(map_dir)
    removed: list[str] = []
    for filename in DERIVED_MAP_ARTIFACTS:
        path = root / filename
        if not path.exists():
            continue
        path.unlink()
...
def validate_same_source_map_metadata(
    ...
    for name in ("tomogram", "octomap", "occupancy_grid"):
        ...
        if str(entry.get("source_map_sha256") or "") != pcd_sha:
            blockers.append(f"metadata.artifacts.{name}.source_map_sha256 does not match map_pcd.sha256")
...
def validate_saved_map_artifact_dir(
    ...
    *,
    ...
    if require_tomogram:
        blockers.append("tomogram artifacts are retired; require octomap instead")
    if require_octomap:
        checked_required_artifacts.append("octomap")
    ...
    for name, required, missing_msg in (
        ("octomap", require_octomap, "octomap required but missing"),
        ...
        if isinstance(entry, Mapping):
            source_matches = str(entry.get("source_map_sha256") or "") == map_sha
            artifacts_report[name]["source_map_sha256_matches_map"] = source_matches
...
def _check_artifact_entry(
    ...
    format_spec = ARTIFACT_FORMATS.get(name)
    required_metadata = (
        format_spec.required_metadata
        if format_spec is not None
        else _RETIRED_ARTIFACT_REQUIRED_METADATA[name]
    )
    _require_fields(
        blockers,
...
def _artifact_file_path(root: Path, name: str, entry: Mapping[str, Any]) -> tuple[Path, bool]:
    ...
    if path_value:
        ...
        try:
            root_relative.relative_to(root_resolved)
        except ValueError:
            return root_relative, False
        return root_relative, True
    return root / ARTIFACT_FORMATS[name].path, True


def _artifact_file_blocker(report: Mapping[str, Mapping[str, Any]], name: str) -> str:
    entry = report.get(name) or {}
    if entry.get("path_within_map_dir") is False:
        return f"{name} path escapes map directory"
...
def write_same_source_map_artifacts(
    *,
    ...
    if bounds is not None:
        ...
        if max_span > float(map_artifact_max_span_m):
            report["blockers"].append(
                f"map bounds span exceeds limit (span={span}, max_allowed={float(map_artifact_max_span_m):.3f}m)"
            )
    ...
    report["source_contract"]["same_source_pcd"] = bool(pcd_path.exists() and point_count > 0 and bool(pcd_sha))
    if build_tomogram:
        report["blockers"].append("tomogram artifact generation is retired; build octomap through the maps pipeline")
    ...
    if not metadata_validation["ok"]:
        report["blockers"].extend(f"metadata contract: {blocker}" for blocker in metadata_validation["blockers"])
    ...
    report["ok"] = bool(report["source_contract"]["same_source_pcd"]) and not report["blockers"]
    return report
