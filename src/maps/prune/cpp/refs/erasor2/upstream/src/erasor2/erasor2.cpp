...
void ERASOR2::saveStaticMap(const string &static_map_path) {
  ...
  for (int i = 0; i < num_data_; ++i) {
    ...
    // PCL safety guard for this GPL reference snapshot: upstream ERASOR2 may
    // leave a frame slot unpopulated after dynamic filtering. PCL's
    // transformPointCloud reads the cloud dimensions and can SIGFPE on a
    // zero-width empty cloud. Mathematically, an empty static set contributes no
    // points to the accumulated static map, so the explicit empty-set guard
    // preserves output semantics while preventing the PCL crash. This file is
    // not compiled into LingTu's default MIT `prune` product path.
