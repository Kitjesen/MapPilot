...
_TERMINAL_LIFECYCLES: Final[frozenset[str]] = frozenset({"SUCCESS", "FAILED", "CANCELLED"})
...
def project_navigation_goal_status(status: Mapping[str, Any]) -> dict[str, Any]:
    ...
    phase = (
        str(projected.get("phase") or projected.get("native_state_name") or projected.get("state_name") or "")
        .strip()
        .upper()
    )
    lifecycle_name = _GOAL_PHASE_TO_LIFECYCLE.get(phase)
    if lifecycle_name is None:
        raise NavigationTaskProjectionError(f"unsupported_navigation_goal_phase:{phase or 'missing'}")
