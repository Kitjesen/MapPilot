...
def _has_positive_mapping_ack(result: Any) -> bool:
    return isinstance(result, Mapping) and (result.get("accepted") is True or result.get("success") is True)
...
def submit_goal(
    ...
    *,
    ...
    if not callable(operation):
        return False
    try:
        limits = {key: value for key, value in {"max_speed_mps": max_speed_mps,
                  "acceptance_radius_m": acceptance_radius_m}.items() if value is not None}
        result = operation(goal, task_id=task_id, request_id=request_id, action="goal", **limits)
    except Exception as exc:
        raise CommandBoundaryError(str(exc)) from exc
    if not _has_positive_mapping_ack(result):
        reason = result.get("message") if isinstance(result, Mapping) else "invalid goal service response"
...
def submit_cancel(
    ...
    *,
    ...
    try:
        ...
        raise CommandBoundaryError(str(exc)) from exc
    if not _has_positive_mapping_ack(result):
        message = result.get("message") if isinstance(result, Mapping) else "invalid goal service response"
...
def _submit_task_control(
    ...
    *,
    ...
    try:
        ...
        raise CommandBoundaryError(str(exc)) from exc
    if not _has_positive_mapping_ack(result):
        message = result.get("message") if isinstance(result, Mapping) else f"invalid {method} service response"
        raise CommandBoundaryError(str(message))
    ...
    if not expected_request_id or returned_request_id != expected_request_id:
        raise CommandBoundaryError(f"goal service returned the wrong request_id for {method}")

    returned_state = str(receipt.get("state") or "").strip()
    if returned_state != expected_state:
        raise CommandBoundaryError(f"goal service returned the wrong acknowledgement state for {method}")
    return receipt
