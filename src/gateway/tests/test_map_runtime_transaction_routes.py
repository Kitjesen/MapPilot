...
def test_map_activate_cannot_bypass_active_session_transaction() -> None:
    ...
    response = asyncio.run(
        _endpoint(gateway, "/api/v1/map/activate")(
            MapNameRequest(name="office"),
        )
    )
    payload = _payload(response)

    assert response.status_code == 409
    assert payload["reason_code"] == "active_session_map_conflict"
    assert maps.commands == []


def test_map_activate_fails_closed_without_runtime_ack() -> None:
    from gateway.gateway_module import GatewayModule
    from gateway.schemas import MapNameRequest
    ...
    assert response.status_code == 409
    assert payload["reason_code"] == "runtime_binding_unconfirmed"
