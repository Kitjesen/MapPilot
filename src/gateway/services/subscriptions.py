...
def setup_subscriptions(gw: Any) -> None:
    ...
    gw.joint_state.subscribe(gw._on_joint_state)
    gw.joint_state.set_policy("latest")
    gw.lidar_scan.subscribe(gw._on_lidar_scan)
    gw.lidar_scan.set_policy("latest")
    gw.map_observation.subscribe(gw._on_map_observation)
    gw.map_observation.set_policy("latest")
