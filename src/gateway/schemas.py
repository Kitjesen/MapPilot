...
class MapActivationArtifactIdentity(BaseModel):
    """One immutable map artifact identity."""
...
class MapActivationMapIdentity(BaseModel):
    """Exact identity of one staged map version."""
    ...
    map_dir: str | None = None
    artifacts: list[MapActivationArtifactIdentity] = Field(min_length=1)


class MapActivationToken(BaseModel):
    """Complete token required to compensate a map activation."""

    model_config = ConfigDict(extra="forbid")

    schema_version: Literal["lingtu.map_activation.v1"]
    target: MapActivationMapIdentity
    previous: MapActivationMapIdentity | None
    changed: bool

    @model_validator(mode="after")
    def validate_change_semantics(self) -> MapActivationToken:
        """Reject contradictory changed/unchanged token states."""
        if self.changed and self.previous is not None and self.previous.map_id == self.target.map_id:
            raise ValueError("changed map activation identities must name different maps")
        if not self.changed and self.previous != self.target:
            raise ValueError("unchanged map activation identities must be exactly equal")
        return self


class MapActivationRestoreRequest(BaseModel):
    """Request to restore the previous identity from an activation token."""

    model_config = ConfigDict(extra="forbid")

    activation_token: MapActivationToken
...
class InspectionTaskListResponse(GatewayResponseModel):
    ...
    schema_version: Literal["lingtu.inspection.task.v1"] = "lingtu.inspection.task.v1"
    retention: Literal["process_local_gateway_projection"]
