...
def _mapd_http_request(gw: Any, cmd: dict[str, Any]) -> dict[str, Any]:
    """Map transport failures to the existing HTTP 503 response."""
    try:
        return mapd_request(gw, cmd)
...
def register_map_routes(app, gw) -> None:
    ...
    async def slam_maps():
        resp = await asyncio.to_thread(
            _mapd_http_request, gw, {"action": "list_maps"}
        )
    ...
    async def get_environment_map_layers():
        ...
        return feedback.snapshot(
            traversability_status=read_traversability_status(),
            nav_endpoint_status=read_navigation_status(),
        )
