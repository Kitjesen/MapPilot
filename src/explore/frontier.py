...
@register("exploration", "wavefront_frontier", description="Wavefront frontier exploration")
class WavefrontFrontierExplorer(Module, layer=2):
    ...
    def _on_navigation_status(self, status: MissionStatus) -> None:
        ...
        if state == "SUCCESS":
            ...
            reason = str(status.get("failure_reason") or status.get("reason") or state.lower())
    ...
    def _compute_frontier_clusters(self) -> list:
        ...
        for c in clusters:
            ...
            if isinstance(frontier_target, (list, tuple)) and len(frontier_target) >= 2:
        ...
        while not self._stop_event.is_set():
            ...
            if costmap_ts > 0 and time.time() - costmap_ts > self._costmap_max_age:
                logger.warning(
                    ...
                    time.time() - costmap_ts,
                    self._costmap_max_age,
                )
    ...
    def health(self) -> dict[str, Any]:
        ...
        with self._state_lock:
            ...
            info["current_goal"] = None if current_goal is None else [float(current_goal[0]), float(current_goal[1])]
            ...
            info["exploration_grid_source"] = self._grid_source_name(self._exploration_grid_data)
            info["visited_goal_count"] = len(self._visited_goals)
            info["frontier_grid_counts"] = self._grid_counts_from_data(self._frontier_grid_data_unlocked())
            info["costmap_counts"] = self._grid_counts_from_data(self._costmap_data)
            info["exploration_grid_counts"] = self._grid_counts_from_data(self._exploration_grid_data)
            info["frontier_debug"] = dict(self._frontier_debug)
            info["current_frontier_candidates"] = int(self._frontier_debug.get("scored_frontier_count") or 0)
            info["current_frontier_raw_clusters"] = int(self._frontier_debug.get("raw_cluster_count") or 0)
        return info
    ...
    def _prune_blocked_goals_unlocked(self, now: float) -> None:
        ...
        if ttl <= 0.0:
            return
        self._blocked_goals = [item for item in self._blocked_goals if now - float(item.get("ts") or 0.0) <= ttl]
    ...
    def _parse_costmap(self, data: dict) -> tuple[np.ndarray | None, dict | None]:
    ...
    def _find_frontier_clusters(
        ...
        for r0, c0 in zip(frontier_rows.tolist(), frontier_cols.tolist()):
            ...
            while cq:
                ...
                for dr, dc in _NEIGH8:
                    nr, nc = cr + dr, cc + dc
                    if 0 <= nr < h and 0 <= nc < w and not cluster_visited[nr, nc] and frontier_mask[nr, nc]:
            ...
            goal_row, goal_col = min(
                cells,
                key=lambda cell: (
                    (cell[0] - mean_row) * (cell[0] - mean_row) + (cell[1] - mean_col) * (cell[1] - mean_col)
                ),
            )
        ...
        self._update_frontier_debug(
            ...
            else ("all_frontier_clusters_below_min_size" if len(clusters) <= 0 else "frontier_clusters_detected"),
        )
        return clusters
    ...
    def _score_clusters(
        ...
        self._update_frontier_debug(
            ...
            frontiers_after_reachability_filter=0,
            scored_frontier_count=0,
            last_empty_reason="no_raw_frontier_clusters" if not clusters else "scoring_frontiers",
        )
        if not clusters:
            return []
        ...
        if blocked_goals:
            ...
            clusters = [c for c in clusters if not self._is_blocked_goal(c["cx"], c["cy"], blocked_goals)]
            self._update_frontier_debug(
        ...
        for c in clusters:
            ...
            if self._visited_goals:
                novelty_dist = min(math.hypot(c["cx"] - vx, c["cy"] - vy) for vx, vy in self._visited_goals)
            ...
            safety_score = self._approx_safety(c["cx"], c["cy"], robot_x, robot_y, meta)
        ...
        self._update_frontier_debug(
            ...
            selected_frontier_approach=bool(clusters[0].get("approach_goal", False)) if clusters else False,
    ...
    def _filter_reachable_clusters(
        ...
        for cluster in clusters:
            ...
            if has_frontier_cells and candidate is None:
                continue
            if candidate is not None and (has_frontier_cells or goal_cell is None or goal_cell not in component):
                row, col = candidate
        ...
        while queue:
            ...
            for dr, dc in _NEIGH8:
                ...
                if (
                    dr
                    and dc
                    and (not self._costmap_cell_free(grid, r, c + dc) or not self._costmap_cell_free(grid, r + dr, c))
                ):
                    continue
                if 0 <= nr < h and 0 <= nc < w and not visited[nr, nc] and self._costmap_cell_free(grid, nr, nc):
                    visited[nr, nc] = True
                    queue.append((nr, nc, depth + 1))
        return component
    ...
    def _best_reachable_approach_cell(
        ...
        *,
        ...
        if require_target_near and math.hypot(wx - target_x, wy - target_y) > self._approach_max_target_distance_m:
            return None
    ...
    def _costmap_cell_free(self, grid: np.ndarray, row: int, col: int) -> bool:
        h, w = grid.shape
        return 0 <= row < h and 0 <= col < w and self._is_traversable_cost_cell(grid[row, col])
