...
struct TarePolicyConfig {
  ...
  double sensor_horizontal_fov_rad{6.283185307179586};
  double sensor_yaw_offset_rad{0.0};
  ...
  std::size_t max_known_map_candidate_samples{128U};
  std::size_t max_known_map_gain_checks_per_candidate{2048U};
  std::size_t max_known_map_visibility_checks{20'000U};
  ...
  bool known_map_require_line_of_sight{true};
...
class TarePolicy final : public IExplorePlanner {
 public:
  ...
  [[nodiscard]] ExploreDecision plan(const ExploreInput &input, const ExploreCancelCheck &cancel);

  [[nodiscard]] TareDecision select(const Grid2D &grid, const Pose2D &robot,
