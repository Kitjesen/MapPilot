...
from compat.lcm.endpoint_runner import LCMEndpointEvent, run_endpoint_service
...
def test_endpoint_runner_can_attach_multiple_sources(monkeypatch) -> None:
    from compat.lcm import endpoint_runner
...
def test_endpoint_runner_forwards_lingtu_messages_to_all_sources() -> None:
    from compat.lcm.endpoint_runner import _notify_sources
...
def test_endpoint_runner_loads_comma_separated_builtin_sources() -> None:
    from compat.lcm.endpoint_runner import _load_sources
...
def test_endpoint_runner_loads_thunder_field_group_as_product_source(monkeypatch) -> None:
    from compat.lcm import endpoint_runner
    ...
    source = endpoint_runner._load_sources("thunder_field")[0]

    assert source.health()["name"] == "thunder_brainstem"


def test_endpoint_runner_accepts_builtin_thunder_brainstem_source_alias() -> None:
    from compat.lcm.endpoint_runner import _load_source

    source = _load_source("thunder_brainstem")

    assert source.health()["name"] == "thunder_brainstem"
    assert source.health()["hardware"] is True
    assert source.health()["role"] == "motion_command_sink"
