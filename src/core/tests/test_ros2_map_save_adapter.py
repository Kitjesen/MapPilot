...
from compat.ros2.map_save import (
    MapSaveError,
    MapSaveTimeout,
    MapSaveUnavailable,
    Ros2MapSaveAdapter,
...
from core.map_save import default_map_save_adapter
...
def test_save_pgo_map_builds_legacy_ros2_command() -> None:
    with patch("compat.ros2.map_save.subprocess.run") as run:
...
def test_save_pgo_map_reports_unavailable_runtime() -> None:
    with patch("compat.ros2.map_save.subprocess.run", side_effect=FileNotFoundError):
...
def test_save_pgo_map_reports_timeout() -> None:
    ...
    with patch("compat.ros2.map_save.subprocess.run", side_effect=error):
...
def test_save_nav_map_builds_legacy_ros2_command() -> None:
    ...
    assert cmd[:2] == ["bash", "-c"]
    assert "ros2 service call /nav/save_map interface/srv/SaveMaps" in command
...
def test_ros2_map_save_adapter_delegates_to_legacy_services() -> None:
    ...
    with patch("compat.ros2.map_save.save_nav_map") as nav, patch(
        "compat.ros2.map_save.save_pgo_map"
    ) as pgo:
...
def test_default_map_save_adapter_resolves_ros2_plugin_after_registry_clear(
    ...
    try:
        ...
        assert type(adapter).__module__ == "compat.ros2.map_save"
