...
from slam import relocalization_service as relocalization
...
def _runtime_topic_default(field_name: str) -> str:
    tree = ast.parse(
        (ROOT / "src" / "core" / "runtime_interface.py").read_text(
            encoding="utf-8-sig",
        )
    )
...
def test_slam_relocalization_service_uses_core_result_contract():
    tree = ast.parse(
        (
            ROOT / "src" / "slam" / "relocalization_service.py"
        ).read_text(encoding="utf-8-sig"),
        filename="relocalization_service.py",
    )
    assert not any(
...
def test_trigger_global_relocalize_uses_global_service_and_unsets_rmw(monkeypatch):
    ...
    assert "/nav/global_relocalize" in command
...
def test_relocalize_saved_map_quotes_path_and_parses_quality(monkeypatch, tmp_path):
    ...
    args, kwargs = calls[0]
    command = args[2]
    assert "/nav/relocalize" in command
