"""OSNet Re-ID feature extraction helper."""
...
def _try_load_bpu(model_path: str) -> object | None:
    ...
    try:
        ...
        raise RuntimeError(f"OSNetReIDEncoder: BPU model at {model_path} exists but failed to load: {exc}") from exc
...
def _try_load_torchreid() -> object | None:
    ...
    try:
        ...
        logger.info(
            "OSNetReIDEncoder: torchreid OSNet loaded (device=%s)", "cuda" if torch.cuda.is_available() else "cpu"
        )
        return extractor
    except Exception as exc:
        raise RuntimeError(f"OSNetReIDEncoder: torchreid available but OSNet failed to load: {exc}") from exc


class OSNetReIDEncoder:
    """O S Net Re I D Encoder."""
    ...
    def encode(self, crop_rgb: np.ndarray) -> np.ndarray:
        ...
        if crop_rgb is None or crop_rgb.ndim != 3 or crop_rgb.shape[2] != 3:
            raise ValueError(
                f"OSNetReIDEncoder.encode: expected (H, W, 3) RGB array, got shape {getattr(crop_rgb, 'shape', None)}"
            )
    ...
    def _encode_bpu(self, img: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if feat.size != _OSNET_FEAT_DIM:
                raise RuntimeError(f"BPU model returned {feat.size}-dim feature, expected {_OSNET_FEAT_DIM}")
            return feat
        except Exception as exc:
    ...
    def _encode_torchreid(self, img: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            feat = np.asarray(output, dtype=np.float32).ravel()
            if feat.size != _OSNET_FEAT_DIM:
                raise RuntimeError(f"torchreid returned {feat.size}-dim feature, expected {_OSNET_FEAT_DIM}")
            return feat
        except Exception as exc:
