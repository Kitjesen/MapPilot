"""Slow-path LLM goal resolution strategies."""
...
class SlowPathMixin:
    ...
    async def resolve(
        ...
        from .resolver import GoalResult
        from .router import AdaCoTDecision
        ...
        if adacot_decision != AdaCoTDecision.SLOW:
            fast_result = self.fast_resolve(instruction, scene_graph_json, robot_position, clip_encoder)
            if fast_result is not None:
                if fast_result.score_entropy > 1.5 and fast_result.confidence < 0.85:
                    logger.info(
                        "[AdaNav] high entropy=%.2f, confidence=%.2f → deferring to Slow Path for verification",
                        fast_result.score_entropy,
            ...
            if adacot_decision == AdaCoTDecision.FAST:
                logger.info("AdaCoT recommended FAST but no match, falling through to Slow")

        filtered_sg = self._selective_grounding(instruction, scene_graph_json, clip_encoder=clip_encoder)

        messages = build_goal_resolution_prompt(instruction, filtered_sg, robot_position, language)
        ...
        if explore_if_unknown and result.action == "explore" and robot_position is not None:
            logger.info(
                "Target unknown, generating exploration waypoint. Reason: %s",
                result.reasoning,
            )
            self._explored_directions.append({"x": result.target_x, "y": result.target_y})
    ...
    async def generate_exploration_waypoint(
        ...
        """Generate exploration waypoint."""
        from decision.llm.prompts import build_exploration_prompt
        from runtime.utils.sanitize import safe_json_loads
                ...
                try:
                    ...
                    if rooms:
                        priors = self._semantic_prior_engine.get_unexplored_priors(
                            ...
                            current_room_id=self._get_current_room_id(robot_position, rooms),
                        )
                        if priors:
                    ...
                    if topology_edges:
                        topo_parts = []
                        room_names = {r.get("room_id", -1): r.get("name", "?") for r in rooms}
                        for te in topology_edges[:8]:
                            ...
                            topo_parts.append(f"{fn}[{visited_f}] <-> {tn}[{visited_t}] ({te.get('type', '?')})")
                        topology_context = "\n".join(topo_parts)
        ...
        try:
            ...
            if norm > 0:
                ...
                logger.warning("LLM exploration returned robot's own position, using random direction")
        ...
        if frontier_nodes:
            ...
            for fnode in self._tsg.frontiers:
                ...
                for edge in self._tsg._adjacency.get(fnode.node_id, []):
                    other_id = edge.to_id if edge.from_id == fnode.node_id else edge.from_id
                ...
                if nearest_room_id is not None:
                    ...
                    if room_node:
                        fnode.predicted_room_type = self._predict_adjacent_room_type(room_node.room_type)

        room_id = self._tsg.record_robot_position(robot_position["x"], robot_position["y"])
    ...
    async def vision_grounding(
        ...
        from decision.llm.prompts import build_vision_grounding_prompt
        ...
        system, user_text = build_vision_grounding_prompt(instruction, scene_graph_json, language)
    ...
    def _selective_grounding(
        ...
        filtered_objects = [o for o in objects if o["id"] in relevant_ids][:max_objects]
        filtered_obj_ids = {o["id"] for o in filtered_objects}

        filtered_relations = [
            r for r in relations if r.get("subject_id") in filtered_obj_ids and r.get("object_id") in filtered_obj_ids
        ][:max_relations]

        filtered_regions = [r for r in regions if any(oid in filtered_obj_ids for oid in r.get("object_ids", []))]

        summary = sg.get("summary", "")
        if filtered_objects:
            labels = [o["label"] for o in filtered_objects[:10]]
            summary = f"Filtered {len(filtered_objects)}/{len(objects)} relevant objects: " + ", ".join(labels)
    ...
    async def _call_with_fallback(self, messages: list[dict[str, str]]) -> str | None:
        """Call with fallback."""
        from decision.llm.client import LLMError
