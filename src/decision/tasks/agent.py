"""Tool-calling loop used by SemanticPlannerModule."""
...
- Use query_memory for fuzzy location requests ("the place where I left my backpack last time").
...
class AgentLoop:
    ...
    async def run(self, instruction: str) -> AgentState:
        ...
        system_msg = (
            ...
            f'- Use query_memory for fuzzy location requests ("the place where I left my backpack last time").\n'
    ...
    def _validate_tool_call(self, name: str, args: dict) -> str | None:
        ...
        if name not in self._known_tool_names:
            ...
            return f"unknown tool '{name}'. Valid tools start with: {sample}"
    ...
    async def _execute_tool(self, tool_call: dict, state: AgentState) -> str:
        ...
        if validation_err is not None:
            ...
            error_msg = f"VALIDATION_ERROR for tool '{name}': {validation_err}. Please emit a corrected tool call."
            state.messages.append(
                {
                    "role": "tool",
        ...
        logger.info("AgentLoop step %d: %s(%s) -> %s", state.step, name, args, result[:100])
        return result
    ...
    def _get_vlm_agent(self):
        ...
        if self._vlm_agent is None:
            try:
                ...
                logger.info("AgentLoop: VLMSceneAgent initialized (backend=%s)", type(self._llm).__name__)
            except Exception as e:
