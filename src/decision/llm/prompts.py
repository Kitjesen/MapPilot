...
def build_goal_resolution_prompt(
    ...
    if robot_position:
        pos_str = f"x={robot_position['x']:.2f}, y={robot_position['y']:.2f}, z={robot_position['z']:.2f}"
        if language == "zh":
    ...
    if objects:
        ...
        if language == "zh":
            context_parts.append(
                f"## 层次3: Objects (物体, 前{min(30, len(objects))}个)\n```json\n{objects_compact}\n```"
            )
        else:
    ...
    if language == "zh":
        ...
        context_parts.append(
            "Reason layer by layer: Room -> Group -> Object. Use topology for reachability. Output JSON."
        )
...
def build_compound_decomposition_prompt(
    ...
    """Build compound decomposition prompt."""
