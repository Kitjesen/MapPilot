"""LCM ingress adapter for Thunder navigation commands."""
...
@register(
    ...
    description="LCM endpoint adapter for Thunder goal, cancel, and instruction ingress",
)
class LCMNavigationCommandBridgeModule(Module, layer=5):
    """Bridge Thunder endpoint command channels into LingTu navigation ports."""
    ...
    def setup(self) -> None:
        ...
        logger.info(
            "LCMNavigationCommandBridgeModule: subscribed to %s command channels for %s",
    ...
    def _subscribe_binding(self, binding: LCMEndpointBinding) -> Any:
        ...
        if transport is None:
            raise RuntimeError("LCM navigation command bridge transport is not initialized")
        ...
        raise TypeError(
            "LCM navigation command bridge transport must expose subscribe() "
    ...
    def _publish_topic(self, topic: str, msg: Any) -> None:
        ...
        if topic == TOPICS.semantic_instruction:
            ...
            return
        logger.debug(
            "LCM navigation command bridge ignored %s payload %s",
            topic,
            type(msg).__name__,
        )
