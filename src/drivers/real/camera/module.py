...
_HEADER = struct.Struct("<4sHHIIIIddddddIddddd")
_MAGIC = b"LTOB"
_VERSION = 2
_KIND_INTRINSICS = 1
_KIND_COLOR = 2
    ...
    try:
        ...
        for item in data.get("devices", []):
            ...
            if item.get("driver") in {"orbbec_native", "orbbec_sdk"}:
                ...
                config.setdefault("device", item.get("id") or CAMERA_ROLE)
...
class OrbbecNativeCameraModule(Module, layer=1):
    ...
    def __init__(
        ...
        **kw,
        ...
        self._uid = str(uid if uid is not None else usb_port if usb_port is not None else config.get("uid", "")).strip()
        ...
        self._device = str(config.get("device") or CAMERA_ROLE)
        self._camera_backend = CAMERA_BACKEND_ORBBEC
    ...
    def health(self) -> dict[str, object]:
        ...
        info["camera_backend"] = self._camera_backend
    ...
    def _publish_intrinsics(self, record: dict[str, object]) -> None:
        intrinsics = CameraIntrinsics(
            ...
            dist_k1=float(record.get("dist_k1", 0.0)),
            dist_k2=float(record.get("dist_k2", 0.0)),
            dist_p1=float(record.get("dist_p1", 0.0)),
            dist_p2=float(record.get("dist_p2", 0.0)),
            dist_k3=float(record.get("dist_k3", 0.0)),
        )
        if self._rotate:
    ...
    def _rotate_intrinsics(self, intrinsics: CameraIntrinsics) -> CameraIntrinsics:
        ...
        if self._rotate == 90:
            return CameraIntrinsics(
                ...
                dist_k1=intrinsics.dist_k1,
                dist_k2=intrinsics.dist_k2,
                dist_p1=intrinsics.dist_p1,
                dist_p2=intrinsics.dist_p2,
