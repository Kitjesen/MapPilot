...
RosTopics default_topics(const std::string &camera_namespace = "/camera");
...
class Camera {
 public:
  ...
  void connect(const CameraConfig &config);
  void disconnect() noexcept;
  bool is_connected() const noexcept;

  DeviceInfo info() const;
  Intrinsics intrinsics(double depth_scale_m = 0.001) const;
  Frames read(uint32_t timeout_ms);
