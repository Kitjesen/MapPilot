...
double compatibleEnvDouble(const char *canonical, const char *compatibility, double fallback) {
...
void validate(const Config &cfg) {
  if (cfg.robot != "doso" && cfg.robot != "go2") {
    throw std::runtime_error("driver robot must be doso or go2");
  }
  if (cfg.domain_id < 0) {
  ...
  if (cfg.robot == "go2") {
    if (cfg.network_interface.empty() ||
        cfg.network_interface.find_first_not_of(
            ...
            std::string::npos) {
      throw std::runtime_error("Go2 requires a valid network interface name");
    }
  } else {
    if (cfg.host.empty()) {
    ...
    const bool loopback =
        normalized_host == "localhost" || normalized_host == "localhost.localdomain" ||
        normalized_host == "127.0.0.1" || normalized_host == "::1" || normalized_host == "[::1]";
    ...
    if (!loopback && !cfg.brainstem_tls.enabled()) {
      throw std::runtime_error("remote Brainstem requires mTLS credentials");
    }
  }
  if (!std::isfinite(cfg.poll_hz) || cfg.poll_hz < kMinimumPollHz || cfg.poll_hz > 1000.0) {
    throw std::runtime_error("poll rate must be finite and within [10, 1000] Hz");
  }
...
Config loadConfig(int argc, char **argv) {
  ...
  cfg.brainstem_tls.ca_file =
      envString("LINGTU_DRIVER_TLS_CA_FILE", envString("LINGTU_BRAINSTEM_TLS_CA_FILE", ""));
  cfg.brainstem_tls.certificate_file =
      envString("LINGTU_DRIVER_TLS_CERT_FILE", envString("LINGTU_BRAINSTEM_TLS_CERT_FILE", ""));
  cfg.brainstem_tls.private_key_file =
      envString("LINGTU_DRIVER_TLS_KEY_FILE", envString("LINGTU_BRAINSTEM_TLS_KEY_FILE", ""));
  cfg.brainstem_tls.server_name =
      envString("LINGTU_DRIVER_TLS_SERVER_NAME", envString("LINGTU_BRAINSTEM_TLS_SERVER_NAME", ""));
  cfg.limits.max_linear_mps =
      compatibleEnvDouble("LINGTU_DRIVER_MAX_LINEAR_MPS", "LINGTU_BRAINSTEM_MAX_LINEAR", 1.0);
  cfg.limits.max_angular_rps =
      compatibleEnvDouble("LINGTU_DRIVER_MAX_ANGULAR_RPS", "LINGTU_BRAINSTEM_MAX_ANGULAR", 1.0);
  cfg.limits.command_timeout = checkedMilliseconds(
      compatibleEnvDouble("LINGTU_DRIVER_CMD_TIMEOUT_MS", "LINGTU_BRAINSTEM_CMD_TIMEOUT_MS", 200.0),
      "command timeout");
  cfg.poll_hz = envDouble("LINGTU_DRIVER_POLL_HZ", 100.0);
  cfg.rpc_timeout =
      checkedMilliseconds(envDouble("LINGTU_DRIVER_RPC_TIMEOUT_MS", 100.0), "RPC timeout");
  cfg.reconnect_delay =
      checkedMilliseconds(envDouble("LINGTU_DRIVER_RECONNECT_MS", 1000.0), "reconnect delay");
  cfg.status_file = envString("LINGTU_DRIVER_STATUS_FILE", "/dev/shm/lingtu/driver_status.json");
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--robot") {
      ...
      cfg.rpc_timeout = checkedMilliseconds(parseDouble(next(), "RPC timeout"), "RPC timeout");
...
const char *usage() noexcept {
  return "usage: lingtu_driver [options]\n"
         "  --robot NAME          doso or go2\n"
         "  --network-interface IFACE  Go2 DDS discovery interface\n"
         ...
         "  --max-linear MPS       physical vx/vy limit in m/s\n"
         "  --max-angular RPS      physical yaw-rate limit in rad/s\n"
         "  --cmd-timeout-ms MS    zero command watchdog\n"
         "  --poll-hz HZ           DDS polling rate (minimum 10 Hz)\n"
         "  --rpc-timeout-ms MS    per-adapter command deadline\n"
