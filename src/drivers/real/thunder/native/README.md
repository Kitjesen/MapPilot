...
`lingtu_driver` is the product `driver` backend for Thunder. It consumes the
canonical `/nav/cmd_vel` command from the derived DDS wire topic
`rt/nav/cmd_vel` and sends normalized `RobotControl.Walk(Vector3)` calls to the
Brainstem service running on the separate robot-control computer.
...
LINGTU_BRAINSTEM_TLS_SERVER_NAME=<optional-certificate-san>
```
...
`HAN_DOG_LINGTU_ALLOWED_IPS=<lingtu-host-ip>` plus the Brainstem server
