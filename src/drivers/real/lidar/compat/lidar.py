...
class Lidar:
    ...
    def connect(self, ip: str | None = None) -> Lidar:
        ...
        try:
            ...
            if self._start_driver:
                ...
                logger.info(
                    "Lidar native driver start skipped; expecting external stream on %s/%s",
                    self._scan_topic,
                    self._imu_topic,
                )

            # 2) Start DDS bridge (subscribe to scan + imu topics)
            if not self._start_dds_bridge():
                raise RuntimeError(f"DDS bridge unavailable for LiDAR topics {self._scan_topic}/{self._imu_topic}")
    ...
    def health(self) -> LidarHealth:
        ...
        # Update driver PID (native driver path is deprecated; may be None)
        if self._native and hasattr(self._native, "_process") and self._native._process:
            proc = self._native._process
            h.driver_pid = proc.pid if proc.poll() is None else None
            h.driver_restarts = getattr(self._native, "_restart_count", 0)
        return h
    ...
    def __repr__(self) -> str:
        return f"Lidar(ip={self._ip!r}, state={self._state.value}, fps={self.fps:.1f})"
    ...
    def _start_native_driver(self, cfg) -> None:
        """Deprecated — NativeModule driver start is no longer supported.

        Logs a deprecation warning and returns without starting any process.
        Callers should use the DDS bridge (default) or manage the Livox
        driver externally via systemd.
        """
        import warnings

        msg = (
            "Lidar(start_driver=True) is deprecated. The NativeModule-based "
            "ROS2 driver launch has been removed. Use the DDS bridge "
            "(default, start_driver=False) or manage the Livox driver "
            "process externally via systemd (lingtu-lidar.service)."
        )
        warnings.warn(msg, DeprecationWarning, stacklevel=2)
        logger.warning(msg)

    def _start_dds_bridge(self) -> bool:
        """Subscribe to LiDAR scan and IMU topics via cyclonedds.

        No rclpy needed - cyclonedds talks DDS directly.
        """
        started = self._dds_adapter.start()
