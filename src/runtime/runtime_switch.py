...
def runtime_spec_summary(spec: RuntimeRunSpec) -> dict[str, object]:
    ...
    return {
        ...
        "nav_in_adapter": spec.nav_in_adapter,
        "nav_out_adapter": spec.nav_out_adapter,
        ...
        "autonomy_backends": {
            str(name): str(value)
            for name, value in spec.autonomy_backends.items()
        },
...
def _startup_gates_for_spec(spec: RuntimeRunSpec) -> list[str]:
    gates: list[str] = []
    if spec.global_planner in {"octoplanner3d", "pct"}:
...
def validate_runtime_switch(spec: RuntimeRunSpec) -> RuntimeSwitchValidation:
    ...
    if spec.nav_in_adapter and (
        spec.env.get("LINGTU_NAV_IN_ADAPTER") != spec.nav_in_adapter
    ):
        blockers.append("env nav input adapter does not match run spec")
    if not spec.nav_in_adapter and "LINGTU_NAV_IN_ADAPTER" in spec.env:
        blockers.append("env nav input adapter exists without run spec adapter")
    if spec.nav_out_adapter and (
        spec.env.get("LINGTU_NAV_OUT_ADAPTER") != spec.nav_out_adapter
    ):
        blockers.append("env nav output adapter does not match run spec")
    if not spec.nav_out_adapter and "LINGTU_NAV_OUT_ADAPTER" in spec.env:
        blockers.append("env nav output adapter exists without run spec adapter")
...
def _product_semantic_warnings(
    ...
    for item in overrides:
        ...
        warnings.append(
            f"runtime compatibility override: {field} {product_value} -> {endpoint_value}"
        )
