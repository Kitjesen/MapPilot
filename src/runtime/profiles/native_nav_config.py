...
def native_nav_profile_config(
    ...
    default_control_mode = str(config.get("native_control_mode") or "").strip().lower()
    if not default_control_mode:
        if canonical_profile == "teleop_avoid":
            default_control_mode = "teleop_avoid"
        elif canonical_profile in {"teleop", "map"}:
            default_control_mode = "teleop"
        else:
            default_control_mode = "autonomy"
    raw_control_mode = str(
        _native_value(config, native_nav_config, "control_mode", default_control_mode)
    ).strip().lower()
    ...
    return NativeNavProfileConfig(canonical_profile, parameters, native_nav, fingerprint)
