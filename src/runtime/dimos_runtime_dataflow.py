...
from .runtime_interface import TOPICS
...
def _latest_local_path_status(
    ...
    path_diagnostics = _mapping(report.get("path_diagnostics")) or _mapping(
        case.get("path_diagnostics")
    )
    path_latest = _mapping(_mapping(path_diagnostics.get("local_path")).get("latest"))
    autonomy_chain = _mapping(report.get("autonomy_chain")) or _mapping(
        case.get("autonomy_chain")
    )
    ...
    path_follower = _mapping(
        _mapping(autonomy_chain.get("path_follower")).get("path_follower")
    )
...
def _pct_provenance_from_live(
    ...
    fallback_flags = {
        "navigation_chain.planner_fallback_used": navigation_chain.get(
            "planner_fallback_used"
        ),
        "navigation_chain.planner_repair_used": navigation_chain.get(
            "planner_repair_used"
        ),
...
def _same_source_provenance_from_live(
    ...
    deliverable_checks = _mapping(
        _nested_mapping_value(report, "deliverable_contract", "checks")
    )
    ...
    tomogram_expected = bool(
        tomogram
        or tomogram_asset
        or same_source_tomogram is not None
    )
    parent_match = bool(
        world_parent and tomogram_parent and world_parent == tomogram_parent
    )
    checked = any(
        value not in (None, "")
        for value in (
    ...
    pcd_proven = (
        same_source_pcd is True
        and bool(map_pcd_sha256)
        and map_pcd_point_count > 0
    )
    ...
    supporting_checks_ok = (
        same_source_check is not False
        and map_artifacts_ok is not False
    )
    ...
    *,
    ...
    edges: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            and (
                latest_local_path.get("checked") is not True
                or latest_local_path.get("ok") is True
            ),
            "evidence": {
...
def _child_report_freshness(
    *,
    ...
    if parent_time > 0.0 and age_vs_parent_s > max_parent_skew_s:
        blockers.append(
            f"child_report_age_vs_parent_s {age_vs_parent_s:.3f} > "
            f"{max_parent_skew_s:.3f}"
        )
    if child_age_s is not None and child_age_s > max_absolute_age_s:
        blockers.append(
            f"child_report_age_s {child_age_s:.3f} > "
            f"{max_absolute_age_s:.3f}"
        )
    return {
        "checked": True,
        "fresh": not blockers,
...
def _failed_edge_ids(summary: Mapping[str, Any]) -> list[str]:
    return [
        ...
        and edge.get("ok") is not True
        and str(edge.get("id") or "")
    ]
...
def _candidate_rejection_reason(
    ...
    if summary.get("ok") is True:
        return (
            "candidate dataflow passes but is not the expected "
            f"{gate_name} gate report"
        )
...
def _has_native_pct_shape(report: Mapping[str, Any]) -> bool:
    return (
        str(report.get("schema_version") or "").startswith(
            "lingtu.native_pct_mujoco_gate"
        )
...
def _has_pct_saved_map_navigation_shape(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        or str(report.get("validation_level") or "")
        == "saved_map_relocalized_pct_navigation"
        or (
...
def _has_large_terrain_shape(report: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        or (
            str(report.get("selection_policy") or "")
            == "first_route_ok_after_primary"
...
def _has_dynamic_obstacle_local_planner_shape(report: Mapping[str, Any]) -> bool:
    schema = str(report.get("schema_version") or "")
    return (
        schema.startswith("lingtu.dynamic_obstacle_local_planner")
        or (
...
def _has_gazebo_runtime_shape(report: Mapping[str, Any]) -> bool:
    schema = str(report.get("schema_version") or "")
    return (
        schema.startswith("lingtu.gazebo_runtime_smoke")
        or (
...
def _has_moving_obstacle_sweep_shape(report: Mapping[str, Any]) -> bool:
    ...
    return schema.startswith("lingtu.moving_obstacle_sweep_gate") or (
        isinstance(report.get("required_pairs"), list)
        and "passed_pair_count" in report
    )
...
def _has_large_loop_closure_shape(report: Mapping[str, Any]) -> bool:
    ...
    return schema.startswith("lingtu.large_loop_closure_gate") or (
        isinstance(report.get("thresholds"), Mapping)
        and "passed_case_count" in report
    )
...
def _summarize_gazebo_runtime_report(
    ...
    *,
    ...
    nav_samples = _mapping(
        _value_from(nav_loop.get("samples"), nav_loop.get("topic_samples"))
    )
    ...
    frontier_goal_ok = (
        isinstance(frontier_goal, list)
        and len(frontier_goal) >= 2
        and _safe_float(frontier_goal[0]) >= 0.75
    )
    trajectory_ok = (
        ...
        and _safe_int(
            trajectory_quality.get("local_path_occupied_overlap_count")
        )
        == 0
        and _safe_float(trajectory_quality.get("min_obstacle_clearance_m")) >= 0.18
    )
    sync_ok = (
        topic_sync.get("ok") is True
        and _safe_float(topic_sync.get("max_cloud_odom_skew_ms")) <= 250.0
    )
    frontier_ok = (
    ...
    frontier_stall_ok = (
        ...
        and str(frontier_stall.get("stop_reason") or "")
    ...
    tare_ok = (
        ...
        and (
            tare.get("runtime_required") is not True
            or tare.get("runtime_available") is True
        )
    )
    ...
    edges = [
        _edge(
            ...
            tf_ok,
            {
                ...
                "topic_samples": {
                    topic: _safe_int(samples.get(topic)) for topic in tf_topics
                },
                "topic_frames": {topic: frames.get(topic) for topic in tf_topics},
                "point_counts": {
                    topic: _safe_int(point_counts.get(topic)) for topic in tf_topics
                },
            },
        ),
        _edge(
            ...
            nav_loop_ok,
            {
                "ok": nav_loop.get("ok"),
                "required_flags": {
                    key: nav_loop.get(key) for key in nav_required_flags
                },
                "odom_delta_m": _safe_float(nav_loop.get("odom_delta_m")),
                "topic_samples": {
                    topic: _safe_int(nav_samples.get(topic))
        ...
        _edge(
            ...
            frontier_ok,
            {
                "ok": frontier.get("ok"),
                "required_flags": {
                    key: frontier.get(key) for key in frontier_required_flags
                },
                ...
                "explored_area_delta_m2": _safe_float(
                    frontier.get("explored_area_delta_m2")
                ),
                "frontier_count_max": _safe_int(
                    frontier.get("frontier_count_max")
                ),
                "frontier_goal": frontier_goal,
                "topic_samples": {
                    topic: _safe_int(frontier_samples.get(topic))
        ...
        _edge(
            ...
            cumulative_ok,
            {
                "cumulative_map_cloud_seen": frontier.get(
                    "cumulative_map_cloud_seen"
                ),
        ...
        _edge(
            ...
            non_motion_ok,
            {
                "gazebo_runtime": {
                    ...
                    "cmd_vel_sent_to_hardware": report.get(
                ...
                "nav_loop": {
                    ...
                    "cmd_vel_sent_to_hardware": nav_loop.get(
                        "cmd_vel_sent_to_hardware"
                    ),
                },
                "frontier_exploration": {
                    ...
                    "cmd_vel_sent_to_hardware": frontier.get(
                        "cmd_vel_sent_to_hardware"
                    ),
                },
                "tare_exploration": {
                    ...
                    "cmd_vel_sent_to_hardware": tare.get(
                        "cmd_vel_sent_to_hardware"
                    ),
                },
            },
...
def _summarize_moving_obstacle_sweep_report(
    ...
    *,
    ...
    required_speed_bins = [
        str(value) for value in report.get("required_speed_bins") or []
    ]
    required_density_bins = [
        str(value) for value in report.get("required_density_bins") or []
    ]
    ...
    for index, case in enumerate(cases):
        ...
        clearance_status[pair] = (
            case.get("ok") is True
            and _safe_float(case.get("trail_min_margin_m")) >= min_clearance_m
        )
        scan_time_status[pair] = (
            str(case.get("scan_time_profile") or "") == "physical_rolling"
        )
        video = _mapping(case.get("video"))
        video_status[pair] = (
            not require_video_file
            or (
...
def _summarize_large_loop_closure_report(
    ...
    *,
    ...
    max_loop_error_m = (
        _safe_float(thresholds.get("max_loop_closure_error_m")) or 0.75
    )
    max_fastlio_loop_error_m = (
        _safe_float(thresholds.get("max_fastlio_loop_closure_error_m")) or 1.0
    )
    max_loop_yaw_error_rad = (
        _safe_float(thresholds.get("max_loop_yaw_error_rad")) or 0.5
    )
    ...
    long_motion_ok = (
        ...
        and _safe_float(best_case.get("fastlio2_path_length_m"))
        >= min_path_length_m * 0.8
        and _safe_float(best_case.get("goal_span_m")) >= min_goal_span_m
    )
    closure_ok = (
        matrix_ok
        and _safe_float(best_case.get("sim_loop_closure_error_m"))
        <= max_loop_error_m
        and _safe_float(best_case.get("fastlio2_loop_closure_error_m"))
        <= max_fastlio_loop_error_m
        and _safe_float(best_case.get("sim_loop_yaw_error_rad"))
        <= max_loop_yaw_error_rad
        and _safe_float(best_case.get("fastlio2_loop_yaw_error_rad"))
        <= max_loop_yaw_error_rad
    )
    nav_chain_ok = (
        matrix_ok
    ...
    edges = [
        ...
        _edge(
            ...
            long_motion_ok,
            {
                ...
                "sim_path_length_m": _safe_float(
                    best_case.get("sim_path_length_m")
                ),
                "fastlio2_path_length_m": _safe_float(
                    best_case.get("fastlio2_path_length_m")
                ),
        ...
        _edge(
            ...
            closure_ok,
            {
                "sim_loop_closure_error_m": _safe_float(
                    best_case.get("sim_loop_closure_error_m")
                ),
                "fastlio2_loop_closure_error_m": _safe_float(
                    best_case.get("fastlio2_loop_closure_error_m")
                ),
                "sim_loop_yaw_error_rad": _safe_float(
                    best_case.get("sim_loop_yaw_error_rad")
                ),
                "fastlio2_loop_yaw_error_rad": _safe_float(
                    best_case.get("fastlio2_loop_yaw_error_rad")
                ),
        ...
        _edge(
            ...
            nav_chain_ok,
            {
                "nav_cmd_vel_nonzero": _safe_int(
                    best_case.get("nav_cmd_vel_nonzero")
                ),
...
def _summarize_dynamic_obstacle_local_planner_report(
    ...
    *,
    ...
    backend_requested = str(
        report.get("backend_requested") or local_backend.get("requested") or ""
    )
    backend_ok = (
    ...
    obstacle_clearance_ok = (
        report.get("obstacle_response_verified") is True
        and clearance >= 0.25
    )
    clear_recovery_ok = (
    ...
    edges = [
        ...
        _edge(
            ...
            clear_recovery_ok,
            {
                "clear_path_recovery_verified": report.get(
                    "clear_path_recovery_verified"
                ),
...
def _large_terrain_pct_provenance(report: Mapping[str, Any]) -> dict[str, Any]:
    pct_rows = [
        ...
        if str(_value_from(row.get("planner_requested"), row.get("planner")) or "").lower()
        == "pct"
    ...
    fallback_rows = [
        row
        for row in pct_rows
        if str(row.get("selected_planner") or "").lower() != "pct"
        or bool(str(row.get("fallback_reason") or ""))
    ]
    native_rows = [
        row
        for row in pct_rows
    ...
    runtime_ok = (
        ...
        or bool(pct_rows)
        and all(_mapping(row.get("native_runtime")).get("ok") is True for row in pct_rows)
    )
...
def _large_terrain_case_safety(case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    pct_plans = [
        plan
        for plan in plans
        if str(_value_from(plan.get("planner_requested"), plan.get("planner")) or "").lower()
        == "pct"
...
def _summarize_large_terrain_report(
    ...
    *,
    ...
    safe_matrix_ok = bool(case_safety) and all(
        item["ok"] and item["route_ok"] and item["safety_ok"] and item["distance_ok"]
        for item in case_safety
    )
    edges = [
        _edge(
...
def _summarize_policy_nav_report(
    ...
    *,
    ...
    nav_checks = [
        check
        for check in checks
        if str(check.get("mode") or "") == "full_stack_policy_nav"
    ]
    nav = _mapping(nav_checks[-1]) if nav_checks else {}
    direct_checks = [
        check for check in checks if str(check.get("mode") or "") == "direct_policy"
    ]
    ...
    close_enough = nav.get("success_seen") is True or (
        _optional_float(dist_for_gate) is not None
        and _safe_float(dist_for_gate) <= 0.10
    )
    ...
    edges = [
        ...
        _edge(
            ...
            planner_configured == "octoplanner3d"
            and planner_status.get("degraded") is not True,
            {
        ...
        _edge(
            ...
            and (local_nonempty_count > 0 or last_local_points >= 2),
            {
                ...
                "local_planner_backend_requested": nav.get(
                    "local_planner_backend_requested"
                ),
                ...
                "control_hint_safety_stop_count": _safe_int(
                    nav.get("control_hint_safety_stop_count")
                ),
            },
        ),
        _edge(
            ...
            and path_cmd_nonzero > 0,
            {
                ...
                "path_follower_backend_requested": nav.get(
                    "path_follower_backend_requested"
                ),
        ...
        _edge(
            ...
            _safe_int(seen.get("mux_cmd")) > 3
            and mux_cmd_nonzero > 0
            and _safe_int(seen.get("direct_fallback")) == 0,
            {
        ...
        _edge(
            ...
            contact_ok,
            {
                "foot_contact_sample_count": _safe_int(
                    contacts.get("foot_contact_sample_count")
                ),
                "unique_feet_count": _safe_int(contacts.get("unique_feet_count")),
                "non_foot_ground_contacts": _safe_int(
                    contacts.get("non_foot_ground_contacts")
                ),
            },
        ),
        _edge(
            "planner_map_ready",
            _safe_int(seen.get("costmap")) > 0
            or costmap_readiness.get("planner_has_map") is True,
            {
...
def _summarize_pct_saved_map_navigation_report(
    ...
    *,
    ...
    native_path_ok = (
        _safe_int(native.get("path_count")) > 0
        or _safe_int(native.get("max_path_poses")) >= 2
    )
    ...
    same_source_ok = (
        same_source_provenance.get("ok") is True
        and same_source_hash_ok
    )
    edges = [
        _edge(
        ...
        _edge(
            ...
            same_source_ok,
            {
                ...
                "same_source_hash_checks": same_source_hash_identity.get("checks")
                or {},
            },
        ),
        _edge(
            ...
            native_pct_ok,
            {
                ...
                "selected_planner": native.get("selected_planner")
                or native.get("planner"),
...
def _summarize_saved_map_relocalize_report(
    ...
    *,
    ...
    service_ok = service.get("available") is True and (
        ...
        or (
            global_reloc
            and "already running" in str(service.get("message") or "").lower()
        )
    )
    saved_map_cloud_ok = (
    ...
    health_ok = (
        _safe_int(localizer.get("tracking_health_samples")) >= min_tracking
        and health_state in {"LOCKED", "RECOVERED"}
    )
    tf_ok = (
        _safe_int(localizer.get("map_to_odom_tf_samples")) > 0
        and map_xy <= max_xy
        and map_z <= max_z
    )
    global_reloc_ok = (
    ...
    edges = [
        ...
        _edge(
            ...
            saved_map_cloud_ok,
            {
                "saved_map_cloud_samples": _safe_int(
                    localizer.get("saved_map_cloud_samples")
                ),
                "saved_map_cloud_points_latest": _safe_int(
                    localizer.get("saved_map_cloud_points_latest")
                ),
        ...
        _edge(
            ...
            health_ok,
            {
                ...
                "tracking_health_samples": _safe_int(
                    localizer.get("tracking_health_samples")
                ),
        ...
        _edge(
            ...
            tf_ok,
            {
                "map_to_odom_tf_samples": _safe_int(
                    localizer.get("map_to_odom_tf_samples")
                ),
        ...
        _edge(
            ...
            {
                "global_relocalization_requested": global_reloc,
                "global_relocalization_validated": report.get(
                    "global_relocalization_validated"
                ),
                "bbs3d_success_observed": localizer.get("bbs3d_success_observed"),
                "bbs3d_disabled_observed": localizer.get("bbs3d_disabled_observed"),
                "lost_health_samples": _safe_int(
                    localizer.get("lost_health_samples")
                ),
            },
        ),
    ]
...
def _summarize_aggregate_case(
    ...
    *,
    ...
    inspection = _mapping(
        _value_from(live.get("lingtu_inspection"), case.get("lingtu_inspection"))
    )
    ...
    return {
        ...
        "remaining_gaps": list(
            _value_from(live.get("remaining_gaps"), case.get("remaining_gaps")) or []
        ),
...
def runtime_dataflow_for_gate(
    ...
    *,
    ...
    gate_report_path = _repo_path(
        gate.get("path")
        or action.get("report_path")
        or action.get("expected_report_path"),
        root=root,
    )
    ...
    if not _has_live_dataflow_shape(gate_report) and not _has_native_pct_shape(
        gate_report
    ):
