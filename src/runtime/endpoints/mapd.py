...
SESSION_ROOT_ENV = "LINGTU_SESSION_ROOT"
...
class ArtifactHandle:
    """One regular-file descriptor returned by ``mapd``."""
...
class MapClient:
    ...
    def __init__(
        ...
        *,
        ...
        configured = socket_path or os.environ.get(SOCKET_PATH_ENV)
        if not configured:
            session_root = os.environ.get(SESSION_ROOT_ENV)
            configured = os.path.join(session_root, "mapd.sock") if session_root else DEFAULT_SOCKET_PATH
        self.socket_path = os.fspath(configured)
        self.timeout_s = max(0.001, min(_MAX_TIMEOUT_S, float(timeout_s)))
    ...
    def service(self, action: str, **arguments: Any) -> dict[str, Any]:
        """Run one bounded map-management operation in the owning ``mapd``."""

        action_value = str(action or "").strip()
        if not action_value:
            raise ValueError("action must be a non-empty string")
        body: dict[str, Any] = {"action": action_value}
        for name, value in arguments.items():
            if value is not None and not isinstance(value, (str, bool, int, float)):
                raise TypeError(f"service argument {name!r} must be a JSON scalar")
            body[str(name)] = value
        try:
            response, _ = self._request(_SERVICE, body)
            return response
        except MapClientError as exc:
            # Domain failures preserve the historical service API: callers get
            # the native result object. Transport/protocol failures have no
            # response body and still raise.
            if exc.response:
                return dict(exc.response)
            raise
    ...
    def open_artifact(self, map_id: str, capability: str) -> ArtifactHandle:
        ...
        response, descriptors = self._request(
            _OPEN_ARTIFACT,
            {"map_id": str(map_id), "capability": str(capability)},
            expect_fd=True,
        )
        ...
        media_type = str(
            artifact_metadata.get("media_type") or artifact_metadata.get("mime") or "application/octet-stream"
        ).strip()
    ...
    def _request(
        ...
        *,
        ...
        if len(request_id) > _MAX_REQUEST_ID_BYTES:
            raise ValueError("request_id exceeds the wire limit")
        try:
            body = json.dumps(dict(arguments), separators=(",", ":"), ensure_ascii=False).encode("utf-8")
        except (TypeError, ValueError) as exc:
            raise TypeError("mapd request arguments must be JSON serializable") from exc
        if len(body) > _MAX_REQUEST_BYTES:
            raise ValueError("mapd request exceeds the size limit")
        deadline_ms = max(1, min(_MAX_DEADLINE_MS, int(self.timeout_s * 1000.0)))
        frame = (
    ...
    def _decode_response(
        ...
        try:
            magic, version, status_code, flags, body_size, binary_size = _RESPONSE_HEADER.unpack(raw_header)
            ...
            if binary_size:
                raise MapClientError("invalid_response", "inline binary mapd responses are not supported")
...
def _receive_header_with_descriptors(transport: Any) -> tuple[bytes, list[int]]:
    ...
    ancillary_size = socket.CMSG_SPACE(descriptor_bytes * 2) if hasattr(socket, "CMSG_SPACE") else descriptor_bytes * 8
