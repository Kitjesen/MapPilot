"""Schema-aware JSON payloads for endpoint adapters."""
...
from runtime.transport.json_codec import FORMAT as ENDPOINT_PAYLOAD_FORMAT
...
_BINARY_ENCODERS = {
    ...
    "lingtu.dds.LivoxFrame": PointCloud2,
    "lingtu.dds.PointCloud2": PointCloud2,
...
_DICT_ENCODERS = {
    ...
    "lingtu.dds.Odometry": Odometry,
    "lingtu.nav.path.v1": Path,
    "lingtu.dds.Path": Path,
    "lingtu.geometry.pose_stamped.v1": PoseStamped,
    "lingtu.dds.PoseStamped": PoseStamped,
    "lingtu.geometry.twist.v1": Twist,
    "lingtu.dds.TwistStamped": Twist,
}
...
def dumps_endpoint_message(binding: Any, msg: Any) -> bytes:
...
def loads_endpoint_message(binding: Any, data: Any) -> Any:
    """Decode one contract-bound endpoint message.

    The adapter also accepts the generic ``runtime.transport.json_codec`` envelope
    so older same-Python endpoint peers can interoperate during migration.
    """
    ...
    if envelope.get("format") != ENDPOINT_PAYLOAD_FORMAT:
        raise ValueError("unsupported LingTu endpoint payload")
...
def _validate_endpoint_envelope(binding: Any, envelope: Mapping[str, Any]) -> None:
    try:
        version = int(envelope.get("schema_version"))
    except (TypeError, ValueError) as exc:
        raise ValueError(f"endpoint schema_version missing for {binding.topic}") from exc
    if version != SCHEMA_VERSION:
        raise ValueError(
            f"endpoint schema_version mismatch for {binding.topic}: "
    ...
    try:
        ...
        raise ValueError(f"endpoint timestamp missing for {binding.topic}") from exc
    except (TypeError, ValueError) as exc:
        raise ValueError(f"endpoint timestamp invalid for {binding.topic}") from exc
...
def _encode_payload(schema: str, msg: Any) -> Any:
    ...
    if schema in {"lingtu.status.localization_quality.v1", "lingtu.dds.Float32"}:
        return float(msg)
    if schema in {"lingtu.status.localization_health.v1", "lingtu.dds.Text"}:
