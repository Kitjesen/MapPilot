...
``runtime.transport``, and optionally attaches a hardware/source plugin that
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "source, 'jsonl' for a no-ROS JSONL sensor/localization feed, "
            "'thunder_brainstem' for the Thunder Brainstem command sink, or "
            "'thunder_field' for configured product field sources. Multiple "
...
def _thunder_field_source_specs() -> list[str]:
    specs: list[str] = ["thunder_brainstem"]
...
def _load_source(spec: str) -> Any:
    ...
    if spec in {"jsonl", "jsonl_replay", "replay_jsonl", "builtin:jsonl"}:
        ...
        return create()
    if spec in {"thunder_brainstem", "brainstem", "builtin:thunder_brainstem"}:
