"""LCM output adapter for Thunder navigation."""
...
@register(
    "navigation",
    "lcm_nav_output",
    description="LCM navigation output adapter for Thunder",
)
class LCMNavOutModule(Module, layer=5):
    ...
    def __init__(
        ...
        **kw: Any,
        ...
        self._backend_status = BackendStatus.configured_as("lcm_nav_output")
    ...
    def setup(self) -> None:
        ...
        logger.info(
            "LCMNavOutModule: publishing navigation egress for %s",
            self._contract.name,
        )
    ...
    def _on_cmd_vel(self, twist: Twist) -> None:
        if not isinstance(twist, Twist):
            ...
            logger.warning(
                "LCMNavOutModule ignored non-Twist cmd_vel: %s",
                type(twist).__name__,
            )
            return
    ...
    def _publish_payload(self, binding: LCMEndpointBinding, payload: bytes) -> None:
        ...
        if transport is None:
            raise RuntimeError("LCM nav output adapter transport is not initialized")
        ...
        if publisher is None:
            ...
            if not callable(create_publisher):
                raise TypeError(
                    "LCM nav output adapter transport must expose publish() "
                    "or create_publisher()"
                )
