"""lingtu.runtime.stream — Typed data-flow ports and pluggable transport layer.
...
Transport and LocalTransport are now defined in runtime.transport.local and
...
class Out(Generic[T]):
    ...
    Two message delivery paths:
      ...
      2. External transports — bound via _bind_transport(), additionally sent by publish()
    ...
    def __init__(self, msg_type: type, name: str, owner: Any = None) -> None:
        ...
        self._transport_bindings: list[tuple[Transport, str]] = []
    ...
    def publish(self, msg: T) -> None:
        ...
        with self._lock:
            ...
            transport_bindings = list(self._transport_bindings)
        for cb in cbs:
            try:
                cb(msg)
            except Exception:
        ...
        # External transports
        for transport, topic in transport_bindings:
            try:
    ...
    def _bind_transport(self, transport: Transport, topic: str | None = None) -> None:
        ...
        topic_name = topic or self._name
        with self._lock:
            for existing_transport, existing_topic in self._transport_bindings:
                if existing_transport is transport and existing_topic == topic_name:
                    self._transport = transport
                    self._transport_topic = topic_name
                    return
            self._transport_bindings.append((transport, topic_name))
            self._transport = transport
            self._transport_topic = topic_name
    ...
    def __repr__(self) -> str:
        if not self._transport_bindings:
            transport_str = ""
        elif len(self._transport_bindings) == 1:
            transport_str = f", transport={self._transport_topic}"
        else:
            transport_str = f", transports={len(self._transport_bindings)}"
