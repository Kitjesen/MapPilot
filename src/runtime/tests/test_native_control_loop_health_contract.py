...
def test_both_cmake_surfaces_build_core_and_register_focused_test() -> None:
    ...
    for cmake in (portable, endpoint):
        assert "test_control_loop_health.cpp" in cmake
        assert "control_loop_health.cpp" in cmake
        assert re.search(r"add_test\s*\(\s*NAME\s+test_control_loop_health\b", cmake)
