...
def test_saved_map_artifact_dir_gate_accepts_same_source_metadata(tmp_path: Path):
    ...
    assert set(result["checked_artifact_formats"]) == set(ARTIFACT_FORMATS)
...
def test_saved_map_artifact_dir_gate_rejects_cwd_relative_artifact_paths(
    tmp_path: Path,
    monkeypatch,
):
    from maps.artifacts import (
...
def test_saved_map_artifact_dir_gate_rejects_escaping_artifact_paths(tmp_path: Path):
    ...
    artifact_dir = tmp_path / "map"
    artifact_dir.mkdir()
    outside = tmp_path / "outside.pcd"
    outside.write_text("VERSION 0.7\nDATA ascii\n", encoding="ascii")
    metadata = build_saved_map_metadata(
        source_profile="mujoco_fastlio2_live_gate",
        data_source="fastlio2",
        slam_source="fastlio2",
    ...
    assert result["ok"] is False
    assert "map_pcd path escapes map directory" in result["blockers"]
