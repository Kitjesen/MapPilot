...
# ---------------------------------------------------------------------------
# W1-1  LocalPlanner -no silent fallback to straight-line paths
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# W1-2  Terrain -no silent fallback when native kernel missing
# ---------------------------------------------------------------------------
...
class TestW1TerrainNoFallback(unittest.TestCase):
    def test_nanobind_backend_degrades_to_simple_when_nav_kernel_missing(self):
        ...
        with mock.patch.object(
            ...
            side_effect=RuntimeError("Terrain [nanobind]: compatible LingTu native navigation kernel not found"),
        ):
...
class TestW1WaypointTrackerYaw(unittest.TestCase):
    ...
    def test_distance_only_legacy_behaviour_preserved(self):
        ...
        from nav.tracking.waypoint_tracker import EV_STUCK, EV_STUCK_WARN
...
class TestW1MockLLMSignals(unittest.TestCase):
    def test_mock_response_is_flagged_and_confidence_zero(self):
        ...
        self.assertTrue(payload.get("mock"), "Mock responses must be tagged with mock=True")
        self.assertEqual(
            payload.get("confidence"),
            0.0,
            "Mock must report confidence=0 so downstream never trusts its output as real inference.",
        )
