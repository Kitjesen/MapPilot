...
def test_route_allows_explicit_external_state_diagnostic_with_odometry_schema() -> None:
    contract = load_route_contract(robot())
    state_at_scan = contract.topic(TOPICS.state_estimation_at_scan)

    assert state_at_scan.consumers == ()
    assert state_at_scan.external_diagnostics_subscribable is True
    assert state_at_scan.role.endswith("_diagnostic")
    assert not any(
        issue.code == "topic_consumers_missing"
        and issue.scope == TOPICS.state_estimation_at_scan
        for issue in validate_route_contract(contract)
    )
...
def test_blueprint_records_route_metadata() -> None:
    graph = Blueprint().add(RouteModule).route_contract(robot()).export_graph(profile="unit")
...
def test_blueprint_route_contract_does_not_change_internal_delivery() -> None:
    graph = (
        Blueprint()
