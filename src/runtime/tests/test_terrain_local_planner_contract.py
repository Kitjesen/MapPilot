...
def test_nanobind_local_planner_applies_cmu_parity_config(monkeypatch):
    ...
    def fake_create_nanobind_backend():
        return create_nanobind_backend(
            nav_kernel_importer=lambda *_args, **_kw: _FakeNavKernel,
            config_getter=lambda: cfg,
        )

    monkeypatch.setattr(runtime, "create_nanobind_backend", fake_create_nanobind_backend)
    _FakeLocalPlanner.captured_params = None
    module = LocalPlanner(backend="nanobind")
    module.setup()

    params = _FakeLocalPlanner.captured_params
...
def test_nanobind_local_planner_uses_adjacent_range_not_path_range_step():
    cfg = SimpleNamespace(
        geometry=SimpleNamespace(
            vehicle_length=1.0,
            vehicle_width=0.6,
    ...
    result = create_nanobind_backend(
        nav_kernel_importer=lambda _symbols: _FakeNavKernel,
        config_getter=lambda: cfg,
        paths_dir=str(
            _REPO_ROOT
            / "src"
    ...
    assert params.adjacent_range == 4.0
    assert params.path_range_step == 0.25
    assert params.path_scale == 0.9
    assert params.min_path_scale == 0.6
    assert params.path_scale_step == 0.1
    assert params.slope_weight == 3.5
    assert params.omni_dir_goal_thre == 1.7
    assert params.min_rel_z == -0.9
    assert params.max_rel_z == 0.35
...
def test_local_planner_default_path_frame_uses_runtime_topic_contract():
    module = LocalPlanner(backend="simple")
...
def test_local_planner_corridor_goal_prefers_current_floor_at_same_xy():
    module = LocalPlanner(backend="simple", corridor_lookahead_m=1.0)
...
def test_local_planner_merges_terrain_ext_and_honors_check_obstacle():
    module = LocalPlanner(backend="nanobind")
    module._on_terrain(
        PointCloud2(
            points=np.asarray([[1.0, 0.0, 0.0, 0.4]], dtype=np.float32),
            frame_id="map",
            ts=1.0,
        )
    )
    module._on_terrain_ext(
        PointCloud2(
            points=np.asarray([[2.0, 0.0, 0.0, 0.5]], dtype=np.float32),
            frame_id="map",
            ts=1.0,
        )
    )

    merged = module._merge_obstacle_clouds()

    assert merged.shape == (2, 4)
    np.testing.assert_allclose(merged[:, 0], np.asarray([1.0, 2.0], dtype=np.float32))

    module._on_check_obstacle(False)

    assert module._merge_obstacle_clouds().shape == (0, 4)


def _run_cmu_py_local_plan(obstacle_half_width: float) -> tuple[np.ndarray, np.ndarray]:
    module = LocalPlanner(backend="cmu_py")
    module.setup()
    published: list[Path] = []
    ...
    module._on_odom(
        Odometry(
            pose=Pose(
                position=Vector3(module._sensor_offset_x, module._sensor_offset_y, 0.0),
...
def test_nanobind_local_planner_publishes_control_hint_for_near_field_stop():
    module = LocalPlanner(backend="nanobind")
    module._core = _FakeLocalPlanner()
    hints: list[dict] = []
    ...
    assert hints[-1]["safety_stop_level"] == 2
    assert hints[-1]["reason"] == "nanobind"


def test_nanobind_local_planner_publishes_map_path_from_planning_origin():
    module = LocalPlanner(backend="nanobind")
    module._core = _FakeLocalPlanner()
    module._sensor_offset_x = 0.3
    module._sensor_offset_y = 0.1
    published: list[Path] = []
    module.local_path._add_callback(published.append)
    module._latest_waypoint = PoseStamped(
        pose=Pose(position=Vector3(4.0, 0.0, 0.0)),
    ...
    module._on_odom(
        Odometry(
            pose=Pose(
                ...
                orientation=Quaternion.from_yaw(float(np.pi / 2.0)),
            ),
            frame_id="map",
            child_frame_id="base_link",
            ts=1.0,
    ...
    assert published
    path = published[-1]
    assert path.frame_id == topic_default_frame_id(TOPICS.local_path)
