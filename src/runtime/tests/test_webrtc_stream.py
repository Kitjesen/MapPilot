"""Unit tests for WebRTCStreamModule 鈥?signalling-layer helpers only.
...
try:
    from gateway.media.webrtc_stream import (
        WebRTCStreamModule,
        _force_h264_first,
        _HAVE_WEBRTC,
    )
except ImportError:  # pragma: no cover 鈥?module itself should always import
...
def test_collect_stats_idle_returns_zero_peers():
    ...
    # `enabled` mirrors `_HAVE_WEBRTC` 鈥?in CI without aiortc installed it's
