...
def test_host_bus_publishes_ordered_navigation_goal_status() -> None:
    bus = HostBus()
    observed = []
    ...
    assert observed[-1].to_dict()["state_name"] == "REACHED"
...
def test_host_bus_projects_ordered_native_exploration_run_events() -> None:
    bus = HostBus(require_exploration_run_events=True)
    observed = []
    bus.exploration_run_event._add_callback(observed.append)
    pending = [
        {
            "timestamp_s": 46.0,
            "frame_id": "map",
            ...
            "event_sequence": 1,
            "kind": 1,
            "exploration_run_id": "01K1M9S4FX27T8XMY6QJNBAV3W",
            "start_request_id": "01K1M9S4FX27T8XMY6QJNBAV3X",
            "command_request_id": "01K1M9S4FX27T8XMY6QJNBAV3X",
            "product_session_id": "product-session-7",
            "state": 1,
            "route": "live",
            "map_id": "",
            "map_content_epoch": 0,
            "reason": "start_admitted",
    ...
    class Session:
        def take_exploration_run_event(self):
            return pending.pop(0) if pending else None

    bus._session = Session()
    bus._exploration_run_events_enabled = True
    bus._drain_exploration_run_events()

    assert len(observed) == 1
    assert observed[0].exploration_run_id == "01K1M9S4FX27T8XMY6QJNBAV3W"
    assert observed[0].state_name == "ADMITTED"
    assert observed[0].terminal is False
    assert bus.health()["exploration_run_events"] == {
        "required": True,
        "enabled": True,
        "received": 1,
        "cursors": {"explore-boot": 1},
        "discarded": 1,
        "error": "",
    }


def test_host_bus_requires_exploration_run_event_capability(monkeypatch) -> None:
    class Session:
        capabilities = 0

        def ensure_host_state_abi(self):
            return None

        def ensure_goal_status_abi(self):
            return None

        def ensure_path_telemetry_abi(self):
            return None

    monkeypatch.setattr(host_bus_module, "get_native_command_session", lambda **_: Session())

    bus = HostBus(require_exploration_run_events=True)
    with pytest.raises(RuntimeError, match="exploration run event ABI is unavailable"):
        bus.setup()
...
def test_host_bus_projects_native_traversability_as_latest_control_risk() -> None:
    bus = HostBus()
    observed: list[dict[str, object]] = []
    bus.traversability._add_callback(observed.append)
    bus._traversability_enabled = True

    class Session:
        def take_traversability_grid(self):
            return {
                "timestamp_s": time.time(),
                "frame_id": "map",
                "receive_sequence": 4,
                ...
                "yaw": 0.0,
                "cells_u8": bytes((0, 20, 80, 100)),
            }

    bus._session = Session()
    bus._poll_traversability()

    assert len(observed) == 1
    payload = observed[0]
    assert payload["source"] == "native_nav_client"
    assert payload["control_authority"] is True
    assert payload["value_semantics"] == "control_risk_0_100"
    assert payload["grid"].shape == (2, 2)
    assert payload["grid"].tolist() == [[0, 20], [80, 100]]
