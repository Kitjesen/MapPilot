...
from .loader import RuntimeGraph, load_runtime_graph, product_requirements, resolve_product_variant_spec
...
def render_env_mermaid(
    ...
    *,
    ...
    if not isinstance(endpoint, Mapping):
        raise ValueError(f"Runtime Graph env {env_name!r} implementation must declare endpoints.contract")
    lines = ["flowchart LR"]
...
def render_product_markdown(product_name: str, graph: RuntimeGraph | None = None) -> str:
    ...
    graph = graph or load_runtime_graph()
    product = resolve_product_variant_spec(product_name, graph.products[product_name])
    topics, capabilities = product_requirements(product_name, product)
    lines = [
        f"# {product_name}",
        "",
...
def _topic_node(topic: str) -> str:
    return "topic_" + topic.strip("/").replace("/", "_").replace("-", "_").replace(".", "_")
