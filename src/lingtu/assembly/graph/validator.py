...
PRODUCT_SCHEMA_VERSION = "lingtu.runtime_graph.product.v3"
...
def _validate_product(
        ...
        if unknown_host_fields:
            issues.append(
                _issue(
                    ...
                    f"Product {name} has unknown host fields: {', '.join(unknown_host_fields)}",
                    scope=f"product:{name}",
                )
            )
        capabilities = host.get("capabilities")
        if not isinstance(capabilities, list) or any(
            not isinstance(value, str) or not value.strip() for value in capabilities or ()
        ):
            issues.append(
                _issue(
    ...
    try:
        required, capabilities = product_requirements(name, product)
    except ValueError as exc:
        issues.append(
            _issue(
                "product_runtime_invalid",
                str(exc),
                scope=f"product:{name}",
            )
        )
...
def _validate_real_product_topic_closure(
    ...
    for name, variants in sorted(resolved_products.items()):
        for variant, product in variants:
            try:
                required_topics = product_requirements(name, product)[0]
            except ValueError:
                # _validate_product reports the malformed Product runtime once.
                continue
...
def _validate_product_semantics(
    ...
    if selectable_local_planners is not None:
        ...
        if (
            ...
            or any(
                not isinstance(planner, str) or planner not in LOCAL_PLANNER_BACKENDS
    ...
    if local_planner:
        ...
        uses_traversability = "local_planner_collision_and_traversability_scoring" in capabilities
        if uses_traversability and native_nav.get("use_traversability_cost", control_mode != "teleop") is not True:
            issues.append(
                _issue(
        ...
        for product_name in sorted(set(supported) & set(products)):
            for variant, product in resolved_products.get(product_name, ()):
                ...
                if allowed_variants is not None and variant not in allowed_variants:
                    continue
                try:
                    required_topics = set(product_requirements(product_name, product)[0])
                except ValueError:
                    # Product-level validation reports the malformed runtime.
                    continue
        ...
        for variant, variant_target in acceptance_targets:
            ...
            if variant is not None and isinstance(product_spec, Mapping):
                try:
                    ...
                    selected_spec = product_spec
            native_nav = selected_spec.get("native_nav") if isinstance(selected_spec, Mapping) else None
            selectable = native_nav.get("local_planners") if isinstance(native_nav, Mapping) else None
            declared_local_planners = (
                {str(planner) for planner in selectable} if isinstance(selectable, (list, tuple)) else set()
            )
            ...
            if isinstance(variant_target, dict) and set(variant_target) == {"local_planners"}:
                ...
                if any(not isinstance(name, str) or not name.strip() or name != name.strip() for name in planner_names):
                    issues.append(
                        _issue(
                            "env_acceptance_target_invalid",
            ...
            for local_planner, leaf_target in leaf_targets:
                leaf_label = (
                    target_label if local_planner is None else f"{target_label} local planner {local_planner!r}"
                )
                if not isinstance(leaf_target, dict) or set(leaf_target) != {
                    ...
                    continue
                if not any(not str(leaf_target.get(field) or "").strip() for field in ("runner", "manifest")):
                    continue
                issues.append(
                    _issue(
                        "env_acceptance_path_invalid",
...
def _validate_native_contract_parity(graph: RuntimeGraph) -> list[RuntimeGraphIssue]:
    ...
    for env_name, endpoint in endpoints:
        missing_topics = sorted(native_topics - _endpoint_topics(endpoint))
        if missing_topics:
            issues.append(
                _issue(
