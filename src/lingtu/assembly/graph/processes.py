...
def _safe_relative_path(
    ...
    *,
    ...
    if not isinstance(value, str) or not value or "\\" in value or "{run_plan_path}" in value:
...
class ProcessCommand:
    ...
    def __post_init__(self) -> None:
        ...
        if any(item == "--run-plan" or item.startswith("--run-plan=") or "{run_plan_path}" in item for item in argv):
        ...
        for variable in ("LINGTU_NAV_CLIENT_LIB", "LINGTU_SLAM_CONTROL"):
            ...
            if dependency_path is not None and dependency_path not in dependency_paths:
                raise ValueError(f"{variable} must be an authenticated direct command dependency")
...
def resolve_stop_before_start(
    ...
    *,
    ...
    if any(not isinstance(target, str) or target != target.strip() for target in ordered) or len(set(ordered)) != len(
        ordered
    ):
    ...
    declared_mode_targets = {
        ...
        for value in (
            implementation.get("processes", {}).values() if isinstance(implementation.get("processes"), Mapping) else ()
        )
...
def _parse_process_definition(
    ...
    *,
    ...
    if manager == "systemd":
        expected_fields = frozenset({*expected_fields, *(_SYSTEMD_PROCESS_OPTIONAL_FIELDS.intersection(value))})
