"""Resolve native runtime parameters with explicit value provenance."""
...
class ResolvedParameter:
    """Final value plus the layer that supplied it."""
...
class RuntimeParameterSet:
    """Validated native parameters and their resolution evidence."""

    values: Mapping[str, ResolvedParameter]
    parameter_profile: str | None
    map_publish_hz: float | None = None
    robot_max_speed_mps: float | None = None
    ...
    def explanation(self) -> dict[str, Any]:
        max_age = float(self.values["map_input.max_age_s"].value)
        diagnostics: dict[str, float] = {}
        if self.map_publish_hz is not None:
            diagnostics["map_publish_period_s"] = 1.0 / self.map_publish_hz
            diagnostics["map_age_budget_frames"] = max_age * self.map_publish_hz
        if self.robot_max_speed_mps is not None:
            diagnostics["travel_during_map_age_m"] = (
                max_age * self.robot_max_speed_mps
            )
        return {
...
def resolve_runtime_parameters(
    *,
    ...
    _apply_parameter_layer(resolved, env_overrides, source="env_override")
    if selected_profile is not None:
        _apply_parameter_layer(
            resolved,
            profile_catalog[selected_profile],
            source=f"parameter_profile:{selected_profile}",
        )
    _apply_parameter_layer(resolved, session_overrides, source="session_override")
    ...
    max_speed = _nonnegative_optional(robot_max_speed_mps, "robot_max_speed_mps")
    ...
    return RuntimeParameterSet(
        values=resolved,
        parameter_profile=selected_profile,
        map_publish_hz=publish_hz,
        robot_max_speed_mps=max_speed,
    )
