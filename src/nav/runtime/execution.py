...
class NavigationExecutionMixin:
    def _start_external_strategy_path(self, goals: list[np.ndarray]) -> None:
        ...
        self._active_external_strategy_path = [np.asarray(point[:3], dtype=float).copy() for point in path]
    ...
    def _reanchor_external_strategy_path(
        ...
        distances = [float(np.linalg.norm(np.asarray(point[:2], dtype=float) - robot[:2])) for point in path]
        ...
        if nearest_distance > max(0.0, self._external_strategy_start_tolerance_m):
            raise RuntimeError(f"path_not_anchored_near_current_odom (nearest={nearest_distance:.2f}m)")
            ...
            self.adapter_status.publish(
                {
                    ...
                    "current_waypoint_distance_m": proximity_status.get("current_waypoint_distance_m"),
    ...
    def _normalise_goal_proximity_completion_threshold(
        ...
        *,
        ...
        if value is None:
            value = final_waypoint_threshold if final_waypoint_threshold is not None else waypoint_threshold
        try:
            parsed = float(value)
    ...
    def _goal_proximity_path_complete_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_waypoint_distance_m": (round(current_wp_distance, 3) if current_wp_distance is not None else None),
        }
    ...
    def _handle_path_complete(self) -> None:
            ...
            self.adapter_status.publish(
                {
                    ...
                    "goal": point_summary(self._goal),
                    "path_terminal_goal": point_summary(self._active_path_terminal_goal),
        ...
        if self._accept_partial_goal_progress and partial_status.get("partial"):
