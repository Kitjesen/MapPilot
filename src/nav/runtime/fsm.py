...
from nav.model.status import MissionStatus, build_mission_status
...
class NavigationFsmMixin:
    ...
    def force_state(self, state: MissionStateInput, *, reason: str) -> bool:
        ...
        phase_reason = f"force_state: {reason.strip()}" if isinstance(reason, str) and reason.strip() else "force_state"
        with self._nav_lock:
            self._state = requested_state
            self._phase_reason = phase_reason
            self._last_rejected_transition = None
    ...
    def _reject_state_transition(
        ...
        *,
        ...
        logger.warning(
            "Navigation: rejected illegal state transition %s -> %s (allowed: %s, reason=%s)",
