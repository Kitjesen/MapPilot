...
void testProbeAttributesEachFusedCostSource() {
  ...
  const lingtu::nav::endpoint::SafetyGridProbeLayers layers{
      &fused, &observed, &occupancy, &height, &surface,
  };
  const auto probe = lingtu::nav::endpoint::buildStraightForwardSafetyGridProbe(
      {0.0, 0.0, 0.0}, layers, 1.2, 0.2, 11, 7, 12.5, 12.4);
  ...
  require(probe.samples[2].unknown_before_overlays && probe.samples[2].fused_cost == 100.0F,
          "unknown fused hard cost must be visible");
  require(probe.samples[3].observed_before_overlays && probe.samples[3].occupancy_cost == 100.0F,
          "occupancy hard cost must be visible independently");
  require(probe.samples[4].height_risk_cost == 40.0F && probe.samples[4].surface_risk_cost == 0.0F,
