...
class BridgePeer {
 public:
  explicit BridgePeer(int domain_id) {
    ...
    request_writer_ = makeWriter(lingtu::message::kNavInspectionEvidenceRequest.dds_topic.data(),
                                 &lingtu_dds_InspectionEvidenceRequest_desc);
    result_reader_ = makeReader(lingtu::message::kNavInspectionEvidenceResult.dds_topic.data(),
  ...
  bool takeResult(ResultSnapshot *output) {
    ...
    if (valid) {
      const auto *wire = static_cast<const lingtu_dds_InspectionEvidenceResult *>(samples[0]);
      ...
      output->analysis_verdict = wire->analysis_verdict == nullptr ? "" : wire->analysis_verdict;
    }
 ...
 private:
  dds_entity_t makeReader(const char *topic_name, const dds_topic_descriptor_t *descriptor) {
    ...
    return checked(dds_create_reader(participant_, topic, qos.get(), nullptr), "dds_create_reader");
  }
  ...
  dds_entity_t makeWriter(const char *topic_name, const dds_topic_descriptor_t *descriptor) {
    ...
    auto qos = lingtu::dds::make_qos(lingtu::dds::qos_for_topic(topic_name));
    return checked(dds_create_writer(participant_, topic, qos.get(), nullptr), "dds_create_writer");
  }
...
void testBridgeRoundTrip() {
  ...
  require(lingtu_inspection_evidence_bridge_take_request(bridge, &request) == 0,
          "take_request must be non-blocking when no sample exists");

  peer.writeRequest("request-a");
  waitUntil([&]() { return lingtu_inspection_evidence_bridge_take_request(bridge, &request) == 1; },
...
void testBridgeRejectsTruncatedResultFields() {
  ...
  require(lingtu_inspection_evidence_bridge_write_result(bridge, &result) == -1,
