...
lingtu::nav::endpoint::MotionLayerConfig layerConfig(
    double voxel_size_m, double decay_s, double inflation_radius_m,
    double ray_clear_max_range_m, double ray_clearing_interval_s,
    std::size_t max_clearing_rays, int min_hits, bool ray_clearing_enabled) {
  lingtu::nav::endpoint::MotionLayerConfig config;
...
void testMinHits() {
  lingtu::nav::endpoint::MotionLayer layer(
      layerConfig(0.10, 1.0, 0.0, 4.0, 0.0, 100, 2, false));
...
void testInflation() {
  lingtu::nav::endpoint::MotionLayer layer(
      layerConfig(0.10, 1.0, 0.20, 4.0, 0.0, 100, 1, false));
...
void testRayClearing() {
  lingtu::nav::endpoint::MotionLayer layer(
      layerConfig(0.10, 10.0, 0.0, 4.0, 0.0, 100, 1, true));
...
int main() {
  ...
  testInflation();
  testRayClearing();
  std::cout << "test_motion_layer_basics passed\n";
  return 0;
}
