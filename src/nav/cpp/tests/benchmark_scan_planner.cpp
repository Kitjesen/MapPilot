...
std::vector<float> obstacleCloud(std::size_t count) {
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    out.push_back(static_cast<float>(-0.1 + 0.8 * unit()));
    out.push_back(0.8F);
  }
  ...
  for (int yi = -7; yi <= 7; ++yi) {
    for (int zi = -2; zi <= 6; ++zi) {
      out.push_back(1.5F);
      out.push_back(static_cast<float>(yi) * 0.10F);
      out.push_back(static_cast<float>(zi) * 0.10F);
      out.push_back(0.8F);
    }
  }
  return out;
...
int main(int argc, char** argv) {
  ...
  if (!planner.configure()) {
    std::cerr << "scan planner configure failed\n";
    return 2;
  }
  ...
  for (int iteration = -5; iteration < iterations; ++iteration) {
    ...
    input.route = {route.data(), static_cast<int>(route.size()),
                   static_cast<std::uint64_t>(iteration + 6), false};
    input.identity = {1, static_cast<std::uint64_t>(iteration + 6), 0};
    input.timestampS = 1.0 + 0.05 * static_cast<double>(iteration + 5);
    input.collision.occupiedXyz = collision_xyz.data();
    input.collision.occupiedCount =
        static_cast<int>(collision_xyz.size() / 3U);
    input.collision.resolution = params.scan.voxelResolution;
    input.collision.aabbMin = {-5.0, -5.0, -2.0};
    input.collision.aabbMax = {5.0, 5.0, 2.0};
    input.collision.resetEpoch = 1;
    input.collision.observationSequence =
        static_cast<std::uint64_t>(iteration + 6);
    input.collision.generation =
        static_cast<std::uint64_t>(iteration + 6);
    input.collision.stampS = input.timestampS;
    input.collision.receiveStampS = input.timestampS;
    input.collision.complete = true;
    input.collision.live = true;
  ...
  std::cout << std::fixed << std::setprecision(3)
            ...
            << ",\"budget_20hz_ms\":50.0"
            << ",\"meets_20hz\":"
            << (percentile(elapsed_ms, 0.95) <= 50.0 ? "true" : "false")
            << "}\n";
  return successes == iterations && astar_runs == iterations ? 0 : 1;
}
