...
void testFarPlannerIsAnExplicitOptionalBackend() {
  ...
  require(
      defaults.global_planner == GlobalPlannerBackend::OctoPlanner3D,
  ...
  const auto far = parse({
      ...
      "--global-planner", "far",
      "--map-root", "/tmp/lingtu/maps",
      "--map", "/tmp/lingtu/maps/active/occupancy.npz",
      "--far-robot-radius-m", "0.28",
      "--far-obstacle-clearance-m", "0.12",
      "--far-max-visibility-distance-m", "24.0",
      "--far-unknown-cost-multiplier", "8.0",
      "--far-corner-separation-cells", "2",
      "--far-snap-radius-cells", "9",
      "--far-max-graph-nodes", "2048",
      "--far-max-visibility-pairs", "120000",
      "--far-max-search-expansions", "180000",
      "--far-allow-unknown-fallback", "true",
      "--far-simplify-path", "false",
  });
  require(far.global_planner == GlobalPlannerBackend::Far, "FAR backend must parse");
  require(
      far.map_path == "/tmp/lingtu/maps/active/occupancy.npz",
  ...
  require(
      std::abs(far.far_options.obstacle_clearance_m - 0.12) < 1e-12,
      "FAR obstacle clearance must parse");
  require(
      std::abs(far.far_options.max_visibility_distance_m - 24.0) < 1e-12,
