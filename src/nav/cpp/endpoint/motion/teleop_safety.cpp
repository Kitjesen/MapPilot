...
constexpr double kGridOverlapEpsilon = 1e-9;
...
nav_kernel::Vec3 mapPointToBody(const nav_kernel::Pose &map_body, const nav_kernel::Vec3 &p_map) {
...
nav_kernel::Twist limitedTeleopCommand(const CommandSafetyConfig &cfg, const nav_kernel::Twist &raw,
                                       bool &limited) {
...
std::vector<SweptPose2D> sweptPoses(const CommandSafetyConfig &cfg, const nav_kernel::Twist &cmd) {
  ...
  if (speed >= cfg.min_motion_speed_mps) {
    ...
    const int sample_count =
        std::max(1, std::max(static_cast<int>(std::ceil(travel_m / kSweepLinearStepM)),
                             static_cast<int>(std::ceil(yaw_span / kSweepAngularStepRad))));
    poses.reserve(static_cast<std::size_t>(sample_count + 1));
    for (int i = 0; i <= sample_count; ++i) {
      const double t = duration_s * static_cast<double>(i) / static_cast<double>(sample_count);
      SweptPose2D pose;
      pose.yaw = cmd.wz * t;
  ...
  const int sample_count =
      std::max(1, static_cast<int>(std::ceil(yaw_span / kSweepAngularStepRad)));
  poses.reserve(static_cast<std::size_t>(sample_count + 1));
  for (int i = 0; i <= sample_count; ++i) {
...
std::vector<SweptPose2D> sweptPathPoses(const CommandSafetyConfig &cfg,
  ...
  for (const auto &point_map : local_path_map) {
    ...
    if (std::hypot(point_body.x - body_path.back().x, point_body.y - body_path.back().y) > 1e-4) {
  ...
  for (std::size_t i = 1; i < body_path.size() && cumulative_m < horizon_m; ++i) {
    ...
    while (next_sample_m <= segment_end_m + 1e-6) {
      const double ratio = std::clamp((next_sample_m - cumulative_m) / segment_m, 0.0, 1.0);
      poses.push_back({
          start.x + ratio * dx,
          start.y + ratio * dy,
          yaw,
    ...
    const double final_ratio = std::clamp((segment_end_m - cumulative_m) / segment_m, 0.0, 1.0);
...
bool pointInsideFootprint(const SweptPose2D &pose, const nav_kernel::Vec3 &point,
                          double half_length, double half_width, double &local_x, double &local_y) {
...
enum class TraversabilityProbeStatus {
  kOk,
  kUnavailable,
  kOutOfBounds,
  kInvalid,
...
struct TraversabilityProbeResult {
  TraversabilityProbeStatus status{TraversabilityProbeStatus::kOk};
  double max_cost{-1.0};
};

const char *traversabilityFailureReason(TraversabilityProbeStatus status) {
  switch (status) {
    case TraversabilityProbeStatus::kUnavailable:
      return "terrain_unavailable";
    case TraversabilityProbeStatus::kOutOfBounds:
      return "terrain_out_of_bounds";
    case TraversabilityProbeStatus::kInvalid:
      return "terrain_invalid";
    case TraversabilityProbeStatus::kBlocked:
      return "terrain_stop";
    case TraversabilityProbeStatus::kOk:
      break;
  }
  return "terrain_invalid";
}

TraversabilityProbeStatus validateTraversabilityGrid(const TraversabilityGrid &grid) {
  if (grid.values.empty()) {
    return TraversabilityProbeStatus::kUnavailable;
  }
  if (grid.rows <= 0 || grid.cols <= 0 || !std::isfinite(grid.resolution) ||
      grid.resolution <= 0.0 || !std::isfinite(grid.origin_x) || !std::isfinite(grid.origin_y)) {
    return TraversabilityProbeStatus::kInvalid;
  }
  const auto rows = static_cast<std::size_t>(grid.rows);
  const auto cols = static_cast<std::size_t>(grid.cols);
  if (cols > std::numeric_limits<std::size_t>::max() / rows || grid.values.size() != rows * cols) {
    return TraversabilityProbeStatus::kInvalid;
  }
  const double grid_max_x = grid.origin_x + static_cast<double>(grid.cols) * grid.resolution;
  const double grid_max_y = grid.origin_y + static_cast<double>(grid.rows) * grid.resolution;
  if (!std::isfinite(grid_max_x) || !std::isfinite(grid_max_y)) {
    return TraversabilityProbeStatus::kInvalid;
  }
  return TraversabilityProbeStatus::kOk;
}

TraversabilityProbeStatus traversabilityCostAt(const TraversabilityGrid &grid, double map_x,
                                               double map_y, double &cost) {
  if (!std::isfinite(map_x) || !std::isfinite(map_y)) {
    return TraversabilityProbeStatus::kInvalid;
  }
  const double grid_max_x = grid.origin_x + static_cast<double>(grid.cols) * grid.resolution;
  const double grid_max_y = grid.origin_y + static_cast<double>(grid.rows) * grid.resolution;
  if (map_x < grid.origin_x || map_y < grid.origin_y || map_x >= grid_max_x ||
      map_y >= grid_max_y) {
    return TraversabilityProbeStatus::kOutOfBounds;
  }
  ...
  if (!std::isfinite(cost) || cost < 0.0 || cost > 100.0) {
    return TraversabilityProbeStatus::kInvalid;
  }
  return TraversabilityProbeStatus::kOk;
}

bool footprintIntersectsGridCell(double center_x, double center_y, double c, double s,
                                 double half_length, double half_width, double cell_x,
                                 double cell_y, double cell_half) {
  const double dx = cell_x - center_x;
  const double dy = cell_y - center_y;
  const double abs_c = std::abs(c);
  const double abs_s = std::abs(s);
  if (std::abs(dx) >= cell_half + half_length * abs_c + half_width * abs_s - kGridOverlapEpsilon) {
    return false;
  }
  if (std::abs(dy) >= cell_half + half_length * abs_s + half_width * abs_c - kGridOverlapEpsilon) {
    return false;
  }
  if (std::abs(dx * c + dy * s) >=
      half_length + cell_half * (abs_c + abs_s) - kGridOverlapEpsilon) {
    return false;
  }
  if (std::abs(-dx * s + dy * c) >=
      half_width + cell_half * (abs_s + abs_c) - kGridOverlapEpsilon) {
    return false;
  }
  return true;
}

TraversabilityProbeResult probeYawSweepTraversability(const CommandSafetyConfig &cfg,
                                                      const nav_kernel::Pose &map_body,
                                                      const std::vector<SweptPose2D> &sweep,
                                                      const TraversabilityGrid &grid) {
  TraversabilityProbeResult result;
  result.status = validateTraversabilityGrid(grid);
  if (result.status != TraversabilityProbeStatus::kOk) {
    return result;
  }
  if (sweep.empty() || !std::isfinite(map_body.position.x) || !std::isfinite(map_body.position.y) ||
      !std::isfinite(map_body.yaw) || !std::isfinite(cfg.vehicle_length_m) ||
      cfg.vehicle_length_m <= 0.0 || !std::isfinite(cfg.vehicle_width_m) ||
      cfg.vehicle_width_m <= 0.0 || !std::isfinite(cfg.obstacle_margin_m) ||
      cfg.obstacle_margin_m < 0.0 || !std::isfinite(cfg.traversability_hard_cost) ||
      cfg.traversability_hard_cost < 0.0 || cfg.traversability_hard_cost > 100.0) {
    result.status = TraversabilityProbeStatus::kInvalid;
    return result;
  }
  const double half_length = std::max(0.1, cfg.vehicle_length_m * 0.5 + cfg.obstacle_margin_m);
  const double half_width = std::max(0.1, cfg.vehicle_width_m * 0.5 + cfg.obstacle_margin_m);
  const double base_c = std::cos(map_body.yaw);
  const double base_s = std::sin(map_body.yaw);
  const double cell_half = grid.resolution * 0.5;
  const double grid_max_x = grid.origin_x + static_cast<double>(grid.cols) * grid.resolution;
  const double grid_max_y = grid.origin_y + static_cast<double>(grid.rows) * grid.resolution;
  bool sampled = false;
  for (const auto &pose : sweep) {
    if (!std::isfinite(pose.x) || !std::isfinite(pose.y) || !std::isfinite(pose.yaw)) {
      result.status = TraversabilityProbeStatus::kInvalid;
      return result;
    }
    const double center_x = map_body.position.x + base_c * pose.x - base_s * pose.y;
    const double center_y = map_body.position.y + base_s * pose.x + base_c * pose.y;
    const double yaw = map_body.yaw + pose.yaw;
    ...
    if (min_x < grid.origin_x - kGridOverlapEpsilon ||
        min_y < grid.origin_y - kGridOverlapEpsilon || max_x > grid_max_x + kGridOverlapEpsilon ||
        max_y > grid_max_y + kGridOverlapEpsilon) {
      result.status = TraversabilityProbeStatus::kOutOfBounds;
      return result;
    }
    const int min_col =
        std::max(0, static_cast<int>(std::floor((min_x - grid.origin_x) / grid.resolution)));
    const int max_col = std::min(
        grid.cols - 1, static_cast<int>(std::floor((max_x - grid.origin_x - kGridOverlapEpsilon) /
                                                   grid.resolution)));
    const int min_row =
        std::max(0, static_cast<int>(std::floor((min_y - grid.origin_y) / grid.resolution)));
    const int max_row = std::min(
        grid.rows - 1, static_cast<int>(std::floor((max_y - grid.origin_y - kGridOverlapEpsilon) /
                                                   grid.resolution)));
    for (int row = min_row; row <= max_row; ++row) {
      ...
      for (int col = min_col; col <= max_col; ++col) {
        ...
        if (!footprintIntersectsGridCell(center_x, center_y, c, s, half_length, half_width, cell_x,
                                         cell_y, cell_half)) {
          continue;
        }
        const std::size_t index =
            ...
            static_cast<std::size_t>(col);
        const double cost = static_cast<double>(grid.values[index]);
        if (!std::isfinite(cost) || cost < 0.0 || cost > 100.0) {
          result.status = TraversabilityProbeStatus::kInvalid;
          return result;
        }
        sampled = true;
        result.max_cost = std::max(result.max_cost, cost);
        if (cost >= cfg.traversability_hard_cost) {
          result.status = TraversabilityProbeStatus::kBlocked;
          return result;
        }
      }
    }
...
evaluateSafetyWithSweep(const CommandSafetyConfig &cfg, const nav_kernel::Twist &raw_request,
  ...
  if (cfg.check_obstacle) {
    ...
    for (std::size_t i = 0; i + 3 < obstacle_xyzh.size(); i += 4) {
      ...
      const auto p_body = mapPointToBody(
          *map_body, {obstacle_xyzh[i + 0], obstacle_xyzh[i + 1], obstacle_xyzh[i + 2]});
      for (const auto &swept_pose : sweep) {
        double local_x = 0.0;
        double local_y = 0.0;
        if (!pointInsideFootprint(swept_pose, p_body, half_length, half_width, local_x, local_y)) {
          continue;
        }
        if (translation_motion) {
          const double local_ahead = path_sweep ? local_x : local_x * dir_x + local_y * dir_y;
  ...
  if (cfg.use_traversability_cost && traversability_fresh && speed >= cfg.min_motion_speed_mps) {
    const auto grid_status = validateTraversabilityGrid(traversability_grid);
    if (grid_status != TraversabilityProbeStatus::kOk) {
      decision.cmd = {};
      decision.stopped = true;
      decision.reason = traversabilityFailureReason(grid_status);
      return decision;
    }
    const double c = std::cos(map_body->yaw);
    const double s = std::sin(map_body->yaw);
    ...
    for (const auto &swept_pose : sweep) {
      ...
      double cost = -1.0;
      const auto sample_status = traversabilityCostAt(traversability_grid, mx, my, cost);
      if (sample_status != TraversabilityProbeStatus::kOk) {
        decision.cmd = {};
        decision.stopped = true;
        decision.reason = traversabilityFailureReason(sample_status);
        return decision;
      }
      max_cost = std::max(max_cost, cost);
      if (cost >= cfg.traversability_hard_cost) {
        nearest_hard_distance = std::min(nearest_hard_distance, swept_pose.travel_m);
      }
  ...
  if (cfg.use_traversability_cost && traversability_fresh && speed < cfg.min_motion_speed_mps &&
      std::abs(decision.cmd.wz) >= 1e-4) {
    const auto probe = probeYawSweepTraversability(cfg, *map_body, sweep, traversability_grid);
    decision.traversability_cost = probe.max_cost;
    if (probe.status != TraversabilityProbeStatus::kOk) {
      decision.cmd = {};
      decision.stopped = true;
      decision.reason = traversabilityFailureReason(probe.status);
      return decision;
    }
  }
...
evaluateCommandSafety(const CommandSafetyConfig &cfg, const nav_kernel::Twist &raw_request,
                      ...
                      const std::vector<float> &obstacle_xyzh,
                      const TraversabilityGrid &traversability_grid, bool traversability_fresh) {
  return evaluateSafetyWithSweep(cfg, raw_request, request_age_s, map_body, obstacle_xyzh,
                                 traversability_grid, traversability_fresh, nullptr);
}
...
CommandSafetyDecision evaluateAutonomyPathSafety(
  ...
  if (!map_body || linearSpeed(raw_request) < cfg.min_motion_speed_mps) {
    return evaluateCommandSafety(cfg, raw_request, 0.0, map_body, obstacle_xyzh,
                                 traversability_grid, traversability_fresh);
  }
  const auto path_sweep = sweptPathPoses(cfg, *map_body, local_path_map);
  return evaluateSafetyWithSweep(cfg, raw_request, 0.0, map_body, obstacle_xyzh,
                                 traversability_grid, traversability_fresh, &path_sweep);
}
