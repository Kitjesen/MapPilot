...
CommandIngressResult CommandIngressController::handle(const CommandIngressRequest &request,
  ...
  if (request.request_id.empty()) {
    return finish({}, kind, {false, "command_request_id_empty"}, false, false);
  }
  if (request.client_id.empty()) {
    return finish(request.request_id, kind, {false, "command_client_id_empty"}, false, false);
  }
  if (!lingtu::message::isKnownNavigationCommandKind(request.raw_kind)) {
    return finish(request.request_id, kind, {false, "unknown_command_kind"}, false, false);
  }
  ...
  if (cached.disposition == JournalDisposition::Conflict) {
    return finish(request.request_id, kind, {false, "idempotency_conflict"}, false, false);
  }
  if (cached.disposition == JournalDisposition::Replay && cached.ack) {
    ++diagnostics_.replayed;
    return finish(request.request_id, kind, *cached.ack, true, false);
  }

  CommandAck ack = dispatch(kind, request.payload);
  journal.remember(identity, ack, timestamp);
  return finish(request.request_id, kind, std::move(ack), false, true);
}

CommandIngressResult CommandIngressController::finish(std::string request_id, CommandKind kind,
                                                      CommandAck ack, bool replayed,
                                                      bool dispatched) {
  if (!ack.accepted) {
  ...
  return {
      std::move(request_id), kind, std::move(ack), replayed, dispatched,
  };
}
