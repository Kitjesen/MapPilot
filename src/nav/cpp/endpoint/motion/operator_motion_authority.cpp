...
OperatorMotionReceipt OperatorMotionAuthority::claim(const OperatorMotionClaim &claim) {
  ...
  if (source_epoch_high_water_.find(claim.source_id) == source_epoch_high_water_.end() &&
      source_epoch_high_water_.size() >= options_.max_retained_source_identities) {
...
OperatorMotionSnapshot OperatorMotionAuthority::snapshot() const {
  return snapshot_;
}
...
std::uint64_t OperatorMotionAuthority::sourceHighWater(const std::string &source_id) const {
  ...
  return it == source_epoch_high_water_.end() ? 0 : it->second;
}
...
bool OperatorMotionAuthority::recordSourceHighWater(const std::string &source_id,
  ...
  if (it == source_epoch_high_water_.end()) {
    ...
    it = source_epoch_high_water_.emplace(source_id, 0).first;
  }
  if (epoch > it->second) {
    it->second = epoch;
  }
