...
[[nodiscard]] constexpr DdsDrainBudget drainBudget(
    DdsDrainProfile profile) noexcept {
  return profile == DdsDrainProfile::kTransform
      ? DdsDrainBudget{kDdsReaderBatchSize, 8}
      : DdsDrainBudget{kDdsReaderBatchSize, 1};
}

template <typename TakeBatch>
std::size_t drainBatches(
    const DdsDrainBudget& budget,
    TakeBatch&& take_batch) {
