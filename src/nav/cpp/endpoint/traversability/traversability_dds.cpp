...
void inflateGridCost(map_layers::Grid2D &grid, int source_row, int source_col, float cost,
                     double radius_m) {
  const int inflate = nav_endpoint::inflationSearchRadiusCells(radius_m, grid.resolution);
  for (int dr = -inflate; dr <= inflate; ++dr) {
    for (int dc = -inflate; dc <= inflate; ++dc) {
      ...
      if (!nav_endpoint::cellCenterWithinInflationRadius(dr, dc, grid.resolution, radius_m)) {
        continue;
      }
...
map_layers::Grid2D buildOccupancyGrid(const std::vector<float> &xyz,
  ...
  diagnostics.observed_free_applied_cells =
      observed_free.apply(grid, stamp_s, cfg.observed_free_ttl_s);
  diagnostics.observed_free_cache_cells = observed_free.size();
...
map_layers::Grid2D buildTerrainRiskGrid(const std::vector<float> &terrain_xyzi,
  ...
  map_layers::Grid2D inflated =
      map_layers::makeGrid2D(size, size, cfg.resolution, raw.originX, raw.originY, 0.0F);
...
void mergeGridMax(map_layers::Grid2D &destination, const map_layers::Grid2D &source) {
  ...
  for (int row = 0; row < source.rows; ++row) {
    for (int col = 0; col < source.cols; ++col) {
      ...
      float &destination_cost =
          destination
              .data[static_cast<std::size_t>(destination.index(destination_row, destination_col))];
      destination_cost = std::max(destination_cost, cost);
    }
  }
}

map_layers::Grid2D inflateRiskGrid(const map_layers::Grid2D &source, double radius_m) {
  map_layers::Grid2D inflated = map_layers::makeGrid2D(source.rows, source.cols, source.resolution,
                                                       source.originX, source.originY, 0.0F);
  for (int row = 0; row < source.rows; ++row) {
    for (int col = 0; col < source.cols; ++col) {
      const float cost = source.data[static_cast<std::size_t>(source.index(row, col))];
      if (cost > 0.0F) {
...
map_layers::Grid2D buildSurfaceTerrainRiskGrid(const std::vector<float> &terrain_xyzi,
  ...
  const map_layers::ElevationMapResult elevation = map_layers::buildElevationMap(
      xyz, origin_x + cfg.radius, origin_y + cfg.radius, cfg.resolution, cfg.radius,
      pose.z + cfg.z_min, pose.z + cfg.z_max);
...
void writeStatus(const CliConfig &cfg, std::uint64_t odom_count, std::uint64_t tf_count,
                 ...
                 std::size_t last_points, std::size_t terrain_points,
  ...
  out << "{\n"
      ...
      << "  \"radius\": " << cfg.radius << ",\n"
      << "  \"exploration_map\": {\"source\": \"maps.rolling_occupancy\", "
    ...
    while (g_running) {
      ...
      dds.drainCloud([&](const lingtu_dds_PointCloud2 &msg) {
        ...
        try {
          ...
          MapCloudResult cloud =
              cloudToMapXyz(msg, cloud_map_body, cloud_map_odom, cfg.terrain_cache_max_points);
          ...
          if (!xyz.empty()) {
            ...
            if (schedule_now + slow_tolerance_s >= next_slow_publish) {
              ...
              terrain_core.updateVehicle(pose.x, pose.y, pose.z, pose.roll, pose.pitch, pose.yaw);
              const auto terrain_core_start = SteadyClock::now();
              const nav_kernel::TerrainResult terrain_result = terrain_core.process(
                  scan_xyzi.data(), static_cast<int>(scan_xyzi.size() / 4), cloud_stamp_s);
              last_terrain_connected_cells =
                  static_cast<std::size_t>(std::max(0, terrain_result.connected_cells));
              last_terrain_ceiling_cells =
              ...
              const nav_kernel::DynamicClearResult clear_result = dynamic_clear.filter(
                  terrain_result.terrain_points, xyz, clear_origin, cloud_stamp_s);
              ...
              const std::vector<float> terrain_ext_xyzi =
                  combineTerrainExt(terrain_xyzi, clear_result.dynamic_xyzi, cfg.max_points);
              height_risk_grid = buildTerrainRiskGrid(clear_result.kept_xyzi, pose, cfg);
              surface_risk_grid = buildSurfaceTerrainRiskGrid(clear_result.kept_xyzi, pose, cfg);
              ...
              const auto [risk_cells, risk_max_cost] = terrainRiskStats(*terrain_risk_grid);
      ...
      if (schedule_now >= next_status) {
        ...
        writeStatus(cfg, odom_count, tf_count, cloud_count, publish_count, slow_publish_count,
                    ...
                    error_count, last_points, last_terrain_points, last_terrain_ext_points,
                    ...
                    last_grid_diagnostics, last_forward_probe, last_timing);
      }
