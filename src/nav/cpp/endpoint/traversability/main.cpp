...
#include "traversability/rolling_exploration_map.hpp"
#include "traversability/safety_grid_probe.hpp"
#include "traversability/terrain_risk.hpp"
...
constexpr double kNearBodyHardProbeRadiusM = 1.0;
constexpr std::size_t kNearBodyHardProbeMaxSamples = 64;
...
std::int64_t stampNanoseconds(double stamp_s) {
...
void drainReader(dds_entity_t reader, const dds_topic_descriptor_t &descriptor, Handler &&handler,
  ...
  nav_endpoint::drainBatches(budget, [&](std::size_t capacity) -> std::ptrdiff_t {
...
mapBodyTransform(const nav_endpoint::RigidTransform &pose, const std::string &pose_frame,
...
FieldOffsets fieldOffsets(const lingtu_dds_PointCloud2 &msg) {
  ...
  for (std::uint32_t i = 0; i < msg.fields._length; ++i) {
    ...
    if (!nav_endpoint::pointFieldIsScalarFloat32(field.datatype, field.count, field.offset,
                                                 msg.point_step)) {
      offsets.invalid = true;
      continue;
    }
...
MapCloudResult cloudToMapXyz(const lingtu_dds_PointCloud2 &msg,
                             const std::optional<nav_endpoint::RigidTransform> &map_body,
                             const std::optional<nav_endpoint::RigidTransform> &map_odom,
                             std::size_t max_points, double self_filter_length_m,
                             double self_filter_width_m, double self_filter_padding_m) {
  ...
  if (!nav_endpoint::pointCloudStorageIsValid(msg.width, msg.height, msg.point_step, msg.row_step,
                                              msg.data._length)) {
  ...
  if (result.input_frame == "map") {
    ...
    transform = map_odom;
  } else if (result.input_frame == "body" || result.input_frame == "base_link" ||
  ...
  std::optional<nav_endpoint::RigidTransform> body_from_map;
  if (map_body && map_body->valid && self_filter_length_m > 0.0 && self_filter_width_m > 0.0) {
    body_from_map = nav_endpoint::inverseTransform(*map_body);
  }
  ...
  for (std::size_t i = 0; i < count; i += stride) {
    ...
    nav_kernel::Vec3 map_point{x, y, z};
    if (transform) {
      map_point = nav_endpoint::transformPoint(*transform, map_point);
    }
    if (body_from_map && nav_endpoint::pointInsideRobotSelfFilterBody(
                             nav_endpoint::transformPoint(*body_from_map, map_point),
                             self_filter_length_m, self_filter_width_m,
                             self_filter_padding_m)) {
      ++result.self_filtered_points;
      continue;
    }
    result.xyz.push_back(static_cast<float>(map_point.x));
    result.xyz.push_back(static_cast<float>(map_point.y));
    result.xyz.push_back(static_cast<float>(map_point.z));
  }
...
struct CliConfig {
  ...
  // Exploration clearance only. Published control-risk grids stay cell-raw;
  // LocalPlanner, recovery, and direct-command safety apply the body footprint.
  double robot_radius{0.45};
  double self_filter_length_m{0.0};
  double self_filter_width_m{0.0};
  double self_filter_padding_m{0.03};
  double observed_free_ttl_s{0.60};
  double observed_free_inflation_radius_m{0.0};
...
std::pair<std::size_t, float> terrainRiskStats(const map_layers::Grid2D &grid);

struct SlowTerrainInput {
  std::uint64_t generation{0};
  double stamp_s{0.0};
  std::vector<float> xyz;
  nav_endpoint::TraversabilityPose pose;
  nav_kernel::Vec3 sensor_origin;
};

struct SlowTerrainResult {
  bool ok{true};
  std::string error;
  std::uint64_t generation{0};
  double stamp_s{0.0};
  TimingDiagnostics timing;
  std::vector<float> terrain_xyzi;
  std::vector<float> terrain_ext_xyzi;
  map_layers::Grid2D height_risk_grid;
  map_layers::Grid2D surface_risk_grid;
  map_layers::Grid2D terrain_risk_grid;
  std::size_t cache_points{0};
  std::size_t terrain_connected_cells{0};
  std::size_t terrain_ceiling_cells{0};
  std::size_t dynamic_clear_points{0};
  std::size_t dynamic_clear_ray_points{0};
  std::size_t dynamic_clear_evidence{0};
  std::size_t dynamic_clear_free_voxels{0};
  std::size_t terrain_risk_cells{0};
  float terrain_risk_max_cost{0.0F};
};

struct SlowTerrainWorkerState {
  explicit SlowTerrainWorkerState(const CliConfig &cfg)
      : terrain_core(terrainParamsFromConfig(cfg)),
        dynamic_clear(dynamicClearParamsFromConfig(cfg)) {}

  nav_kernel::TerrainAnalysisCore terrain_core;
  nav_kernel::DynamicClearCore dynamic_clear;
};
...
CliConfig parseArgs(int argc, char **argv) {
  ...
  cfg.exploration_enabled = cfg.product == "explore";
  cfg.status_file = envOrEmpty("LINGTU_TRAVERSABILITY_STATUS_FILE");
  if (cfg.status_file.empty()) {
    const std::string session_root = envOrEmpty("LINGTU_SESSION_ROOT");
    if (!session_root.empty()) {
      cfg.status_file =
          (std::filesystem::path(session_root) / "traversability.status.json").string();
    }
  }
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--domain-id") {
    ...
    } else if (arg == "--self-filter-length-m") {
      cfg.self_filter_length_m = std::stod(next());
    } else if (arg == "--self-filter-width-m") {
      cfg.self_filter_width_m = std::stod(next());
    } else if (arg == "--self-filter-padding-m") {
      cfg.self_filter_padding_m = std::stod(next());
    } else if (arg == "--observed-free-ttl-s") {
      cfg.observed_free_ttl_s = std::stod(next());
    } else if (arg == "--observed-free-inflation-radius-m") {
      cfg.observed_free_inflation_radius_m = std::stod(next());
      ...
      cfg.terrain_under_vehicle_m = std::stod(next());
    } else if (arg == "--terrain-connection-height-m") {
      cfg.terrain_connection_height_m = std::stod(next());
    } else if (arg == "--terrain-ceiling-height-m") {
      cfg.terrain_ceiling_height_m = std::stod(next());
    } else if (arg == "--terrain-connectivity-radius-cells") {
      cfg.terrain_connectivity_radius_cells = std::stoi(next());
    } else if (arg == "--terrain-seed-radius-cells") {
      cfg.terrain_seed_radius_cells = std::stoi(next());
    } else if (arg == "--terrain-seed-max-error-m") {
      cfg.terrain_seed_max_error_m = std::stod(next());
    ...
    } else if (arg == "--terrain-threads") {
      cfg.terrain_threads = static_cast<std::size_t>(std::stoull(next()));
      ...
      throw std::runtime_error(
          ...
          "[--self-filter-length-m M] [--self-filter-width-m M] "
          "[--self-filter-padding-m M] "
  ...
  if (cfg.self_filter_length_m <= 0.0) {
    const std::string value = envOrEmpty("LINGTU_NAV_VEHICLE_LENGTH_M");
    if (!value.empty()) {
      cfg.self_filter_length_m = std::stod(value);
    }
  }
  if (cfg.self_filter_width_m <= 0.0) {
    const std::string value = envOrEmpty("LINGTU_NAV_VEHICLE_WIDTH_M");
    if (!value.empty()) {
      cfg.self_filter_width_m = std::stod(value);
    }
  }
  ...
  cfg.self_filter_length_m = std::max(0.0, cfg.self_filter_length_m);
  cfg.self_filter_width_m = std::max(0.0, cfg.self_filter_width_m);
  cfg.self_filter_padding_m = std::max(0.0, cfg.self_filter_padding_m);
  cfg.observed_free_ttl_s = std::max(0.1, cfg.observed_free_ttl_s);
  cfg.observed_free_inflation_radius_m = std::max(0.0, cfg.observed_free_inflation_radius_m);
  cfg.terrain_soft_height_m = std::max(0.0, cfg.terrain_soft_height_m);
  cfg.terrain_hard_height_m = std::max(cfg.terrain_soft_height_m + 0.01, cfg.terrain_hard_height_m);
  ...
  cfg.terrain_connectivity_radius_cells = std::max(1, cfg.terrain_connectivity_radius_cells);
  cfg.terrain_seed_radius_cells = std::max(0, cfg.terrain_seed_radius_cells);
  cfg.terrain_seed_max_error_m = std::max(0.0, cfg.terrain_seed_max_error_m);
  ...
  cfg.terrain_cache_max_points =
      std::max<std::size_t>(cfg.max_points, cfg.terrain_cache_max_points);
  cfg.terrain_threads = std::clamp<std::size_t>(cfg.terrain_threads, 1, 4);
  if (envOrEmpty("LINGTU_ENV") == "sim" && cfg.product_session_id.empty()) {
...
nav_kernel::TerrainParams terrainParamsFromConfig(const CliConfig &cfg) {
  ...
  params.maxPointsPerVoxel =
      std::max<std::size_t>(32, cfg.terrain_cache_max_points / kMergedTerrainVoxelCount);
  ...
  params.ceilingFilteringHeight = cfg.terrain_ceiling_height_m;
  params.terrainConnectivityRadiusCells = cfg.terrain_connectivity_radius_cells;
  params.groundSeedSearchRadiusCells = cfg.terrain_seed_radius_cells;
  params.maxGroundSeedError = cfg.terrain_seed_max_error_m;
  ...
  params.workerThreads = static_cast<int>(cfg.terrain_threads);
  return params;
}
...
nav_kernel::DynamicClearParams dynamicClearParamsFromConfig(const CliConfig &cfg) {
  ...
  params.raycastClearMinFrames = static_cast<std::uint32_t>(cfg.dynamic_clear_raycast_min_frames);
...
nav_endpoint::RollingExplorationConfig rollingExplorationConfig(const CliConfig &cfg) {
  nav_endpoint::RollingExplorationConfig out;
  out.size_x = std::max(32, static_cast<int>(std::floor(2.0 * cfg.radius / cfg.resolution)));
  out.size_y = out.size_x;
  out.size_z = std::max(16, static_cast<int>(std::ceil(6.0 / cfg.resolution)));
  out.resolution_m = static_cast<float>(cfg.resolution);
  out.max_ray_range_m = static_cast<float>(2.0 * cfg.radius);
  out.obstacle_min_z_m = static_cast<float>(cfg.obstacle_min_z);
  out.obstacle_max_z_m = static_cast<float>(cfg.z_max);
  out.inflation_radius_m = static_cast<float>(cfg.robot_radius);
  return out;
}
...
std::vector<float> combineTerrainExt(const std::vector<float> &kept_xyzi,
  ...
  const std::size_t dynamic_budget = max_points == 0 ? 0 : max_points - kept_points;
...
map_layers::Grid2D buildOccupancyGrid(const std::vector<float> &xyz,
                                      const nav_endpoint::TraversabilityPose &pose,
                                      const nav_kernel::Vec3 &sensor_origin, double stamp_s,
                                      nav_endpoint::ObservedFreeCache &observed_free,
                                      const CliConfig &cfg, SafetyGridDiagnostics &diagnostics,
  ...
  const double origin_x = nav_endpoint::snappedSafetyGridOrigin(pose.x, cfg.radius, cfg.resolution);
  const double origin_y = nav_endpoint::snappedSafetyGridOrigin(pose.y, cfg.radius, cfg.resolution);
  map_layers::Grid2D grid =
      nav_endpoint::makeUnknownSafetyGrid(size, size, cfg.resolution, origin_x, origin_y, 100.0F);
  const std::size_t n = xyz.size() / 3;
  for (std::size_t i = 0; i < n; ++i) {
    ...
    observed_free.observeRay(sensor_origin.x, sensor_origin.y, x, y, stamp_s, cfg.radius * 2.0);
  }
  diagnostics = {};
  diagnostics.origin_x = grid.originX;
  diagnostics.origin_y = grid.originY;
  diagnostics.alignment_residual_x =
      nav_endpoint::safetyGridAlignmentResidual(grid.originX, grid.resolution);
  diagnostics.alignment_residual_y =
      nav_endpoint::safetyGridAlignmentResidual(grid.originY, grid.resolution);
  diagnostics.total_cells = grid.data.size();
  diagnostics.observed_free_cache_cells = observed_free.size();
  diagnostics.observed_free_applied_cells =
      observed_free.apply(grid, stamp_s, cfg.observed_free_ttl_s, 0.0F,
                          cfg.observed_free_inflation_radius_m);
  diagnostics.observed_free_cache_cells = observed_free.size();
  diagnostics.observed_before_overlays_cells = static_cast<std::size_t>(std::count_if(
      grid.data.begin(), grid.data.end(), [](float value) { return value < 100.0F; }));
  ...
  std::transform(grid.data.begin(), grid.data.end(), observed_before_overlays.begin(),
                 [](float value) -> std::uint8_t { return value < 100.0F ? 1U : 0U; });
  map_layers::Grid2D occupancy_source =
      map_layers::makeGrid2D(size, size, cfg.resolution, origin_x, origin_y, 0.0F);
  for (std::size_t i = 0; i < n; ++i) {
    const float x = xyz[i * 3 + 0];
    const float y = xyz[i * 3 + 1];
    const float z = xyz[i * 3 + 2];
    ...
    float &cell = occupancy_source.data[
        static_cast<std::size_t>(occupancy_source.index(row, col))];
    cell = std::max(cell, 100.0F);
  }
...
map_layers::Grid2D buildTerrainRiskGrid(const std::vector<float> &terrain_xyzi,
                                        const nav_endpoint::TraversabilityPose &pose,
                                        const CliConfig &cfg) {
  const int size = static_cast<int>(std::floor((2.0 * cfg.radius) / cfg.resolution));
  const double origin_x = nav_endpoint::snappedSafetyGridOrigin(pose.x, cfg.radius, cfg.resolution);
  const double origin_y = nav_endpoint::snappedSafetyGridOrigin(pose.y, cfg.radius, cfg.resolution);
  map_layers::Grid2D raw =
      map_layers::makeGrid2D(size, size, cfg.resolution, origin_x, origin_y, 0.0F);
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    const float cost = nav_endpoint::terrainHeightRiskCost(height, cfg.terrain_soft_height_m,
...
map_layers::Grid2D buildSurfaceTerrainRiskGrid(const std::vector<float> &terrain_xyzi,
                                               const nav_endpoint::TraversabilityPose &pose,
                                               const CliConfig &cfg) {
  const double origin_x = nav_endpoint::snappedSafetyGridOrigin(pose.x, cfg.radius, cfg.resolution);
  const double origin_y = nav_endpoint::snappedSafetyGridOrigin(pose.y, cfg.radius, cfg.resolution);
  const auto elevation = nav_endpoint::supportElevation(
      terrain_xyzi, origin_x + cfg.radius, origin_y + cfg.radius, cfg.resolution, cfg.radius,
      pose.z + cfg.z_min, pose.z + cfg.z_max, static_cast<float>(cfg.terrain_hard_height_m));
  if (!elevation) {
    throw std::runtime_error("connected support elevation is invalid");
  }
  const map_layers::TerrainRiskResult metrics = map_layers::computeTerrainRisk(*elevation);
  map_layers::Grid2D graded =
      map_layers::makeGrid2D(metrics.risk.rows, metrics.risk.cols, metrics.risk.resolution,
                             metrics.risk.originX, metrics.risk.originY, 0.0F);
  for (std::size_t i = 0; i < graded.data.size(); ++i) {
    const float slope_cost = nav_endpoint::terrainSlopeRiskCost(
        metrics.slopeDeg.data[i], cfg.terrain_soft_slope_deg, cfg.terrain_hard_slope_deg);
    const float step_cost = nav_endpoint::terrainHeightRiskCost(
        metrics.stepHeight.data[i], cfg.terrain_soft_height_m, cfg.terrain_hard_height_m);
    const float roughness_cost = nav_endpoint::terrainHeightRiskCost(
        metrics.roughness.data[i], cfg.terrain_soft_height_m, cfg.terrain_hard_height_m);
    graded.data[i] = std::max(slope_cost, std::max(step_cost, roughness_cost));
  }
  return graded;
}

SlowTerrainResult processSlowTerrainInput(const SlowTerrainInput &input,
                                          SlowTerrainWorkerState &state,
                                          const CliConfig &cfg) {
  SlowTerrainResult out;
  out.generation = input.generation;
  out.stamp_s = input.stamp_s;
  const auto slow_start = SteadyClock::now();
  const std::vector<float> scan_xyzi = xyzToXyzi(input.xyz);
  state.terrain_core.updateVehicle(input.pose.x, input.pose.y, input.pose.z, input.pose.roll,
                                   input.pose.pitch, input.pose.yaw);
  const auto terrain_core_start = SteadyClock::now();
  const nav_kernel::TerrainResult terrain_result = state.terrain_core.process(
      scan_xyzi.data(), static_cast<int>(scan_xyzi.size() / 4), input.stamp_s);
  out.terrain_connected_cells =
      static_cast<std::size_t>(std::max(0, terrain_result.connected_cells));
  out.terrain_ceiling_cells =
      static_cast<std::size_t>(std::max(0, terrain_result.ceiling_cells));
  out.timing.terrain_core_ms += elapsedMs(terrain_core_start);
  out.cache_points = state.terrain_core.storedPointCount();

  const nav_kernel::DynamicClearOrigin clear_origin{
      input.sensor_origin.x,
      input.sensor_origin.y,
      input.sensor_origin.z,
      true,
  };
  const auto dynamic_clear_start = SteadyClock::now();
  const nav_kernel::DynamicClearResult clear_result =
      state.dynamic_clear.filter(terrain_result.terrain_points, input.xyz, clear_origin,
                                 input.stamp_s);
  out.timing.dynamic_clear_ms += elapsedMs(dynamic_clear_start);

  const auto terrain_pack_start = SteadyClock::now();
  out.terrain_xyzi = limitXyzi(clear_result.kept_xyzi, cfg.max_points);
  out.terrain_ext_xyzi =
      combineTerrainExt(out.terrain_xyzi, clear_result.dynamic_xyzi, cfg.max_points);
  out.height_risk_grid = buildTerrainRiskGrid(clear_result.kept_xyzi, input.pose, cfg);
  out.surface_risk_grid = buildSurfaceTerrainRiskGrid(clear_result.kept_xyzi, input.pose, cfg);
  out.terrain_risk_grid = out.height_risk_grid;
  if (!nav_endpoint::mergeRiskGridMax(out.terrain_risk_grid, out.surface_risk_grid)) {
    throw std::runtime_error("terrain risk grid merge failed");
  }
  const auto [risk_cells, risk_max_cost] = terrainRiskStats(out.terrain_risk_grid);
  out.terrain_risk_cells = risk_cells;
  out.terrain_risk_max_cost = risk_max_cost;
  out.dynamic_clear_points = clear_result.stats.dynamic_points;
  out.dynamic_clear_ray_points = clear_result.stats.ray_cleared_points;
  out.dynamic_clear_evidence = clear_result.stats.evidence_voxels;
  out.dynamic_clear_free_voxels = clear_result.stats.free_voxels;
  out.timing.terrain_pack_ms += elapsedMs(terrain_pack_start);
  out.timing.slow_terrain_ms += elapsedMs(slow_start);
  return out;
}

void updateFusedGridDiagnostics(const map_layers::Grid2D &grid,
...
struct ExplorationGridMessage {
  lingtu_dds_ExplorationGrid msg{};
  std::vector<std::uint8_t> data;
  std::string session_id;
  std::string map_id;
};

struct ExplorationExecutionGridMessage {
  lingtu_dds_ExplorationExecutionGrid msg{};
...
OccupancyMessage toOccupancyMessage(const map_layers::Grid2D &grid, double stamp_s,
                                    const char *frame_id = "map") {
  OccupancyMessage out;
  const double stamp = std::isfinite(stamp_s) && stamp_s > 0.0 ? stamp_s : nowSeconds();
  fillHeader(out.msg.header, stamp, frame_id);
  out.msg.info.map_load_time.sec = static_cast<std::int32_t>(stamp);
  out.msg.info.map_load_time.nanosec = static_cast<std::uint32_t>(
      (stamp - static_cast<double>(out.msg.info.map_load_time.sec)) * 1e9);
...
OccupancyMessage toExplorationMessage(const map_layers::Grid2D &grid,
                                      const std::vector<std::uint8_t> &observed, double stamp_s) {
  OccupancyMessage out = toOccupancyMessage(grid, stamp_s);
  const auto cells = map_layers::encodeExplorationOccupancy(grid, observed);
  out.data.clear();
  out.data.reserve(cells.size());
  for (const std::int8_t cell : cells) {
    out.data.push_back(static_cast<std::uint8_t>(cell));
  }
  out.msg.data._maximum = static_cast<std::uint32_t>(out.data.size());
  out.msg.data._length = static_cast<std::uint32_t>(out.data.size());
  out.msg.data._buffer = out.data.data();
...
toExplorationSnapshotMessage(const map_layers::ExplorationGridSnapshot &snapshot,
  ...
  ExplorationGridMessage out;
  out.msg.header = occupancy.msg.header;
  out.msg.info = occupancy.msg.info;
  out.data = std::move(occupancy.data);
  out.msg.data._maximum = static_cast<std::uint32_t>(out.data.size());
  out.msg.data._length = static_cast<std::uint32_t>(out.data.size());
  out.msg.data._buffer = out.data.data();
  out.msg.data._release = false;
  ...
  out.msg.map_content_epoch = identity.map_content_epoch;
...
void fillOptionalTime(lingtu_dds_Time &out, double stamp_s) {
  out = {};
  if (!std::isfinite(stamp_s) || stamp_s <= 0.0 ||
      stamp_s >= static_cast<double>(std::numeric_limits<std::int32_t>::max()) + 1.0) {
    return;
  }
  const std::int64_t stamp_ns = stampNanoseconds(stamp_s);
  if (stamp_ns <= 0) {
    return;
  }
  out.sec = static_cast<std::int32_t>(stamp_ns / 1'000'000'000LL);
  out.nanosec = static_cast<std::uint32_t>(stamp_ns % 1'000'000'000LL);
}

bool executionTerrainRiskReady(const map_layers::ExplorationGridSnapshot &snapshot,
                               const std::optional<map_layers::Grid2D> &terrain_risk,
                               double terrain_risk_stamp_s, double maximum_age_s) {
  if (!terrain_risk || !std::isfinite(terrain_risk_stamp_s) || terrain_risk_stamp_s <= 0.0 ||
      !std::isfinite(maximum_age_s) || maximum_age_s < 0.0) {
    return false;
  }
  try {
    snapshot.Validate();
    terrain_risk->validate("exploration execution terrain risk");
  } catch (const std::exception &) {
    return false;
  }
  if (!std::isfinite(snapshot.grid.originX) || !std::isfinite(snapshot.grid.originY) ||
      !std::isfinite(terrain_risk->originX) || !std::isfinite(terrain_risk->originY) ||
      !map_layers::sameGeometry(snapshot.grid, *terrain_risk)) {
    return false;
  }
  for (const float cost : terrain_risk->data) {
    if (!std::isfinite(cost) || cost < 0.0F || cost > 100.0F) {
      return false;
    }
  }
  const double snapshot_stamp_s = static_cast<double>(snapshot.stamp_ns) * 1e-9;
  if (!std::isfinite(snapshot_stamp_s) || snapshot_stamp_s <= 0.0) {
    return false;
  }
  const double age_s = snapshot_stamp_s - terrain_risk_stamp_s;
  return age_s >= 0.0 && age_s <= maximum_age_s;
}

ExplorationExecutionGridMessage
toExplorationExecutionSnapshotMessage(const map_layers::ExplorationGridSnapshot &snapshot,
                                      const std::optional<map_layers::Grid2D> &terrain_risk,
                                      const std::string &session_id, std::uint64_t reset_epoch,
                                      double terrain_risk_stamp_s, double terrain_risk_max_age_s) {
  snapshot.Validate();
  OccupancyMessage occupancy = toExplorationMessage(snapshot.grid, snapshot.observed,
                                                    static_cast<double>(snapshot.stamp_ns) * 1e-9);
  const bool terrain_risk_ready = executionTerrainRiskReady(
      snapshot, terrain_risk, terrain_risk_stamp_s, terrain_risk_max_age_s);

  ExplorationExecutionGridMessage out;
  out.msg.header = occupancy.msg.header;
  out.msg.info = occupancy.msg.info;
  out.occupancy = std::move(occupancy.data);
  out.msg.occupancy._maximum = static_cast<std::uint32_t>(out.occupancy.size());
  out.msg.occupancy._length = static_cast<std::uint32_t>(out.occupancy.size());
  out.msg.occupancy._buffer = out.occupancy.data();
  out.msg.occupancy._release = false;

  out.terrain_cost.assign(out.occupancy.size(), 100U);
  if (terrain_risk_ready) {
    for (std::size_t cell = 0U; cell < out.terrain_cost.size(); ++cell) {
      out.terrain_cost[cell] = static_cast<std::uint8_t>(std::lround(terrain_risk->data[cell]));
    }
  }
  out.msg.terrain_cost._maximum = static_cast<std::uint32_t>(out.terrain_cost.size());
  out.msg.terrain_cost._length = static_cast<std::uint32_t>(out.terrain_cost.size());
  out.msg.terrain_cost._buffer = out.terrain_cost.data();
  out.msg.terrain_cost._release = false;
  out.session_id = session_id;
  out.msg.session_id = const_cast<char *>(out.session_id.c_str());
  out.msg.reset_epoch = reset_epoch;
  out.msg.generation = snapshot.generation;
  out.msg.live = true;
  fillOptionalTime(out.msg.terrain_risk_stamp, terrain_risk_stamp_s);
  out.msg.terrain_risk_ready = terrain_risk_ready;
  return out;
}
...
void writeStatus(const CliConfig &cfg, std::uint64_t odom_count, std::uint64_t tf_count,
                 std::uint64_t cloud_count, std::uint64_t publish_count,
                 std::uint64_t slow_publish_count, std::uint64_t map_clearing_count,
                 std::uint64_t cloud_clearing_count, std::uint64_t tf_reject_count,
                 std::uint64_t odom_frame_reject_count, std::uint64_t cloud_frame_reject_count,
                 std::uint64_t map_frame_jump_clear_count, std::uint64_t error_count,
                 std::size_t last_points, std::size_t self_filtered_points,
                 std::size_t terrain_points,
                 std::size_t terrain_ext_points, std::size_t dynamic_clear_points,
                 std::size_t dynamic_clear_ray_points, std::size_t dynamic_clear_evidence,
                 std::size_t dynamic_clear_free_voxels, std::size_t cache_points,
                 std::size_t terrain_risk_cells, float terrain_risk_max_cost,
                 std::size_t terrain_connected_cells, std::size_t terrain_ceiling_cells,
                 const std::string &exploration_session_id, std::uint64_t exploration_reset_epoch,
                 std::uint64_t exploration_snapshot_count, std::size_t exploration_free_cells,
                 std::size_t exploration_occupied_cells, std::size_t exploration_unknown_cells,
                 bool has_odom, bool has_map_odom_tf, double map_odom_tf_age_s,
                 double cloud_pose_gap_s, const std::string &odom_frame_id,
                 const std::string &cloud_frame_id, const std::string &last_frame_error,
                 const SafetyGridDiagnostics &grid_diagnostics,
                 const nav_endpoint::SafetyGridForwardProbe &forward_probe,
                 const nav_endpoint::SafetyGridNearBodyProbe &near_body_hard_probe,
  ...
  out << "{\n"
      ...
      << "  \"slow_hz\": " << cfg.slow_hz << ",\n"
      << "  \"tick_hz\": " << cfg.tick_hz << ",\n"
      << "  \"terrain_threads\": " << cfg.terrain_threads << ",\n"
      << "  \"resolution\": " << cfg.resolution << ",\n"
      << "  \"radius\": " << cfg.radius << ",\n"
      << "  \"exploration_map\": {\"enabled\": "
      << (cfg.exploration_enabled ? "true" : "false")
      << ", \"source\": \"maps.rolling_occupancy\", "
      << "\"topic\": \"/nav/exploration_snapshot\", "
      << "\"session_id\": \"" << jsonEscape(exploration_session_id) << "\", "
      << "\"reset_epoch\": " << exploration_reset_epoch << ", "
      << "\"snapshots\": " << exploration_snapshot_count << ", "
      << "\"free_cells\": " << exploration_free_cells << ", "
      << "\"occupied_cells\": " << exploration_occupied_cells << ", "
      << "\"unknown_cells\": " << exploration_unknown_cells << "},\n"
      << "  \"observed_free\": {\"ttl_s\": " << cfg.observed_free_ttl_s << ", "
      << "\"inflation_radius_m\": " << cfg.observed_free_inflation_radius_m << ", "
      ...
      << "\"observed_before_overlays_cells\": " << grid_diagnostics.observed_before_overlays_cells
      << ", "
      << "\"unknown_before_overlays_cells\": " << grid_diagnostics.unknown_before_overlays_cells
      << ", "
      ...
      << "  \"terrain_connectivity\": {\"enabled\": "
      << (cfg.terrain_connectivity ? "true" : "false") << ", "
      << "\"connected_cells\": " << terrain_connected_cells << ", "
      << "\"ceiling_cells\": " << terrain_ceiling_cells << ", "
      << "\"support_height_m\": " << cfg.terrain_under_vehicle_m << ", "
      << "\"max_step_m\": " << cfg.terrain_connection_height_m << ", "
      << "\"ceiling_delta_m\": " << cfg.terrain_ceiling_height_m << "},\n"
      << "  \"forward_probe\": {\"grid_generation\": " << forward_probe.grid_generation << ", "
  ...
  for (std::size_t i = 0; i < forward_probe.samples.size(); ++i) {
    ...
    out << "{\"distance_m\": " << sample.distance_m << ", "
        ...
        << "\"observed_before_overlays\": " << (sample.observed_before_overlays ? "true" : "false")
        << ", "
        << "\"unknown_before_overlays\": " << (sample.unknown_before_overlays ? "true" : "false")
        << ", "
  ...
  out << "]},\n"
      << "  \"near_body_hard_probe\": {\"radius_m\": " << near_body_hard_probe.radius_m
      << ", \"hard_cost\": " << near_body_hard_probe.hard_cost
      << ", \"total_count\": " << near_body_hard_probe.total_count
      << ", \"returned_count\": " << near_body_hard_probe.samples.size()
      << ", \"truncated\": " << (near_body_hard_probe.truncated ? "true" : "false")
      << ", \"pose\": {\"x\": " << near_body_hard_probe.pose.x
      << ", \"y\": " << near_body_hard_probe.pose.y
      << ", \"yaw\": " << near_body_hard_probe.pose.yaw << "}, \"samples\": [";
  for (std::size_t i = 0; i < near_body_hard_probe.samples.size(); ++i) {
    const auto &sample = near_body_hard_probe.samples[i];
    if (i > 0) {
      out << ", ";
    }
    out << "{\"distance_m\": " << sample.distance_m << ", "
        << "\"body_x\": " << sample.body_x << ", "
        << "\"body_y\": " << sample.body_y << ", "
        << "\"map_x\": " << sample.map_x << ", "
        << "\"map_y\": " << sample.map_y << ", "
        << "\"row\": " << sample.row << ", "
        << "\"col\": " << sample.col << ", "
  ...
  out << "]},\n"
      ...
      << "  \"self_filter\": {\"vehicle_length_m\": " << cfg.self_filter_length_m
      << ", \"vehicle_width_m\": " << cfg.self_filter_width_m
      << ", \"padding_m\": " << cfg.self_filter_padding_m
      << ", \"filtered_points\": " << self_filtered_points << "},\n"
  ...
  out.close();
  std::error_code ec;
  if (!lingtu::native::replaceSnapshotFile(tmp, path, &ec)) {
    std::fprintf(stderr, "traversability_dds: failed to replace status snapshot %s: %s\n",
...
class DdsRuntime {
 public:
  explicit DdsRuntime(int domain_id) {
    ...
    traversability_writer_ = writer(lingtu::message::kNavTraversability.dds_topic.data(),
                                    &lingtu_dds_OccupancyGrid_desc, "traversability");
    local_traversability_writer_ =
        writer(lingtu::message::kNavLocalTraversability.dds_topic.data(),
               &lingtu_dds_OccupancyGrid_desc, "local_traversability");
    exploration_grid_writer_ = writer(lingtu::message::kNavExplorationGrid.dds_topic.data(),
                                      &lingtu_dds_OccupancyGrid_desc, "exploration_grid");
    exploration_snapshot_writer_ = writer(lingtu::message::kNavExplorationSnapshot.dds_topic.data(),
                                          &lingtu_dds_ExplorationGrid_desc, "exploration_snapshot");
    exploration_execution_snapshot_writer_ =
        writer(lingtu::message::kNavExplorationExecutionSnapshot.dds_topic.data(),
               &lingtu_dds_ExplorationExecutionGrid_desc, "exploration_execution_snapshot");
    terrain_map_writer_ = writer(lingtu::message::kNavTerrainMap.dds_topic.data(),
                                 &lingtu_dds_PointCloud2_desc, "terrain_map");
    terrain_map_ext_writer_ = writer(lingtu::message::kNavTerrainMapExt.dds_topic.data(),
  ...
  void writeTraversability(const map_layers::Grid2D &grid, double source_stamp_s) {
  ...
  void writeLocalTraversability(const map_layers::Grid2D &grid, double source_stamp_s) {
    OccupancyMessage msg = toOccupancyMessage(grid, source_stamp_s, "odom");
    logDdsError(dds_write(local_traversability_writer_, &msg.msg),
                "dds_write(local_traversability)");
  }

  void writeExplorationGrid(const map_layers::Grid2D &grid,
                            const std::vector<std::uint8_t> &observed, double source_stamp_s) {
    OccupancyMessage msg = toExplorationMessage(grid, observed, source_stamp_s);
    logDdsError(dds_write(exploration_grid_writer_, &msg.msg), "dds_write(exploration_grid)");
  }
  ...
  void writeExplorationExecutionSnapshot(const map_layers::ExplorationGridSnapshot &snapshot,
                                         const std::optional<map_layers::Grid2D> &terrain_risk,
                                         const std::string &session_id, std::uint64_t reset_epoch,
                                         double terrain_risk_stamp_s,
                                         double terrain_risk_max_age_s) {
    ExplorationExecutionGridMessage msg =
        toExplorationExecutionSnapshotMessage(snapshot, terrain_risk, session_id, reset_epoch,
                                              terrain_risk_stamp_s, terrain_risk_max_age_s);
    msg.msg.occupancy._maximum = static_cast<std::uint32_t>(msg.occupancy.size());
    msg.msg.occupancy._length = static_cast<std::uint32_t>(msg.occupancy.size());
    msg.msg.occupancy._buffer = msg.occupancy.data();
    msg.msg.terrain_cost._maximum = static_cast<std::uint32_t>(msg.terrain_cost.size());
    msg.msg.terrain_cost._length = static_cast<std::uint32_t>(msg.terrain_cost.size());
    msg.msg.terrain_cost._buffer = msg.terrain_cost.data();
    msg.msg.session_id = const_cast<char *>(msg.session_id.c_str());
    logDdsError(dds_write(exploration_execution_snapshot_writer_, &msg.msg),
                "dds_write(exploration_execution_snapshot)");
  }
 ...
 private:
  ...
  dds_entity_t exploration_snapshot_writer_{0};
  dds_entity_t exploration_execution_snapshot_writer_{0};
  dds_entity_t cloud_clearing_reader_{0};
  dds_entity_t traversability_writer_{0};
  dds_entity_t local_traversability_writer_{0};
  dds_entity_t exploration_grid_writer_{0};
  ...
  try {
    ...
    const nav_endpoint::ExplorationSnapshotIdentity exploration_identity =
        nav_endpoint::resolveExplorationSnapshotIdentity({
            ...
            envOrEmpty("LINGTU_MAP_CONTENT_EPOCH"),
        });
    ...
    double last_map_odom_receive_s = 0.0;
    ...
    std::size_t last_self_filtered_points = 0;
    ...
    std::size_t last_terrain_connected_cells = 0;
    std::size_t last_terrain_ceiling_cells = 0;
    SafetyGridDiagnostics last_grid_diagnostics;
    double last_cloud_pose_gap_s = -1.0;
    double last_cloud_stamp_s = 0.0;
    std::size_t cache_points = 0;
    ...
    nav_endpoint::SafetyGridNearBodyProbe last_near_body_hard_probe;
    double last_terrain_risk_stamp_s = 0.0;
    std::uint64_t slow_input_generation = 0U;
    std::uint64_t last_slow_result_generation_applied = 0U;
    std::uint64_t slow_reset_generation = 0U;
    const auto slow_state = std::make_shared<SlowTerrainWorkerState>(cfg);
    auto slow_worker = nav_endpoint::makeLatestOnlyWorker<SlowTerrainInput, SlowTerrainResult>(
        [slow_state, cfg](SlowTerrainInput input) -> SlowTerrainResult {
          try {
            return processSlowTerrainInput(input, *slow_state, cfg);
          } catch (const std::exception &exc) {
            SlowTerrainResult result;
            result.ok = false;
            result.error = exc.what();
            result.generation = input.generation;
            result.stamp_s = input.stamp_s;
            return result;
          }
        },
        [slow_state]() {
          slow_state->terrain_core.clear();
          slow_state->dynamic_clear.reset();
        });
    nav_endpoint::ObservedFreeCache observed_free(cfg.resolution);
    std::optional<nav_endpoint::RollingExplorationMap> rolling_exploration;
    if (cfg.exploration_enabled) {
      rolling_exploration.emplace(rollingExplorationConfig(cfg));
    }
    ...
    std::size_t exploration_occupied_cells = 0U;
    std::size_t exploration_unknown_cells = 0U;
    auto clear_runtime_maps = [&]() {
      slow_reset_generation = slow_input_generation;
      slow_worker.requestReset();
      cache_points = 0;
      ...
      last_terrain_connected_cells = 0;
      last_terrain_ceiling_cells = 0;
      ...
      last_forward_probe = {};
      last_near_body_hard_probe = {};
      last_terrain_risk_stamp_s = 0.0;
      observed_free.clear();
      if (rolling_exploration) {
        rolling_exploration->Clear();
      }
      ++exploration_reset_epoch;
      exploration_free_cells = 0U;
      exploration_occupied_cells = 0U;
      exploration_unknown_cells = 0U;
      exploration_snapshot_count = 0U;
    };
    auto refresh_map_body = [&]() {
      map_body = odom_body ? mapBodyTransform(*odom_body, odom_frame_id, map_odom)
                           : std::optional<nav_endpoint::RigidTransform>{};
    };
    auto clear_source_epoch = [&]() {
      clear_runtime_maps();
      ...
      odom_body.reset();
      map_body.reset();
      last_map_odom_receive_s = 0.0;
      last_cloud_stamp_s = 0.0;
      ++clock_rebase_count;
    };
    double last_publish = 0.0;
    double next_slow_publish = 0.0;
    double next_status = steadySeconds() + 5.0;
    ...
    std::fprintf(stderr,
                 "lingtu_traversability_dds: domain=%d publish_hz=%.2f slow_hz=%.2f "
                 "resolution=%.2f radius=%.1f\n",
                 cfg.domain_id, cfg.publish_hz, cfg.slow_hz, cfg.resolution, cfg.radius);
    ...
    while (g_running) {
      ...
      dds.drainTf([&](const lingtu_dds_TFMessage &msg) {
        ...
        const auto stamp_decision =
            nav_endpoint::classifySourceOrder(map_odom ? map_odom->stamp_s : 0.0,
                                              next_map_odom->stamp_s, kSourceClockRebaseThresholdS);
        if (stamp_decision == nav_endpoint::SourceStampDecision::kReject) {
          ++tf_reject_count;
          last_frame_error = "map_odom_tf_stamp_invalid";
          return;
        }
        bool map_frame_jump = false;
        if (map_odom) {
          const auto correction = nav_endpoint::composeTransforms(
              *next_map_odom, nav_endpoint::inverseTransform(*map_odom));
          const double translation_jump =
              std::sqrt(correction.translation.x * correction.translation.x +
                        correction.translation.y * correction.translation.y +
                        correction.translation.z * correction.translation.z);
          map_frame_jump = translation_jump > 0.50 || std::abs(correction.yaw) > 0.25;
        }
        if (map_frame_jump) {
          clear_runtime_maps();
          map_body_buffer.clear();
        ...
        } else if (stamp_decision == nav_endpoint::SourceStampDecision::kClockRebase) {
          clear_source_epoch();
          last_frame_error = "source_clock_rebase";
        }
        map_odom = next_map_odom;
        last_map_odom_receive_s = steadySeconds();
      ...
      dds.drainOdometry([&](const lingtu_dds_Odometry &msg) {
        ++odom_count;
        if (const char *frame_error = nav_endpoint::odometryFramePairError(msg)) {
        ...
        const auto odom_stamp_decision =
            nav_endpoint::classifySourceOrder(odom_body ? odom_body->stamp_s : 0.0,
                                              next_odom_body.stamp_s, kSourceClockRebaseThresholdS);
        if (odom_stamp_decision == nav_endpoint::SourceStampDecision::kReject) {
          ++odom_frame_reject_count;
          last_frame_error = "odom_stamp_invalid";
          return;
        }
        if (odom_stamp_decision == nav_endpoint::SourceStampDecision::kClockRebase) {
          clear_source_epoch();
          last_frame_error = "source_clock_rebase";
        }
        odom_frame_id = frame;
      ...
      dds.drainCloud([&](const lingtu_dds_PointCloud2 &msg) {
        ++cloud_count;
        const double schedule_now = steadySeconds();
        const double cloud_stamp_s = nav_endpoint::ddsStampSeconds(msg.header.stamp);
        const auto cloud_stamp_decision = nav_endpoint::classifySourceOrder(
            last_cloud_stamp_s, cloud_stamp_s, kSourceClockRebaseThresholdS);
        if (cloud_stamp_decision == nav_endpoint::SourceStampDecision::kReject) {
        ...
        if (last_publish > 0.0 && schedule_now - last_publish < publish_period_s * 0.75) {
          return;
        }
        last_cloud_pose_gap_s = map_body_buffer.nearestGap(cloud_stamp_s);
        const auto cloud_map_body = map_body_buffer.sample(cloud_stamp_s, cfg.cloud_pose_max_gap_s);
        if (!cloud_map_body) {
          ++cloud_frame_reject_count;
        ...
        const auto cloud_sensor_origin =
            cloud_pose ? nav_endpoint::traversabilitySensorOrigin(*cloud_pose,
                                                                  {
        ...
        if (source_frame == "odom") {
          ...
          cloud_map_odom = map_odom_buffer.sample(cloud_stamp_s, tf_gap_s);
          if (!cloud_map_odom) {
            ++tf_reject_count;
        ...
        last_publish = schedule_now;
        try {
          const auto convert_start = SteadyClock::now();
          MapCloudResult cloud = cloudToMapXyz(
              msg, cloud_map_body, cloud_map_odom, cfg.terrain_cache_max_points,
              cfg.self_filter_length_m, cfg.self_filter_width_m, cfg.self_filter_padding_m);
          ...
          last_self_filtered_points = cloud.self_filtered_points;
          if (!xyz.empty()) {
            ...
            map_layers::Grid2D occupancy = buildOccupancyGrid(
                fast_xyz, pose, *cloud_sensor_origin, cloud_stamp_s, observed_free, cfg,
                last_grid_diagnostics, observed_before_overlays, occupancy_source_grid);
            timing.fast_occupancy_ms += elapsedMs(occupancy_start);
            std::optional<map_layers::ExplorationGridSnapshot> execution_snapshot;
            if (rolling_exploration) {
              try {
                auto exploration = rolling_exploration->Update(
                    fast_xyz, static_cast<float>(cloud_sensor_origin->x),
                    static_cast<float>(cloud_sensor_origin->y),
                    static_cast<float>(cloud_sensor_origin->z), static_cast<float>(pose.z),
                    stampNanoseconds(cloud_stamp_s));
                execution_snapshot = exploration;
                if (terrain_risk_grid &&
                    map_layers::sameGeometry(exploration.grid, *terrain_risk_grid)) {
                  for (std::size_t cell = 0U; cell < exploration.grid.data.size(); ++cell) {
                    if (exploration.observed[cell] != 0U) {
                      exploration.grid.data[cell] =
                          std::max(exploration.grid.data[cell], terrain_risk_grid->data[cell]);
                    }
                  }
                }
                ...
                exploration_occupied_cells = exploration.occupied_cells;
                exploration_unknown_cells = exploration.unknown_cells;
                ++exploration_snapshot_count;
              } catch (const std::exception &exc) {
                ++error_count;
                std::fprintf(stderr, "traversability_dds: exploration map update failed: %s\n",
                             exc.what());
              }
            }
            ...
            if (next_slow_publish <= 0.0) {
              next_slow_publish = schedule_now;
            }
            if (schedule_now + slow_tolerance_s >= next_slow_publish) {
              do {
                next_slow_publish += slow_period_s;
              } while (next_slow_publish <= schedule_now - slow_period_s);
              SlowTerrainInput slow_input;
              slow_input.generation = ++slow_input_generation;
              slow_input.stamp_s = cloud_stamp_s;
              slow_input.xyz = xyz;
              slow_input.pose = pose;
              slow_input.sensor_origin = *cloud_sensor_origin;
              slow_worker.submit(std::move(slow_input));
            }
            if (auto slow_result = slow_worker.takeLatestResult()) {
              const std::uint64_t reject_before_generation =
                  std::max(last_slow_result_generation_applied, slow_reset_generation);
              if (!slow_result->ok) {
                ++error_count;
                last_slow_result_generation_applied =
                    std::max(last_slow_result_generation_applied, slow_result->generation);
                std::fprintf(stderr, "traversability_dds: slow terrain worker failed: %s\n",
                             slow_result->error.c_str());
              } else if (nav_endpoint::slowTerrainResultCanUpdate(
                             slow_result->generation, reject_before_generation)) {
                timing.slow_terrain_ms += slow_result->timing.slow_terrain_ms;
                timing.terrain_core_ms += slow_result->timing.terrain_core_ms;
                timing.dynamic_clear_ms += slow_result->timing.dynamic_clear_ms;
                timing.terrain_pack_ms += slow_result->timing.terrain_pack_ms;
                height_risk_grid = std::move(slow_result->height_risk_grid);
                surface_risk_grid = std::move(slow_result->surface_risk_grid);
                terrain_risk_grid = std::move(slow_result->terrain_risk_grid);
                last_terrain_risk_stamp_s = slow_result->stamp_s;
                last_terrain_risk_cells = slow_result->terrain_risk_cells;
                last_terrain_risk_max_cost = slow_result->terrain_risk_max_cost;
                last_terrain_points = slow_result->terrain_xyzi.size() / 4;
                last_terrain_ext_points = slow_result->terrain_ext_xyzi.size() / 4;
                last_dynamic_clear_points = slow_result->dynamic_clear_points;
                last_dynamic_clear_ray_points = slow_result->dynamic_clear_ray_points;
                last_dynamic_clear_evidence = slow_result->dynamic_clear_evidence;
                last_dynamic_clear_free_voxels = slow_result->dynamic_clear_free_voxels;
                last_terrain_connected_cells = slow_result->terrain_connected_cells;
                last_terrain_ceiling_cells = slow_result->terrain_ceiling_cells;
                cache_points = slow_result->cache_points;
                last_slow_result_generation_applied = slow_result->generation;
                if (!slow_result->terrain_xyzi.empty() || !slow_result->terrain_ext_xyzi.empty()) {
                  const auto terrain_write_start = SteadyClock::now();
                  dds.writeTerrainMap(slow_result->terrain_xyzi, slow_result->stamp_s);
                  dds.writeTerrainMapExt(slow_result->terrain_ext_xyzi, slow_result->stamp_s);
            ...
            const auto control_risk = nav_endpoint::makeControlRiskGrid(
                occupancy, terrain_risk_grid ? &*terrain_risk_grid : nullptr);
            if (!control_risk) {
              throw std::runtime_error("control risk grid invalid");
            }
            updateFusedGridDiagnostics(*control_risk, last_grid_diagnostics);
            ...
            last_forward_probe = nav_endpoint::buildStraightForwardSafetyGridProbe(
                {pose.x, pose.y, pose.yaw}, probe_layers, kNativeTeleopStraightProbeHorizonM,
                kNativeTeleopTerrainMinimumStepM, publish_count + 1, slow_publish_count,
                cloud_stamp_s, last_terrain_risk_stamp_s);
            last_near_body_hard_probe = nav_endpoint::buildNearBodyHardSafetyGridProbe(
                {pose.x, pose.y, pose.yaw}, probe_layers, kNearBodyHardProbeRadiusM, 90.0F,
                kNearBodyHardProbeMaxSamples);
            const auto write_start = SteadyClock::now();
            if (execution_snapshot) {
              dds.writeExplorationExecutionSnapshot(*execution_snapshot, terrain_risk_grid,
                                                    exploration_session_id, exploration_reset_epoch,
                                                    last_terrain_risk_stamp_s,
                                                    std::max(0.0, 1.5 / cfg.slow_hz));
            }
            dds.writeTraversability(*control_risk, cloud_stamp_s);
            if (odom_frame_id == "odom" && odom_body && map_odom && map_odom->valid) {
              const auto local_risk = nav_endpoint::projectRollingRiskGridToOdom(
                  *control_risk, *map_odom, odom_body->translation,
                  cfg.terrain_no_data_obstacle ? 100.0F : 0.0F);
              if (local_risk.ok()) {
                dds.writeLocalTraversability(*local_risk.grid, cloud_stamp_s);
              } else {
                last_frame_error = std::string("local_risk_projection_") +
                                   std::string(local_risk.reason);
              }
            }
      ...
      const double schedule_now = steadySeconds();
      if (schedule_now >= next_status) {
        next_status = schedule_now + 5.0;
        std::fprintf(
            stderr,
            "traversability_dds: odom=%llu registered_clouds=%llu published=%llu slow=%llu "
            "errors=%llu clock_rebases=%llu points=%zu timing(loop=%.2f input=%.2f convert=%.2f "
            "occ=%.2f slow=%.2f core=%.2f clear=%.2f pack=%.2f write=%.2f overrun=%.2f)\n",
            ...
            static_cast<unsigned long long>(clock_rebase_count), last_points, last_timing.loop_ms,
            last_timing.input_callbacks_ms, last_timing.cloud_convert_ms,
            last_timing.fast_occupancy_ms, last_timing.slow_terrain_ms, last_timing.terrain_core_ms,
            last_timing.dynamic_clear_ms, last_timing.terrain_pack_ms, last_timing.dds_write_ms,
            last_timing.overrun_ms);
        writeStatus(cfg, odom_count, tf_count, cloud_count, publish_count, slow_publish_count,
                    map_clearing_count, cloud_clearing_count, tf_reject_count,
                    odom_frame_reject_count, cloud_frame_reject_count, map_frame_jump_clear_count,
                    error_count, last_points, last_self_filtered_points, last_terrain_points,
                    last_terrain_ext_points,
                    last_dynamic_clear_points, last_dynamic_clear_ray_points,
                    last_dynamic_clear_evidence, last_dynamic_clear_free_voxels, cache_points,
                    last_terrain_risk_cells, last_terrain_risk_max_cost,
                    last_terrain_connected_cells, last_terrain_ceiling_cells,
                    exploration_session_id, exploration_reset_epoch, exploration_snapshot_count,
                    exploration_free_cells, exploration_occupied_cells, exploration_unknown_cells,
                    map_body.has_value(), map_odom.has_value(),
                    last_map_odom_receive_s > 0.0 ? schedule_now - last_map_odom_receive_s : -1.0,
                    last_cloud_pose_gap_s, odom_frame_id, cloud_frame_id, last_frame_error,
                    last_grid_diagnostics, last_forward_probe, last_near_body_hard_probe,
                    last_timing);
      }
      ...
      if (before_sleep < next_tick) {
        ...
        timing.sleep_ms = elapsedMs(before_sleep, SteadyClock::now());
      } else {
      ...
      if (timing.cloud_convert_ms > 0.0 || timing.fast_occupancy_ms > 0.0 ||
          ...
          timing.dds_write_ms > 0.0 || timing.overrun_ms > 0.0) {
        last_timing = timing;
      }
    }
    return 0;
