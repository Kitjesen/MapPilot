...
inline bool pointFieldIsScalarFloat32(std::uint8_t datatype, std::uint32_t count,
                                      std::uint32_t offset, std::uint32_t point_step) {
  return datatype == kPointFieldFloat32 && count == 1 && point_step >= sizeof(float) &&
         offset <= point_step - sizeof(float);
}

inline bool pointCloudStorageIsValid(std::uint32_t width, std::uint32_t height,
                                     std::uint32_t point_step, std::uint32_t row_step,
  ...
  const std::size_t actual_row_step =
      row_step == 0 ? minimum_row_step : static_cast<std::size_t>(row_step);
