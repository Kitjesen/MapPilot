...
void writeLocalCandidatesJson(
  ...
  out << "\"traversability_soft_cost\": "
      << snapshot.traversabilitySoftCost << ", ";
  out << "\"traversability_hard_cost\": "
      << snapshot.traversabilityHardCost << ", ";
  out << "\"valid_rotation_count\": " << snapshot.validRotationCount << ", ";
  out << "\"returned_count\": " << snapshot.candidates.size() << ", ";
  out << "\"truncated\": "
      << (snapshot.candidates.size() < static_cast<std::size_t>(
              std::max(0, snapshot.validRotationCount))
              ? "true"
              : "false")
      << ", ";
  out << "\"selected_flat_group_id\": " << snapshot.selectedGroupId << ", ";
  out << "\"selected_rotation_index\": "
      << (snapshot.selectedGroupId >= 0
              ? snapshot.selectedGroupId / nav_kernel::kGroupNum
              : -1)
      << ", ";
  ...
  for (std::size_t index = 0; index < snapshot.candidates.size(); ++index) {
    ...
    out << "\"state\": \""
        << nav_kernel::localCandidateStateName(candidate.state) << "\", ";
    out << "\"dominant_state\": \""
        << nav_kernel::localCandidateStateName(candidate.state) << "\", ";
    out << "\"rotation_allowed\": "
        << (candidate.rotationAllowed ? "true" : "false") << ", ";
    ...
    out << "\"collision_free_path_count\": "
        << candidate.collisionFreePathCount << ", ";
    out << "\"direction_allowed_path_count\": "
        << candidate.directionAllowedPathCount << ", ";
    out << "\"terrain_allowed_path_count\": "
        << candidate.terrainAllowedPathCount << ", ";
    out << "\"direction_scored_path_count\": "
        << candidate.directionScoredPathCount << ", ";
    out << "\"height_cost_allowed_path_count\": "
        << candidate.heightCostAllowedPathCount << ", ";
    out << "\"terrain_soft_penalized_path_count\": "
        << candidate.terrainSoftPenalizedPathCount << ", ";
    out << "\"contributing_path_count\": "
...
void writeLocalMapJson(
    ...
    bool obstacle_points_fresh,
    bool traversability_fresh) {
  const bool enabled = point_limit > 0;
  const std::size_t total_points = obstacle_xyzh.size() / 4;
  std::vector<std::size_t> obstacle_indices(total_points);
  std::iota(obstacle_indices.begin(), obstacle_indices.end(), 0);
  ...
  const bool has_debug_origin = sensor_origin.valid &&
      std::isfinite(sensor_origin.x) && std::isfinite(sensor_origin.y);
  const auto sampled_obstacles = enabled
      ? detail::sampleDebugIndices(
            ...
            [&](std::size_t index) {
              ...
              const double dy = static_cast<double>(obstacle_xyzh[offset + 1]) - sensor_origin.y;
              return dx * dx + dy * dy;
            })
      : std::vector<std::size_t>{};
  const auto sampled_risk = enabled
      ? detail::sampleDebugIndices(
            risk_indices,
            point_limit,
            has_debug_origin,
            [&](std::size_t flat) {
              const std::size_t cols = traversability.cols > 0
                  ? static_cast<std::size_t>(traversability.cols)
                  : 1;
  ...
  const bool traversability_complete =
      enabled && risk_count == risk_indices.size();
  out << "{";
  out << "\"enabled\": " << (enabled ? "true" : "false") << ", ";
  out << "\"frame_id\": \"map\", ";
  out << "\"obstacle_points_fresh\": "
      << (obstacle_points_fresh ? "true" : "false") << ", ";
  out << "\"obstacle_points_total\": " << total_points << ", ";
  out << "\"obstacle_points_returned\": " << point_count << ", ";
  out << "\"obstacle_points_truncated\": "
      << (obstacle_points_truncated ? "true" : "false") << ", ";
  out << "\"obstacle_sampling\": \""
      << (obstacle_points_truncated && has_debug_origin
              ? "nearest_sensor_origin"
              : obstacle_points_truncated ? "uniform_source_order" : "complete")
      << "\", ";
  out << "\"sampling_origin_xy\": [" << sensor_origin.x << ", "
      << sensor_origin.y << "], ";
  out << "\"obstacle_points\": [";
  for (std::size_t index = 0; index < point_count; ++index) {
    ...
    out << "[" << obstacle_xyzh[offset] << ", "
        << obstacle_xyzh[offset + 1] << ", "
        << obstacle_xyzh[offset + 2] << ", "
        << obstacle_xyzh[offset + 3] << "]";
  }
  out << "], ";
  out << "\"traversability\": {";
  ...
  out << "\"origin_xy\": [" << traversability.origin_x << ", "
  ...
  out << "\"truncated\": "
      << (risk_count < risk_indices.size() ? "true" : "false") << ", ";
  out << "\"sampling\": \""
      << (risk_count < risk_indices.size() && has_debug_origin
              ? "nearest_sensor_origin"
              : risk_count < risk_indices.size() ? "uniform_source_order" : "complete")
      << "\", ";
  out << "\"default_cost\": " << (traversability_complete ? "0" : "null") << ", ";
  out << "\"unreported_cells\": \""
      << (traversability_complete ? "zero_cost" : "not_serialized") << "\", ";
  out << "\"risk_cells\": [";
  for (std::size_t index = 0; index < risk_count; ++index) {
    ...
    const std::size_t cols = traversability.cols > 0
        ? static_cast<std::size_t>(traversability.cols)
        : 1;
    out << "[" << flat / cols << ", " << flat % cols << ", "
        << traversability.values[flat] << "]";
  }
  out << "]}}";
}
...
void writeStatusSnapshot(
  ...
  const StatusSnapshotWriterDiagnostics snapshot_diagnostics =
      snapshot_writer.diagnostics();
  ...
  out << "{\n"
      ...
      << (cfg.operator_takeover_latched ? "true" : "false") << ", "
      << "\"resume_required\": "
      << (cfg.resume_required ? "true" : "false") << "},\n"
      << "  \"final_cmd_vel\": ";
  writeTwistJson(out, final_cmd_vel);
  out << ",\n"
      << "  \"global_planner\": \"" << jsonEscape(cfg.global_planner) << "\",\n"
      ...
      << "\"simplify_path\": "
      << (cfg.far_options.simplify_path ? "true" : "false") << "},\n"
      ...
      << "  \"live_obstacle_inflation_radius_m\": "
      << cfg.live_obstacle_inflation_radius_m << ",\n"
      << "  \"obstacle_inflation\": {\"owner\": \"local_planner_footprint\", "
      << "\"margin_m\": " << cfg.live_obstacle_inflation_radius_m << ", "
      ...
      << "  \"obstacle_source_shares\": {"
      << "\"registered_cloud\": " << cfg.obstacle_registered_share << ", "
      << "\"terrain_map\": " << cfg.obstacle_terrain_share << ", "
      << "\"terrain_map_ext\": " << cfg.obstacle_terrain_ext_share << "},\n"
      << "  \"terrain_map_ext_diagnostics_only\": "
      << (cfg.obstacle_terrain_ext_share <= 0.0 ? "true" : "false") << ",\n"
      << "  \"terrain_map_ext_planner_enabled\": "
      << (cfg.obstacle_terrain_ext_share > 0.0 ? "true" : "false") << ",\n"
      ...
      << "\"xyz\": [" << last_sensor_origin.x << ", "
      << last_sensor_origin.y << ", " << last_sensor_origin.z << "]},\n"
      ...
      << (cfg.input_require_traversability ? "true" : "false") << ", "
      << "\"require_localization_health\": "
      << (cfg.input_require_localization_health ? "true" : "false") << ", "
      << "\"require_driver_control\": "
      << (cfg.input_require_driver_control ? "true" : "false") << ", "
      << "\"odom_age_s\": " << (std::isfinite(input_gate.odom_age_s) ? input_gate.odom_age_s : -1.0) << ", "
      << "\"tf_age_s\": " << (std::isfinite(input_gate.tf_age_s) ? input_gate.tf_age_s : -1.0) << ", "
      << "\"cloud_age_s\": " << (std::isfinite(input_gate.cloud_age_s) ? input_gate.cloud_age_s : -1.0) << ", "
      << "\"traversability_age_s\": "
      << (std::isfinite(input_gate.traversability_age_s)
              ? input_gate.traversability_age_s
              : -1.0)
      << ", "
      ...
      << (std::isfinite(input_gate.driver_control_age_s)
              ? input_gate.driver_control_age_s
              : -1.0)
      << ", "
      << "\"driver_control_ready\": "
      << (input_gate.driver_control_ready ? "true" : "false") << ", "
      << "\"driver_control_reason\": \""
      << jsonEscape(input_gate.driver_control_reason) << "\", "
      << "\"localization_healthy\": "
      << (input_gate.localization_healthy ? "true" : "false") << ", "
      << "\"localization_state\": \""
      << jsonEscape(input_gate.localization_state) << "\", "
  ...
  writeLocalMapJson(
      ...
      local_map_obstacle_xyzh,
      local_map_traversability,
      last_sensor_origin,
      input_gate.ready,
      has_traversability);
  out << ",\n"
  ...
  out << "\n"
      ...
      << "\"writing\": "
      << (snapshot_diagnostics.writing ? "true" : "false") << ", "
      << "\"pending\": "
      << (snapshot_diagnostics.pending ? "true" : "false") << "},\n"
      << "  \"timing_ms\": {\n"
      << "    \"loop\": " << timing.loop_ms << ",\n"
      ...
      << "  }\n"
      << "}\n";
  snapshot_writer.submit(out.str());
}
