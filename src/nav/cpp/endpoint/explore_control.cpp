...
ExplorationControlResult ExploreControl::Finish(
...
ExplorationControlResult ExploreControl::Apply(
    const ExplorationControlRequest& request) {
  ...
  if (!std::isfinite(request.stamp_s) ||
      !std::isfinite(request.now_s) ||
      !std::isfinite(request.max_age_s) ||
      !std::isfinite(request.future_tolerance_s) ||
      request.max_age_s <= 0.0 ||
      request.future_tolerance_s < 0.0 ||
  ...
  if (request.request_id.size() > 128U ||
      request.session_id.size() > 128U ||
  ...
  switch (kind) {
    case lingtu::message::ExplorationCommandKind::kStart: {
      ...
      active_ = true;
      paused_ = false;
      session_id_ = request.session_id.empty()
          ? request.request_id
          : request.session_id;
      ExplorationControlResult actions;
    ...
    case lingtu::message::ExplorationCommandKind::kStop: {
      if (!active_ && !paused_ && !request.goal_pending &&
      ...
      actions.clear_history = true;
      actions.request_cancel = request.goal_pending;
      actions.cancel_reason = request.reason.empty()
          ? "exploration_stopped"
          : request.reason;
