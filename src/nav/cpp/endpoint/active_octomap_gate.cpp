...
MapStore* ActiveOctomapGate::resolveStore(
    const std::filesystem::path& configured_octomap_path,
...
ActiveOctomapIdentityResult ActiveOctomapGate::currentIdentity(
  ...
  auto source = validateActiveSource(
      *store, configured_octomap_path, expected_frame_id_);
  if (!source.ok()) {
...
ActiveOctomapGateResult ActiveOctomapGate::prepare(
  ...
  if (cached_artifact_ &&
      lingtu::nav::plan::sameMapIdentity(
          cached_artifact_->identity(), first.source->identity) &&
  ...
  const bool stable =
      ...
      lingtu::nav::plan::sameMapIdentity(
          first.source->identity, second.source->identity) &&
  ...
  cached_artifact_ = std::shared_ptr<const ValidatedOctomap>(
      new ValidatedOctomap(
          snapshot,
          std::move(second.source->identity)));
  return {cached_artifact_, ""};
}
