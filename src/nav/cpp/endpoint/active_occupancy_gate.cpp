...
ActiveSourceResult validateActiveSource(
    MapStore& store,
...
plan::far::FarGridMap toFarMap(
    const lingtu::maps::OccupancyArtifactData& occupancy,
    plan::MapIdentity identity,
  ...
  for (std::size_t i = 0; i < occupancy.grid.size(); ++i) {
    ...
    out.cells[i] = value < 0
        ? plan::far::kUnknownCell
        : value >= 50 ? plan::far::kOccupiedCell : plan::far::kFreeCell;
  }
  out.Validate();
  return out;
}
...
ActiveOccupancyGate::ActiveOccupancyGate(
...
MapStore* ActiveOccupancyGate::resolveStore(
    const std::filesystem::path& configured_occupancy_path,
    std::string* error) const {
  const auto root = map_root_.empty()
      ? inferMapRoot(configured_occupancy_path, error)
...
ActiveOccupancyIdentityResult ActiveOccupancyGate::currentIdentity(
  ...
  auto source = validateActiveSource(
      *store, configured_occupancy_path, expected_frame_id_);
  if (!source.ok()) {
...
ActiveOccupancyGateResult ActiveOccupancyGate::prepare(
  ...
  auto first = validateActiveSource(
      *store, configured_occupancy_path, expected_frame_id_);
  if (!first.ok()) {
  ...
  auto second = validateActiveSource(
      *store, configured_occupancy_path, expected_frame_id_);
  const bool stable = second.ok() &&
  ...
  if (!stable) {
    ...
    return {
        nullptr,
        second.ok()
            ? "active map or occupancy bytes changed during validation"
...
plan::GlobalPlanResult runWithActiveOccupancy(
  ...
  if (!prepared.ok()) {
    throw std::runtime_error(
        "active_occupancy_gate_rejected: " + prepared.reason);
  }
