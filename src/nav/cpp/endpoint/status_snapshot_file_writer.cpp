...
bool writeSnapshotFile(
  ...
  if (!output) {
    std::fprintf(
        stderr,
        "nav_native: failed to open status snapshot %s\n",
  ...
  if (!output) {
    std::fprintf(
        stderr,
        "nav_native: failed to write status snapshot %s\n",
        temporary.string().c_str());
    return false;
  }

  std::error_code replace_error;
  if (!lingtu::message::replaceSnapshotFile(
          temporary, path, &replace_error)) {
    std::fprintf(
        stderr,
        "nav_native: failed to replace status snapshot %s: %s\n",
        path.string().c_str(),
        replace_error.message().c_str());
    return false;
  }
  return true;
}
...
struct StatusSnapshotFileWriter::Impl {
  ...
  void flush() {
    ...
    idle.wait(lock, [this]() {
      return !pending.has_value() && !stats.writing;
    });
  }
  ...
  void run() noexcept {
    ...
    while (true) {
      ...
      if (!pending.has_value() && stopping) break;
...
StatusSnapshotFileWriter::StatusSnapshotFileWriter(
    std::filesystem::path path)
