...
std::int64_t stampNanoseconds(double stamp_s) {
  return static_cast<std::int64_t>(
      std::llround(std::max(0.0, stamp_s) * 1e9));
}

std::string explorationSessionId() {
  if (const char* configured =
          std::getenv("LINGTU_EXPLORATION_SESSION_ID");
...
CliConfig parseArgs(int argc, char** argv) {
  ...
  cfg.terrain_ceiling_height_m = std::max(
      cfg.terrain_connection_height_m + 0.01,
      cfg.terrain_ceiling_height_m);
  cfg.terrain_connectivity_radius_cells =
      std::max(1, cfg.terrain_connectivity_radius_cells);
...
nav_endpoint::RollingExplorationConfig rollingExplorationConfig(
    const CliConfig& cfg) {
  nav_endpoint::RollingExplorationConfig out;
  out.size_x = std::max(
      32, static_cast<int>(std::floor(2.0 * cfg.radius / cfg.resolution)));
  out.size_y = out.size_x;
  out.size_z = std::max(
      16, static_cast<int>(std::ceil(6.0 / cfg.resolution)));
...
OccupancyMessage toExplorationMessage(
    const map_layers::Grid2D& grid,
...
ExplorationGridMessage toExplorationSnapshotMessage(
    const map_layers::ExplorationGridSnapshot& snapshot,
    const std::string& session_id,
    std::uint64_t reset_epoch) {
  snapshot.Validate();
  OccupancyMessage occupancy = toExplorationMessage(
      snapshot.grid,
      snapshot.observed,
  ...
  out.msg.data._maximum =
      static_cast<std::uint32_t>(out.data.size());
  out.msg.data._length =
      static_cast<std::uint32_t>(out.data.size());
  out.msg.data._buffer = out.data.data();
  out.msg.data._release = false;
  ...
  out.msg.session_id =
      const_cast<char*>(out.session_id.c_str());
  out.msg.map_id = const_cast<char*>(out.map_id.c_str());
  out.msg.map_version = 0;
  out.msg.artifact_hash =
      const_cast<char*>(out.artifact_hash.c_str());
...
void writeStatus(
    ...
    std::size_t terrain_connected_cells,
    std::size_t terrain_ceiling_cells,
    const std::string& exploration_session_id,
    std::uint64_t exploration_reset_epoch,
    std::uint64_t exploration_snapshot_count,
    std::size_t exploration_free_cells,
    std::size_t exploration_occupied_cells,
    std::size_t exploration_unknown_cells,
    bool has_odom,
 ...
 public:
  explicit DdsRuntime(int domain_id) {
    ...
    traversability_writer_ = writer(
        ...
        "traversability");
    exploration_grid_writer_ = writer(
        lingtu::message::kNavExplorationGrid.dds_topic.data(),
        &lingtu_dds_OccupancyGrid_desc,
        "exploration_grid");
    exploration_snapshot_writer_ = writer(
        lingtu::message::kNavExplorationSnapshot.dds_topic.data(),
        &lingtu_dds_ExplorationGrid_desc,
  ...
  void writeExplorationGrid(
      const map_layers::Grid2D& grid,
      const std::vector<std::uint8_t>& observed,
      double source_stamp_s) {
    OccupancyMessage msg = toExplorationMessage(grid, observed, source_stamp_s);
    logDdsError(
        dds_write(exploration_grid_writer_, &msg.msg),
        "dds_write(exploration_grid)");
  }

  void writeExplorationSnapshot(
      const map_layers::ExplorationGridSnapshot& snapshot,
      const std::string& session_id,
      std::uint64_t reset_epoch) {
    ExplorationGridMessage msg = toExplorationSnapshotMessage(
        snapshot, session_id, reset_epoch);
    logDdsError(
        dds_write(exploration_snapshot_writer_, &msg.msg),
...
int main(int argc, char** argv) {
  try {
    ...
    nav_endpoint::RollingExplorationMap rolling_exploration(
        rollingExplorationConfig(cfg));
    const std::string exploration_session_id =
    ...
    while (g_running) {
      ...
      dds.drainTf([&](const lingtu_dds_TFMessage& msg) {
        ...
        const auto stamp_decision = nav_endpoint::classifySourceOrder(
            map_odom ? map_odom->stamp_s : 0.0,
            next_map_odom->stamp_s,
        ...
        if (map_odom) {
          ...
          map_frame_jump =
              translation_jump > 0.50 || std::abs(correction.yaw) > 0.25;
        }
        if (map_frame_jump) {
          clear_runtime_maps();
          map_body_buffer.clear();
          ...
          ++map_frame_jump_clear_count;
            ...
            stamp_decision == nav_endpoint::SourceStampDecision::kClockRebase) {
          clear_source_epoch();
      ...
      dds.drainOdometry([&](const lingtu_dds_Odometry& msg) {
        ...
        const auto odom_stamp_decision = nav_endpoint::classifySourceOrder(
            odom_body ? odom_body->stamp_s : 0.0,
            next_odom_body.stamp_s,
            kSourceClockRebaseThresholdS);
        ...
        if (odom_stamp_decision ==
            nav_endpoint::SourceStampDecision::kClockRebase) {
          clear_source_epoch();
          last_frame_error = "source_clock_rebase";
        }
        odom_frame_id = frame;
      ...
      dds.drainCloud([&](const lingtu_dds_PointCloud2& msg) {
        ...
        if (last_publish > 0.0 &&
            schedule_now - last_publish < publish_period_s * 0.75) {
          return;
        }
          ...
          if (!xyz.empty()) {
            ...
            try {
              auto exploration = rolling_exploration.Update(
                  fast_xyz,
                  static_cast<float>(cloud_sensor_origin->x),
                  static_cast<float>(cloud_sensor_origin->y),
                  static_cast<float>(cloud_sensor_origin->z),
                  static_cast<float>(pose.z),
                  stampNanoseconds(cloud_stamp_s));
              if (terrain_risk_grid &&
                  map_layers::sameGeometry(
                      exploration.grid, *terrain_risk_grid)) {
                for (std::size_t cell = 0U;
                     cell < exploration.grid.data.size();
                     ++cell) {
                  if (exploration.observed[cell] != 0U) {
                    exploration.grid.data[cell] = std::max(
                        exploration.grid.data[cell],
                        terrain_risk_grid->data[cell]);
                  }
                }
              }
              const auto exploration_write_start =
                  SteadyClock::now();
              dds.writeExplorationGrid(
                  exploration.grid,
                  exploration.observed,
                  cloud_stamp_s);
              dds.writeExplorationSnapshot(
                  exploration,
                  exploration_session_id,
                  exploration_reset_epoch);
              timing.dds_write_ms +=
                  elapsedMs(exploration_write_start);
              exploration_free_cells = exploration.free_cells;
              exploration_occupied_cells =
                  exploration.occupied_cells;
              exploration_unknown_cells =
                  exploration.unknown_cells;
              ++exploration_snapshot_count;
            } catch (const std::exception& exc) {
              ++error_count;
              std::fprintf(
                  stderr,
                  "traversability_dds: exploration map update failed: %s\n",
                  exc.what());
            }
            ...
            if (schedule_now + slow_tolerance_s >= next_slow_publish) {
              ...
              const nav_kernel::TerrainResult terrain_result =
                  terrain_core.process(
                      ...
                      cloud_stamp_s);
              last_terrain_connected_cells = static_cast<std::size_t>(
                  std::max(0, terrain_result.connected_cells));
              last_terrain_ceiling_cells = static_cast<std::size_t>(
                  std::max(0, terrain_result.ceiling_cells));
      ...
      if (schedule_now >= next_status) {
        ...
        std::fprintf(
            stderr,
            "traversability_dds: odom=%llu registered_clouds=%llu published=%llu slow=%llu errors=%llu clock_rebases=%llu points=%zu timing(loop=%.2f input=%.2f convert=%.2f occ=%.2f slow=%.2f core=%.2f clear=%.2f pack=%.2f write=%.2f overrun=%.2f)\n",
            ...
            static_cast<unsigned long long>(clock_rebase_count),
            last_points,
            last_timing.loop_ms,
        ...
        writeStatus(
            ...
            exploration_reset_epoch,
            exploration_snapshot_count,
            exploration_free_cells,
            exploration_occupied_cells,
            exploration_unknown_cells,
            map_body.has_value(),
            map_odom.has_value(),
            last_map_odom_receive_s > 0.0
                ? schedule_now - last_map_odom_receive_s
                : -1.0,
