...
bool identityGridOrigin(const lingtu_dds_Quaternion& orientation) {
  ...
  return std::abs(normalized.x) <= kTolerance &&
      std::abs(normalized.y) <= kTolerance &&
...
std::optional<ExplorationSnapshot> parseExplorationSnapshot(
    const lingtu_dds_ExplorationGrid& message,
  ...
  if (config.max_grid_cells == 0U ||
      width > config.max_grid_cells / height) {
  ...
  if (cell_count > config.max_grid_cells ||
      message.data._buffer == nullptr ||
      message.data._length != cell_count) {
    reject(
        rejection_reason,
        cell_count > config.max_grid_cells
            ? "snapshot_grid_resource_limit"
...
bool snapshotFresh(
    const ExplorationSnapshot& snapshot,
    double now_s,
    const ExploreInputGateConfig& config,
    std::string* rejection_reason) {
  if (!sourceStampFresh(
