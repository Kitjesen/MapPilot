...
bool identityGridOrigin(const lingtu_dds_Quaternion &orientation) {
  const Quaternion value = quaternionFromDds(orientation);
  if (!quaternionIsFiniteAndNonzero(value)) {
    return false;
  }
...
parseExplorationSnapshot(const lingtu_dds_ExplorationGrid &message,
  ...
  snapshot.identity.map_content_epoch = message.map_content_epoch;
  ...
  const Route route = snapshot.identity.live ? Route::Live : Route::Map;
  if (!isCanonicalExploreRouteMapIdentity(route, snapshot.identity.map_id,
                                          snapshot.identity.map_content_epoch)) {
    reject(rejection_reason, "snapshot_route_map_identity_invalid");
    return std::nullopt;
  }
...
std::optional<TimedMapPose> mapPoseFromOdometry(const lingtu_dds_Odometry &message,
  ...
  if (const char *frame_error = odometryFramePairError(message)) {
    reject(rejection_reason, frame_error);
    return std::nullopt;
  }
