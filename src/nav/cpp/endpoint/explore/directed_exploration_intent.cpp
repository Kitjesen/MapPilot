...
DirectedExplorationIntentStore::Set(const std::string &session_id,
  ...
  if (session_id.empty()) {
    result.reason = "directed_target_session_empty";
    return result;
  }
  if (!map.valid()) {
  ...
  current_ = DirectedExplorationIntent{
      {target_x, target_y, 0.0}, map, session_id, revision, now_s + ttl_s,
  };
...
DirectedExplorationIntentStore::current(const std::string &session_id,
                                        const lingtu::explore::ExploreMapIdentity &map,
                                        double now_s) const {
  if (!current_.has_value() || !std::isfinite(now_s) || now_s >= current_->expires_at_s ||
      session_id != current_->session_id || !sameMapEpoch(map, current_->map)) {
    return std::nullopt;
  }
  return current_;
}
