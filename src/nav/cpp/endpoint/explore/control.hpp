...
class ExploreControl final {
 public:
  ...
  [[nodiscard]] const std::string &product_session_id() const noexcept { return product_session_id_; }
 ...
 private:
  struct AckRecord {
    ...
    std::string request_product_session_id;
    bool request_has_directed_target{false};
    double request_directed_target_x{0.0};
    double request_directed_target_y{0.0};
    double request_directed_target_ttl_s{0.0};
    bool accepted{false};
    std::string reason;
    std::string product_session_id;
    std::uint64_t intent_revision{0U};
  ...
  std::string last_start_product_session_id_;
