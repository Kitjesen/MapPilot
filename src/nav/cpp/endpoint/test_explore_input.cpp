...
lingtu_dds_ExplorationGrid makeSnapshot(
...
lingtu_dds_Odometry makeOdometry(char* frame, double stamp_s) {
  ...
  message.header.stamp.nanosec =
      static_cast<std::uint32_t>((stamp_s - std::floor(stamp_s)) * 1e9);
  message.header.frame_id = frame;
...
void testValidSnapshot() {
  ...
  std::string reason;
  const auto parsed = lingtu::nav::endpoint::parseExplorationSnapshot(
      message, {}, &reason);
...
void testStrictTrinaryAndIdentity() {
  ...
  auto message = makeSnapshot(data, frame, session);
  std::string reason;
  assert(!lingtu::nav::endpoint::parseExplorationSnapshot(
      message, {}, &reason));
...
void testMapFrameOdometry() {
  ...
  std::string reason;
  const auto pose = lingtu::nav::endpoint::mapPoseFromOdometry(
      message, std::nullopt, -1.0, 100.1, {}, &reason);
...
void testOdomFrameRequiresFreshTransform() {
  ...
  std::string reason;
  assert(!lingtu::nav::endpoint::mapPoseFromOdometry(
      message, std::nullopt, -1.0, 100.1, {}, &reason));
