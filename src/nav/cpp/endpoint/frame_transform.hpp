...
inline bool quaternionIsFiniteAndNonzero(const lingtu_dds_Quaternion &q) {
  ...
  return std::isfinite(q.x) && std::isfinite(q.y) && std::isfinite(q.z) && std::isfinite(q.w) &&
         std::isfinite(norm_squared) && norm_squared > 1e-24;
}
...
inline RigidTransform rigidTransformFromOdometry(const lingtu_dds_Odometry &message) {
  ...
  const bool rotation_valid = quaternionIsFiniteAndNonzero(message.pose.pose.orientation);
...
inline lingtu_dds_Quaternion multiplyQuaternions(const lingtu_dds_Quaternion &raw_lhs,
...
inline nav_kernel::Vec3 rotatePoint(const lingtu_dds_Quaternion &raw_q,
  ...
  return {
      2.0 * dot * u.x + (s * s - (u.x * u.x + u.y * u.y + u.z * u.z)) * point.x + 2.0 * s * cross.x,
      2.0 * dot * u.y + (s * s - (u.x * u.x + u.y * u.y + u.z * u.z)) * point.y + 2.0 * s * cross.y,
      2.0 * dot * u.z + (s * s - (u.x * u.x + u.y * u.y + u.z * u.z)) * point.z + 2.0 * s * cross.z,
  };
}

inline nav_kernel::Vec3 transformPoint(const RigidTransform &transform,
                                       const nav_kernel::Vec3 &point) {
...
inline RigidTransform composeTransforms(const RigidTransform &parent_middle,
...
inline bool transformJumpExceeds(const RigidTransform &previous, const RigidTransform &next,
  ...
  const double translation_jump_m = std::sqrt(correction.translation.x * correction.translation.x +
...
inline nav_kernel::Pose transformPose(const RigidTransform &transform,
...
inline std::optional<RigidTransform> mapOdomTransformFromTf(const lingtu_dds_TFMessage &message) {
  ...
  for (std::uint32_t index = 0; index < message.transforms._length; ++index) {
    ...
    const std::string child =
        stamped.child_frame_id == nullptr ? std::string{} : std::string(stamped.child_frame_id);
    if (!((parent == "map" && child == "odom") || (parent == "odom" && child == "map"))) {
      continue;
    }
    RigidTransform transform;
    transform.translation = {
