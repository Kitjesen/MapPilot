...
bool writeSnapshotFile(const std::filesystem::path &path, const std::string &snapshot) {
  ...
  if (!lingtu::native::replaceSnapshotFile(temporary, path, &replace_error)) {
...
struct StatusSnapshotFileWriter::Impl {
  ...
  void submit(std::string snapshot) {
    submitFactory([snapshot = std::move(snapshot)]() { return snapshot; });
  }

  void submitFactory(SnapshotFactory factory) {
    if (!worker.joinable() || !factory)
      return;
    {
      std::lock_guard<std::mutex> lock(mutex);
      ++stats.submitted;
  ...
  void run() noexcept {
    ...
    while (true) {
      ...
      SnapshotFactory factory = std::move(*pending);
      pending.reset();
      stats.pending = false;
      stats.writing = true;
      lock.unlock();
  ...
  std::optional<SnapshotFactory> pending;
...
void StatusSnapshotFileWriter::submitFactory(SnapshotFactory factory) {
  impl_->submitFactory(std::move(factory));
}
