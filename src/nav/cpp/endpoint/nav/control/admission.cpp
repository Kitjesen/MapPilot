...
std::string stampError(const TeleopAdmissionRequest &request, const TeleopAdmissionContext &context,
  ...
  if (!std::isfinite(context.request_not_before_s)) {
    return "teleop_resume_boundary_invalid";
  }
  if (context.request_not_before_s > 0.0 &&
      request.source_stamp_s <= context.request_not_before_s) {
    return "teleop_source_predates_resume";
  }
  return {};
}
