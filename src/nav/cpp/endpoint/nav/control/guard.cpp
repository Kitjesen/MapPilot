...
ControlLoopRuntimeGuard::ControlLoopRuntimeGuard(ControlLoopRuntimeGuardConfig config)
  ...
  if (config_.latch_miss_streak == 0) {
    throw std::invalid_argument("latch miss streak must be positive");
  }
}
...
ControlLoopRuntimeGuard::observe(const ControlLoopHealthSnapshot &health) {
  ...
  const bool rolling_warning =
      reason == "p95_utilization_high" || reason == "deadline_miss_ratio_high";
  if (rolling_warning) {
    // Rolling windows describe capacity and recent history. They remain visible,
    // but only a current deadline streak may create a motion latch.
    unhealthy_samples_ = 0;
    reason_ = reason;
    if (state_ == ControlLoopRuntimeGuardState::kMonitoring ||
        state_ == ControlLoopRuntimeGuardState::kSuspect) {
      state_ = ControlLoopRuntimeGuardState::kMonitoring;
      recovery_samples_ = 0;
    } else if (state_ == ControlLoopRuntimeGuardState::kLatched) {
      if (health.current_miss_streak == 0) {
        ++recovery_samples_;
        if (recovery_samples_ >= config_.recovery_confirmation_samples) {
          if (config_.auto_resume) {
            state_ = ControlLoopRuntimeGuardState::kMonitoring;
  ...
  const bool deadline_streak = reason == "consecutive_deadline_misses";
  if (deadline_streak && health.current_miss_streak < config_.latch_miss_streak) {
    // A short scheduling stall is observable but recoverable. The driver
    // watchdog owns immediate stale-command stopping; only a sustained loop
    // failure creates the operator-resume boundary.
    unhealthy_samples_ = 0;
    recovery_samples_ = 0;
    reason_ = reason;
    if (state_ == ControlLoopRuntimeGuardState::kMonitoring ||
  ...
  ++unhealthy_samples_;
  if (deadline_streak || unhealthy_samples_ >= config_.unhealthy_confirmation_samples) {
