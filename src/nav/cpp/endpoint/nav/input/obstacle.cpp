...
constexpr double kPredictionStepS = 0.25;
constexpr std::size_t kMaxPredictedClusters = 32;
...
std::size_t appendPredictedObstacleCloud(std::vector<float> &out,
  ...
  const std::size_t prediction_points =
      std::min(max_prediction_points, predicted_xyzh.size() / 4U);
  out.reserve(out.size() + prediction_points * 4U);
  std::size_t appended = 0U;
  for (std::size_t index = 0U; index < prediction_points; ++index) {
    const std::size_t base = index * 4U;
    if (!std::isfinite(predicted_xyzh[base]) ||
        !std::isfinite(predicted_xyzh[base + 1U]) ||
        !std::isfinite(predicted_xyzh[base + 2U]) ||
        !std::isfinite(predicted_xyzh[base + 3U])) {
      continue;
    }
    out.insert(out.end(), predicted_xyzh.begin() + static_cast<std::ptrdiff_t>(base),
               predicted_xyzh.begin() + static_cast<std::ptrdiff_t>(base + 4U));
    ++appended;
  }
  return appended;
}
...
std::vector<float> MotionLayer::snapshot(std::size_t max_points, double now_s) {
  ...
  snapshot(out, max_points, now_s);
  return out;
}

void MotionLayer::snapshot(std::vector<float> &out, std::size_t max_points, double now_s) {
  prune(now_s);
  out.clear();
  if (cells_.empty()) {
    return;
  }
  auto &output_cells = output_cells_scratch_;
  output_cells.clear();
  ...
  if (inflation_steps == 0) {
    auto &obstacles = obstacle_scratch_;
    obstacles.clear();
  ...
  if (max_points > 0 && output_cells.size() > max_points) {
    auto &reduced = reduced_cells_scratch_;
    reduced.clear();
    ...
    for (int attempt = 0; attempt < 8; ++attempt) {
      ...
      for (const auto &entry : output_cells) {
        ...
        if (!inserted) {
          const double candidate_distance = distance_squared(&cell);
          const double retained_distance = distance_squared(&it->second);
          if (candidate_distance < retained_distance ||
              (candidate_distance == retained_distance && cell.height > it->second.height)) {
            it->second = cell;
          }
        }
      }
...
std::vector<float> MotionLayer::snapshotPredictedDynamic(std::size_t max_points, double now_s) {
  std::vector<float> predicted;
  snapshotPredictedDynamic(predicted, max_points, now_s);
  return predicted;
}
...
void MotionLayer::snapshotPredictedDynamic(std::vector<float> &predicted, std::size_t max_points,
                                           double now_s) {
  predicted.clear();
  const std::size_t prediction_budget = max_points == 0
                                            ? kMaxDynamicPredictionPoints
                                            : std::min(kMaxDynamicPredictionPoints, max_points);
  if (prediction_budget == 0) {
    stats_.predicted_points = 0;
    stats_.prediction_clusters = 0;
    stats_.prediction_horizon_s = kDynamicPredictionHorizonS;
    return;
  }

  const auto clusters = dynamicClusters(kMaxPredictedClusters, now_s);
  predicted.reserve(std::min(prediction_budget, clusters.size() * 20) * 4);
  std::size_t added = 0;
  ...
  for (const auto &cluster : clusters) {
    ...
    if (!std::isfinite(speed) || speed < config_.dynamic_min_speed_mps ||
        !std::isfinite(cluster.radius_xy) || cluster.radius_xy < 0.0 ||
        !std::isfinite(cluster.height) || cluster.height < config_.dynamic_min_height_m ||
        cluster.height > config_.dynamic_max_height_m) {
      continue;
    }
    for (double dt = kPredictionStepS; dt <= kDynamicPredictionHorizonS + 1e-9;
         dt += kPredictionStepS) {
      const float x = static_cast<float>(cluster.x + cluster.vx * dt);
      const float y = static_cast<float>(cluster.y + cluster.vy * dt);
      const float z = static_cast<float>(cluster.z + cluster.vz * dt);
      const float extent = static_cast<float>(cluster.radius_xy);
      const float points[5][2] = {
          {x, y}, {x + extent, y}, {x - extent, y}, {x, y + extent}, {x, y - extent},
      };
      bool cluster_added = false;
      for (const auto &point : points) {
        if (added >= prediction_budget) {
          budget_full = true;
          break;
        }
        predicted.push_back(point[0]);
        predicted.push_back(point[1]);
        predicted.push_back(z);
        predicted.push_back(cluster.height);
        ++added;
        cluster_added = true;
      }
      if (cluster_added && dt == kPredictionStepS) {
        ++predicted_clusters;
      }
      if (budget_full) {
        break;
  ...
  stats_.predicted_points = added;
  stats_.prediction_clusters = predicted_clusters;
  stats_.prediction_horizon_s = kDynamicPredictionHorizonS;
}
...
std::vector<DynamicCluster> MotionLayer::dynamicClusters(std::size_t max_clusters, double now_s) {
  ...
  auto &candidate_cells = candidate_cells_scratch_;
  ...
  auto &visited = visited_scratch_;
  visited.clear();
  visited.reserve(candidate_cells.size());
  auto &queue = cluster_queue_scratch_;
  queue.clear();
  for (const auto &entry : candidate_cells) {
    ...
    for (const auto &key : candidate.keys) {
      ...
      const Cell &cell = *cell_it->second;
      candidate.cluster.radius_xy =
          std::max(candidate.cluster.radius_xy,
  ...
  for (std::size_t i = 0; i < cluster_tracks_.size(); ++i) {
    ...
    const auto &track = cluster_tracks_[i];
    const double unseen_s = std::max(0.0, current_stamp_s_ - track.last_seen_s);
    if (unseen_s <= config_.dynamic_track_ttl_s) {
      next_tracks.push_back(track);
      if (track.confirmed) {
        const double ttl = std::max(1e-6, config_.dynamic_track_ttl_s);
        const double confidence_scale = std::clamp(1.0 - unseen_s / ttl, 0.0, 1.0);
        last_dynamic_clusters_.push_back(DynamicCluster{
            track.id,
            track.x + track.vx * unseen_s,
            track.y + track.vy * unseen_s,
            track.z + track.vz * unseen_s,
            track.vx,
            track.vy,
            track.vz,
            track.age_s + unseen_s,
            track.confidence * confidence_scale,
            track.cells,
            track.radius_xy,
            track.height,
        });
...
void MotionLayer::clear() {
  cells_.clear();
  output_cells_scratch_.clear();
  reduced_cells_scratch_.clear();
  obstacle_scratch_.clear();
  current_hit_keys_.clear();
  free_keys_scratch_.clear();
  current_dynamic_keys_.clear();
  candidate_cells_scratch_.clear();
  visited_scratch_.clear();
  cluster_queue_scratch_.clear();
  cluster_tracks_.clear();
  last_dynamic_clusters_.clear();
