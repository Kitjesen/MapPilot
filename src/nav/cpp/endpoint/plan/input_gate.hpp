...
inline SourceStampDecision classifySourceOrder(double previous_stamp_s, double incoming_stamp_s,
  ...
  return rollback_s > std::max(0.0, clock_rebase_threshold_s) ? SourceStampDecision::kClockRebase
                                                              : SourceStampDecision::kReject;
}

inline SourceStampDecision classifySourceStamp(double previous_stamp_s, double incoming_stamp_s,
                                               double receive_now_s, double future_tolerance_s,
                                               double max_age_s) {
  if (!std::isfinite(incoming_stamp_s) || incoming_stamp_s <= 0.0 ||
  ...
  const bool previous_is_future = previous_age_s < -bounded_future_tolerance_s;
  const bool incoming_matches_current_epoch =
      incoming_age_s >= -bounded_future_tolerance_s &&
      (bounded_max_age_s <= 0.0 || incoming_age_s <= bounded_max_age_s);
  return previous_is_future && incoming_matches_current_epoch ? SourceStampDecision::kClockRebase
                                                              : SourceStampDecision::kReject;
}
...
class OdometrySpeedMonitor {
 public:
  ...
  double observe(double stamp_s, std::string_view frame_id, double x, double y, double z, double vx,
    ...
    } else if (sample_count_ > 0 && stamp_s - samples_[sample_count_ - 1].stamp_s <= 1e-6) {
      return std::numeric_limits<double>::infinity();
    }
    ...
    // Normal motion gating keeps DDS 3-D twist failures immediate. Stop
    // confirmation can select pose-only evidence after a matching driver ACK,
    // because estimator velocity bias must not override observed planar rest.
    // Geometry is derived from a causal five-sample
 ...
 private:
  ...
  static double planarDistance(const std::array<double, 3> &lhs, const std::array<double, 3> &rhs) {
...
class InputGate {
 public:
  ...
  InputGateState evaluate(const InputSnapshot &inputs) {
    ...
    state.odom_age_s =
        config_.require_odom
            ? age(inputs.now_s, receiveOrSource(inputs.odom_receive_s, inputs.odom_stamp_s))
            : -1.0;
    state.tf_age_s =
        config_.require_odom && inputs.odom_requires_tf
            ? age(inputs.now_s, receiveOrSource(inputs.tf_receive_s, inputs.tf_stamp_s))
            : -1.0;
    state.cloud_age_s =
        config_.require_cloud
            ? age(inputs.now_s, receiveOrSource(inputs.cloud_receive_s, inputs.cloud_stamp_s))
            : -1.0;
    state.traversability_age_s =
        config_.require_traversability
            ? age(inputs.now_s,
                  receiveOrSource(inputs.traversability_receive_s, inputs.traversability_stamp_s))
            : -1.0;
    ...
    state.driver_control_age_s =
        config_.require_driver_control
            ? age(inputs.now_s,
                  receiveOrSource(inputs.driver_control_receive_s, inputs.driver_control_stamp_s))
            : -1.0;
  ...
  InputGateState evaluate(double now_s, double last_odom_s, double last_tf_s, double last_cloud_s,
                          bool require_tf) {
 ...
 private:
  ...
  static bool advanced(std::uint64_t current, std::uint64_t previous) { return current > previous; }

  bool allRequiredInputsAdvanced(const InputSnapshot &inputs) const {
    if (config_.require_odom && !advanced(inputs.odom_generation, recovery_generations_.odom)) {
      return false;
    }
    ...
    if (config_.require_cloud && !advanced(inputs.cloud_generation, recovery_generations_.cloud)) {
      return false;
    }
    if (config_.require_traversability &&
        !advanced(inputs.traversability_generation, recovery_generations_.traversability)) {
      return false;
    }
    if (config_.require_localization_health &&
        !advanced(inputs.localization_health_generation,
                  recovery_generations_.localization_health)) {
      return false;
    }
    if (config_.require_driver_control &&
        !advanced(inputs.driver_control_generation, recovery_generations_.driver_control)) {
      return false;
    }
    return true;
  }
