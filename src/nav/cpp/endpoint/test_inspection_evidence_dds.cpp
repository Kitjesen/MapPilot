...
class EvidencePeer {
 public:
  ...
  bool takeRequest(lingtu_dds_InspectionEvidenceRequest* output) {
    return takeOne(
        request_reader_, output, lingtu_dds_InspectionEvidenceRequest_desc);
  }
  ...
  void writeResult(
      ...
      const std::string& analysis_verdict) {
...
inspection::Executor makeActionPendingExecutor() {
  ...
  std::string error;
  require(
      executor.Start(route, "run-1", "factory", 7, 0.0, &error),
      "inspection route must start");
  ...
  executor.Tick(1.61);
  require(
      executor.status().state == inspection::RunState::kActionPending,
...
void testEvidenceQosIsReliableAndBounded() {
  for (const auto* topic : {
    ...
    require(
        lingtu::dds::qos_for_topic(topic) ==
            lingtu::dds::QosProfile::InspectionEvidence,
    ...
    require(
        dds_qget_reliability(qos.get(), &reliability, &max_blocking_time),
        "evidence QoS must define reliability");
    require(
        reliability == DDS_RELIABILITY_RELIABLE,
        "evidence QoS must be reliable");
    dds_history_kind_t history{};
    int32_t depth = 0;
    require(
        dds_qget_history(qos.get(), &history, &depth),
        "evidence QoS must define history");
    require(
        history == DDS_HISTORY_KEEP_LAST && depth == 32,
        "evidence QoS must keep a bounded history");
    dds_duration_t deadline = 0;
    dds_duration_t lifespan = 0;
    require(
        dds_qget_deadline(qos.get(), &deadline) && deadline == DDS_SECS(5),
        "evidence QoS must have a 5 second deadline");
    require(
        dds_qget_lifespan(qos.get(), &lifespan) && lifespan == DDS_SECS(35),
...
void testRequestStatusAndMatchedResultHandshake() {
  ...
  require(
      !runtime.inspectionEvidenceWorkerMatched(),
  ...
  for (int attempt = 0; attempt < 100; ++attempt) {
    require(
        runtime.writeInspectionEvidenceRequest(
            *request,
            executor.route()->map_id,
            executor.route()->map_version,
            executor.status().deadline_s),
        "evidence request write must succeed");
    std::this_thread::sleep_for(10ms);
    if (peer.takeRequest(&request_wire)) break;
  }
  ...
  require(
      executor.OnActionStarted(request->request_id, 1.70),
  ...
  require(
      std::string(status_wire.action_request_id) == request->request_id,
  ...
  runtime.drainInspectionEvidenceResults(
      [&](const lingtu_dds_InspectionEvidenceResult& result) {
        require(
            !lingtu::nav::endpoint::applyInspectionEvidenceResult(
                executor, result, 1.80),
  ...
  runtime.drainInspectionEvidenceResults(
      [&](const lingtu_dds_InspectionEvidenceResult& result) {
        applied = lingtu::nav::endpoint::applyInspectionEvidenceResult(
            executor, result, 1.90) || applied;
      });
  require(applied, "matching persisted evidence result must advance inspection");
  require(
      executor.status().state == inspection::RunState::kSucceeded,
