...
Config parseArgs(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (arg == "--domain") {
      ...
      config.policy.max_frontier_cells =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-frontier-clusters") {
      config.policy.max_frontier_clusters =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-coverage-cells") {
      config.policy.max_coverage_cells =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-keyposes") {
      config.policy.max_keyposes =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-keypose-edges") {
      config.policy.max_keypose_edges =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-keypose-neighbor-links") {
      config.policy.max_keypose_neighbor_links =
          static_cast<std::size_t>(std::stoull(value()));
    } else if (arg == "--max-route-targets") {
      config.policy.max_route_targets =
          static_cast<std::size_t>(std::stoull(value()));
 ...
 public:
  explicit DdsRuntime(int domain_id) {
    ...
    tf_reader_ = reader(
        ...
        "tf");
    tf_static_reader_ = reader(
        lingtu::message::kTfStatic,
        &lingtu_dds_TFMessage_desc,
        "tf_static");
    snapshot_reader_ = reader(
        lingtu::message::kNavExplorationSnapshot,
        &lingtu_dds_ExplorationGrid_desc,
        "exploration_snapshot");
    control_reader_ = reader(
        lingtu::message::kNavExplorationCommand,
        &lingtu_dds_ExplorationCommandRequest_desc,
        "exploration_command");
    control_ack_writer_ = writer(
  ...
  void drainSnapshot(Handler&& handler) {
    drainReader<lingtu_dds_ExplorationGrid>(
        snapshot_reader_,
        lingtu_dds_ExplorationGrid_desc,
        std::forward<Handler>(handler));
  }
  ...
  void writeControlAck(
    ...
    logDdsError(
        dds_write(control_ack_writer_, static_cast<const void*>(&message)),
...
bool sameMapEpoch(
    const ExploreMapIdentity& left,
    const ExploreMapIdentity& right) {
  return left.sameSource(right) &&
...
std::string statusSnapshot(
    ...
    bool paused,
    const std::string& session_id,
    const std::optional<TimedMapPose>& robot,
    const std::optional<ExplorationSnapshot>& snapshot,
    const std::optional<PendingGoal>& pending,
    const std::optional<QueuedGoal>& queued,
    const std::optional<std::string>& cancel_reason,
    const ExploreDecision& decision,
    const Counters& counters,
    double now_s) {
  const double odometry_age_s =
      robot.has_value() ? now_s - robot->stamp_s : -1.0;
  const double snapshot_age_s =
      snapshot.has_value() ? now_s - snapshot->stamp_s : -1.0;
  ...
  output << "{\n"
         ...
         << "  \"ready\": "
         << (robot.has_value() && snapshot.has_value() ? "true" : "false")
         << ",\n"
  ...
  if (snapshot.has_value()) {
    ...
    output << "  \"map\": {\"frame_id\": \""
           << jsonEscape(map.frame_id) << "\", \"session_id\": \""
           << jsonEscape(map.session_id) << "\", \"map_id\": \""
           << jsonEscape(map.map_id) << "\", \"map_version\": "
           << map.map_version << ", \"artifact_hash\": \""
           << jsonEscape(map.artifact_hash) << "\", \"reset_epoch\": "
           << map.reset_epoch << ", \"generation\": " << map.generation
  ...
  if (pending.has_value()) {
    ...
    output << "  \"pending_goal\": {\"x\": " << queued->target.x
           << ", \"y\": " << queued->target.y
           << ", \"age_s\": -1.0},\n";
  } else {
  ...
  output << ",\n"
         << "  \"planner\": {\"phase\": \""
         << jsonEscape(diagnostics.phase)
         << "\", \"accepted_generation\": "
         << diagnostics.accepted_generation
         << ", \"reachable_free_cells\": "
         << diagnostics.reachable_free_cells
         ...
         << ", \"state_committed\": "
         << (diagnostics.state_committed ? "true" : "false") << "},\n"
         << "  \"counters\": {"
         ...
         << ", \"plan_rejected\": " << counters.plan_rejected
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    lingtu::nav::endpoint::StatusSnapshotFileWriter status_writer(
        config.status_file);
    ...
    std::fprintf(
        stderr,
        "lingtu_explore_dds: domain=%d tick_hz=%.2f input=%s command=%s\n",
        config.domain_id,
        config.tick_hz,
    ...
    while (g_running.load(std::memory_order_relaxed)) {
      ...
      auto acceptTf = [&](const lingtu_dds_TFMessage& message) {
        ...
        const auto transform =
            lingtu::nav::endpoint::mapOdomTransformFromTf(message);
      ...
      dds.drainSnapshot([&](const lingtu_dds_ExplorationGrid& message) {
        ...
        std::string reason;
        auto parsed = lingtu::nav::endpoint::parseExplorationSnapshot(
            message, config.input_gate, &reason);
        if (!parsed.has_value() ||
            !lingtu::nav::endpoint::snapshotFresh(
        ...
        const bool map_epoch_changed =
            snapshot.has_value() &&
            !sameMapEpoch(snapshot->identity, parsed->identity);
      ...
      const bool robot_fresh =
          robot.has_value() &&
          lingtu::nav::endpoint::sourceStampFresh(
              robot->stamp_s,
              loop_now_s,
      ...
      dds.drainControl([&](const lingtu_dds_ExplorationCommandRequest& message) {
        ...
        request.goal_pending = pending.has_value();
        request.cancellation_pending = cancel_reason.has_value();

        const auto result = exploration_control.Apply(request);
        ...
        if (result.request_cancel && pending.has_value()) {
          cancel_reason = result.cancel_reason.empty()
              ? "exploration_control_cancel"
      ...
      if (pending.has_value() && robot_fresh && snapshot_fresh &&
        ...
        const double distance = std::hypot(
            pending->target.x - robot->pose.x,
            pending->target.y - robot->pose.y);
        if (distance <= config.arrival_tolerance_m) {
          visited.push_back(pending->target);
      ...
      if (pending.has_value() && !cancel_reason.has_value()) {
        ...
        } else if (loop_now_s - pending->accepted_s >
      ...
      if (pending.has_value() && cancel_reason.has_value() &&
          (last_command_attempt_s < 0.0 ||
           loop_now_s - last_command_attempt_s >=
               config.command_retry_s)) {
        last_command_attempt_s = loop_now_s;
        try {
          commands.navigation().cancel(
              *cancel_reason,
          ...
          last_reason =
              std::string("exploration_cancel_rejected:") + error.what();
        }
      }

      if (exploration_control.running() &&
          !pending.has_value() && !cancel_reason.has_value() &&
          queued.has_value() && snapshot_fresh && robot_fresh) {
        if (!sameMapEpoch(queued->map, snapshot->identity)) {
          ...
          try {
            ...
            last_reason =
                std::string("exploration_goal_rejected:") + error.what();
          }
        }
      }

      if (exploration_control.running() &&
          !pending.has_value() && !queued.has_value() &&
        ...
        const bool already_committed =
            previous.state_committed &&
            previous.session_id == snapshot->identity.session_id &&
        ...
        if (!already_committed) {
          ...
          try {
            ...
            ++counters.plan_rejected;
            last_reason =
                std::string("exploration_plan_exception:") + error.what();
          }
        }
      }
      ...
      if (cancel_reason.has_value()) {
        ...
        last_reason = freshness_reason.empty()
            ? "exploration_snapshot_missing"
            : freshness_reason;
      ...
      if (loop_now_s >= next_status_s) {
        ...
        status_writer.submit(statusSnapshot(
            ...
            runtime_state,
            last_reason,
            exploration_control.active(),
            exploration_control.paused(),
            exploration_control.session_id(),
            robot_fresh ? robot : std::nullopt,
            snapshot_fresh ? snapshot : std::nullopt,
            pending,
            queued,
            cancel_reason,
            last_decision,
    ...
    if (pending.has_value()) {
      try {
        ...
        std::fprintf(
            stderr,
            "explore_dds: shutdown cancel rejected: %s\n",
            error.what());
      }
    }
    status_writer.flush();
