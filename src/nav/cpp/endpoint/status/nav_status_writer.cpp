...
void writeLocalMapJson(std::ostream &out, std::size_t point_limit,
                       const std::vector<float> &obstacle_xyzh,
                       const TraversabilityGrid &traversability, const SensorOrigin &sensor_origin,
                       bool obstacle_points_fresh, bool traversability_fresh) {
...
void writeStatusSnapshot(
  ...
  const bool final_output_published =
  ...
  const bool final_output_driver_acknowledged =
      final_output_published && driver_control_fresh && driver_control.last_command_accepted &&
  ...
  const StatusSnapshotWriterDiagnostics snapshot_diagnostics = snapshot_writer.diagnostics();
  ...
  out << ",\n"
      << "  \"final_output\": {\"published\": " << (final_output_published ? "true" : "false")
      << ", \"producer_boot_id\": \"" << jsonEscape(final_output.producer_boot_id)
      << "\", \"output_sequence\": " << final_output.output_sequence
      << ", \"driver_acknowledged\": " << (final_output_driver_acknowledged ? "true" : "false")
      << "},\n"
      ...
      << "\"terminal_goal_z_tolerance_m\": " << cfg.octoplanner_options.terminal_goal_z_tolerance_m
      << "},\n"
      << "  \"far\": {"
      ...
      << "  \"live_obstacle_ray_clearing\": " << (cfg.live_obstacle_ray_clearing ? "true" : "false")
      << ",\n"
      ...
      << (std::isfinite(input_gate.localization_health_age_s) ? input_gate.localization_health_age_s
                                                              : -1.0)
      << ", "
  ...
  out << ",\n"
      << "  \"local_map\": ";
  writeLocalMapJson(out, cfg.local_map_debug_point_limit, local_map_obstacle_xyzh,
                    local_map_traversability, last_sensor_origin, input_gate.ready,
                    has_traversability);
  out << ",\n"
      << "  \"last_plan\": {\n"
