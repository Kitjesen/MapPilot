...
class InspectionStatusFileWriter {
 public:
  explicit InspectionStatusFileWriter(std::filesystem::path map_root);
  InspectionStatusFileWriter(std::filesystem::path map_root, StatusSnapshotFileWriter::Sink sink);
