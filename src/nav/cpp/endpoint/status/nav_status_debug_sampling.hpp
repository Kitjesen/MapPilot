...
std::vector<std::size_t> sampleDebugIndices(const std::vector<std::size_t> &candidates,
                                            std::size_t limit, bool prioritize_origin,
  ...
  if (prioritize_origin) {
    ...
    std::partial_sort(sampled.begin(), sampled.begin() + static_cast<std::ptrdiff_t>(limit),
                      sampled.end(), compare);
  ...
  for (std::size_t index = 0; index < limit; ++index) {
    const std::size_t source = limit <= 1 ? 0 : index * (candidates.size() - 1) / (limit - 1);
