...
bool VelocitySmoother::CommitApplied(const Twist &applied, double timestamp_s,
                                     std::string *error) {
  if (!configured_) {
    SetError(error, "not_configured");
    return false;
  }
  if (!Finite(applied) || !Finite(timestamp_s) || timestamp_s < 0.0) {
    SetError(error, "applied_invalid");
    return false;
  }
  if (has_last_step_ && timestamp_s < last_step_s_) {
    SetError(error, "applied_out_of_order");
    return false;
  }
  if (applied.vx < config_.x.minimum || applied.vx > config_.x.maximum ||
      applied.vy < config_.y.minimum || applied.vy > config_.y.maximum ||
      applied.wz < config_.yaw.minimum || applied.wz > config_.yaw.maximum) {
    SetError(error, "applied_out_of_bounds");
    return false;
  }
  last_output_ = applied;
  last_step_s_ = timestamp_s;
  has_last_step_ = true;
  SetError(error, std::string{});
  return true;
}
...
VelocitySmootherOutput VelocitySmoother::Step(double now_s) {
  ...
  if (!has_last_step_) {
    ...
    has_last_step_ = true;
    output.command = {
        ApplyDeadband(base.vx, config_.x),
        ApplyDeadband(base.vy, config_.y),
        ApplyDeadband(base.wz, config_.yaw),
    };
    output.valid = true;
  ...
  if (now_s == last_step_s_) {
    // No time advanced, so no feedback was actually consumed this step.
    output.command = {
        ApplyDeadband(last_output_.vx, config_.x),
        ApplyDeadband(last_output_.vy, config_.y),
        ApplyDeadband(last_output_.wz, config_.yaw),
    };
    output.valid = true;
    output.feedback_used = false;
    output.reason = "no_time_advance";
    return output;
  }

  const double elapsed_s = now_s - last_step_s_;
  output.dt_clamped = elapsed_s > config_.max_step_s;
  const double dt = std::min(elapsed_s, config_.max_step_s);
  ...
  candidate.vx = Clamp(candidate.vx, config_.x);
  candidate.vy = Clamp(candidate.vy, config_.y);
  candidate.wz = Clamp(candidate.wz, config_.yaw);

  output.command = {
      ApplyDeadband(candidate.vx, config_.x),
      ApplyDeadband(candidate.vy, config_.y),
      ApplyDeadband(candidate.wz, config_.yaw),
  };
  output.valid = true;
  output.limited = Different(output.command.vx, target_.vx) ||
                   Different(output.command.vy, target_.vy) ||
                   Different(output.command.wz, target_.wz);
