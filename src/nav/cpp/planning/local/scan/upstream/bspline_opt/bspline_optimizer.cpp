...
void BsplineOptimizer::setParam(const BsplineOptimizerParams &params) {
  lambda1_ = params.lambda_smooth;
  lambda2_ = params.lambda_collision;
  lambda3_ = params.lambda_feasibility;
  lambda4_ = params.lambda_fitness;
...
double BsplineOptimizer::estimateSegmentYaw(const Eigen::Vector3d &from,
 ...
 * It was written separately, just because I did it once and it has been running stably since March
 * 2020. But I will merge then someday.*/
...
BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/) {
  ...
  for (int i = order_; i <= i_end; ++i) {
    for (double a = 1.0; a >= 0.0; a -= step_size) {
      ...
      // cout << setprecision(5);
      // cout << (a * init_points.col(i-1) + (1-a) * init_points.col(i)).transpose() << " occ1=" <<
      // occ << endl;
  ...
  for (size_t i = 0; i < segment_ids.size(); ++i) {
    ...
    if (ret == AStarResult::Success) {
      vector<Eigen::Vector3d> path = a_star_->path();
      if (path.size() < 2) {
        return a_star_paths;
      }
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    // cout << "i = " << i << " first = " << segment_ids[i].first << " second = " <<
    // segment_ids[i].second << endl;
    ...
    // cout << "final:" << "i = " << i << " first = " << final_segment_ids[i].first << " second = "
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
      ...
      int Astar_id = a_star_paths[i].size() / 2,
          last_Astar_id;  // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better,
                          // but it needs more computation
      double val = (a_star_paths[i][Astar_id] - cps_.points.col(j)).dot(ctrl_pts_law),
             last_val = val;
    ...
    /* Corner case: the segment length is too short. Here the control points may outside the A*
     * path, leading to opposite gradient direction. So I have to take special care of it */
    if (segment_ids[i].second - segment_ids[i].first == 1) {
      ...
      Eigen::Vector3d middle_point =
          (cps_.points.col(segment_ids[i].second) + cps_.points.col(segment_ids[i].first)) / 2;
      int Astar_id = a_star_paths[i].size() / 2,
          last_Astar_id;  // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better,
                          // but it needs more computation
...
int BsplineOptimizer::earlyExit(void *func_data, const double *x, const double *g, const double fx,
                                const double xnorm, const double gnorm, const double step, int n,
                                int k, int ls) {
...
double BsplineOptimizer::costFunctionRebound(void *func_data, const double *x, double *grad,
                                             const int n) {
  BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);
...
double BsplineOptimizer::costFunctionRefine(void *func_data, const double *x, double *grad,
                                            const int n) {
  BsplineOptimizer *opt = reinterpret_cast<BsplineOptimizer *>(func_data);
...
void BsplineOptimizer::calcDistanceCostRebound(const Eigen::MatrixXd &q, double &cost,
  ...
  if (iter_num > 3 &&
      smoothness_cost / (cps_.size - 2 * order_) <
...
void BsplineOptimizer::calcFitnessCost(const Eigen::MatrixXd &q, double &cost,
    ...
    for (int j = 0; j < 3; j++) {
      if (vi(j) > max_vel_ + demarcation) {
        ...
        cost += (ar * diff * diff + br * diff + cr) *
                ts_inv3;  // multiply ts_inv3 to make vel and acc has similar magnitude
    ...
    for (int j = 0; j < 3; j++) {
      if (vi(j) > max_vel_) {
        ...
        cost += pow(vi(j) - max_vel_, 2) *
                ts_inv2;  // multiply ts_inv3 to make vel and acc has similar magnitude
    ...
    if (occ) {
      ...
      for (j = i - 1; j >= 0; --j) {
        occ = grid_map_->getInflateOccupancy(cps_.points.col(j),
                                             estimateControlPointYaw(cps_.points, j));
        if (!occ) {
          in_id = j;
          break;
        }
  ...
  if (flag_new_obs_valid) {
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i) {
      ...
      if (ret == AStarResult::Success) {
        ...
        a_star_paths.push_back(path);
      } else if (ret == AStarResult::SearchError && i + 1 < segment_ids.size()) {
...
void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine,
                                          const int n) {
  ...
  Eigen::MatrixXd grad_3D =
      lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility;
  grad_3D.row(2).setZero();
  memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
}

void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine,
                                         const int n) {
  memcpy(cps_.points.data() + 3 * order_, x, n * sizeof(x[0]));
