...
using lingtu::nav::plan::far::FarDiagnostics;
...
int32_t lingtu_nav_far_update_map(
  ...
  try {
    ...
    const bool has_map_version = map->map_version > 0;
    const bool has_artifact_hash = !EmptyOrNull(map->artifact_sha256);
    const bool has_any_identity = has_map_id || map->map_version != 0 || has_artifact_hash;
    if (has_any_identity && !(has_map_id && has_map_version && has_artifact_hash)) {
      throw std::invalid_argument(
          "FAR map identity requires map_id, positive map_version, and artifact_sha256 together");
    }
    if (has_any_identity) {
      value.identity.map_id = map->map_id;
      value.identity.version = map->map_version;
      value.identity.artifact_sha256 = map->artifact_sha256;
