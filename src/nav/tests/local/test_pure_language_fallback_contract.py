...
LOCAL_PLANNER_SERVICE = (
    ROOT / "src" / "nav" / "services" / "plan" / "local_planner" / "service.py"
)
...
def test_base_autonomy_readme_does_not_advertise_ros2_module_backends() -> None:
    ...
    readme = (ROOT / "src" / "nav" / "local" / "README.md").read_text(
        encoding="utf-8"
    )
...
def test_legacy_ros2_local_autonomy_packages_have_been_removed() -> None:
    ...
    Per ``docs/architecture/ROS2_DECOUPLING_MIGRATION_PLAN.md`` Phase 1: these
    four packages were confirmed unused by every production profile (they were
    ...
    still_present = [
        ...
        for package_dir in LEGACY_ROS2_PACKAGE_DIRS
