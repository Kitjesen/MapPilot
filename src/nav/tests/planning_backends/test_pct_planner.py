"""Tests for the PCT planner without native libraries."""
...
@pytest.fixture
def pct_backend():
    """PCT planner with no tomogram (unavailable, tests graceful degradation)."""
    from nav.services.plan.global_planner.algorithm.pct.planner import PCTPlanner
    backend = PCTPlanner(tomogram_path="")
...
class TestPCTBackend:
    """PCT planner tests (native .so unavailable, degrades gracefully)."""

    def test_instantiation_without_tomogram(self, pct_backend):
        """PCT planner created without tomogram is marked unavailable."""
