"""Mapless direct-path planner for lightweight runtimes."""
...
@register(
    "planner_backend",
    "direct",
    description="Mapless direct-path planner for the local lite Profile runtime",
)
class DirectPathBackend:
    ...
    def update_map(self, *_args, **_kwargs) -> None:
        """Accept live-map updates without making the `lite` Profile depend on map modules."""
