...
class MapSaveAdapterFailed(Exception):
    """Adapter failures already shaped for MapService API responses."""


class MapRuntimeBridge:
    """Owns live runtime inputs used by MapService.
    ...
    def on_map_cloud_frame(self, frame: MapCloudFrame | dict[str, Any]) -> None:
        ...
        try:
            map_frame = (
                MapCloudFrame.from_dict(frame) if isinstance(frame, dict) else frame
            )
    ...
    def write_binary_xyz_pcd(path: Path, points: np.ndarray) -> int:
        ...
        with path.open("wb") as fh:
            ...
            fh.write(pts.astype("<f4", copy=False).tobytes())
        return int(len(pts))
