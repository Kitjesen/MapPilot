...
// Source timestamps and accepted-sample generations are part of the safety
// contract. Callers must copy timestamps from the DDS message Header (or the
// health payload's `ts` field) and increment a generation only after accepting
// a new sample from that DDS input.
...
class InputGate {
 public:
  ...
  InputGateState evaluate(const InputSnapshot& inputs) {
    ...
    state.odom_age_s =
        config_.require_odom ? age(inputs.now_s, inputs.odom_stamp_s) : -1.0;
    state.tf_age_s = config_.require_odom && inputs.odom_requires_tf
        ? age(inputs.now_s, inputs.tf_stamp_s)
        : -1.0;
    state.cloud_age_s =
        config_.require_cloud ? age(inputs.now_s, inputs.cloud_stamp_s) : -1.0;
    state.traversability_age_s = config_.require_traversability
        ? age(inputs.now_s, inputs.traversability_stamp_s)
        : -1.0;
    state.localization_health_age_s = config_.require_localization_health
        ? age(inputs.now_s, inputs.localization_health_stamp_s)
        : -1.0;
    state.driver_control_age_s = config_.require_driver_control
        ? age(inputs.now_s, inputs.driver_control_stamp_s)
        : -1.0;
