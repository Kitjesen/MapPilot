...
inline std::optional<TraversabilityPose> traversabilityPose(
  ...
  if (!std::isfinite(pose.roll) || !std::isfinite(pose.pitch) ||
...
inline std::optional<nav_kernel::Vec3> traversabilitySensorOrigin(
    const TraversabilityPose& pose,
    const nav_kernel::Vec3& sensor_offset) {
  if (!std::isfinite(sensor_offset.x) || !std::isfinite(sensor_offset.y) ||
      !std::isfinite(sensor_offset.z) ||
      !quaternionIsFiniteAndNonzero(pose.rotation)) {
    return std::nullopt;
  }
