...
inline double snappedSafetyGridOrigin(
    double center,
    double radius,
    double resolution) {
  const double raw_origin = center - radius;
  if (!std::isfinite(raw_origin) || !std::isfinite(resolution) ||
      resolution <= 0.0) {
    return raw_origin;
  }
  const double scaled = std::nextafter(
      raw_origin / resolution,
      std::numeric_limits<double>::infinity());
  return std::floor(scaled) * resolution;
}

inline double safetyGridAlignmentResidual(
    double origin,
    double resolution) {
  if (!std::isfinite(origin) || !std::isfinite(resolution) ||
      resolution <= 0.0) {
    return -1.0;
  }
...
inline lingtu::maps::layers::Grid2D makeUnknownSafetyGrid(
    int rows,
    int cols,
    double resolution,
    double origin_x,
  ...
  return lingtu::maps::layers::makeGrid2D(
      rows,
      cols,
      resolution,
      origin_x,
...
inline bool markObservedRayFree(
    lingtu::maps::layers::Grid2D& grid,
    double origin_x,
    double origin_y,
    double hit_x,
  ...
  const double grid_diagonal = std::hypot(
      static_cast<double>(grid.cols) * grid.resolution,
  ...
  const int steps = std::max(
      1,
      static_cast<int>(std::ceil(traversed_distance / sample_step)));
  bool observed = false;
  for (int step = 0; step <= steps; ++step) {
    const double ray_distance = std::min(
        distance,
        static_cast<double>(step) * traversed_distance / static_cast<double>(steps));
