...
void MotionLayer::configure(MotionLayerConfig config) {
  ...
  config.static_free_min_frames =
      std::max(config.free_min_frames, config.static_free_min_frames);
  config.dynamic_min_cells = std::max<std::size_t>(1, config.dynamic_min_cells);
  config.dynamic_free_min_frames =
      std::max<std::uint32_t>(1, config.dynamic_free_min_frames);
  ...
  config.dynamic_max_height_m =
      std::max(config.dynamic_min_height_m, config.dynamic_max_height_m);
  config.dynamic_confirm_frames =
      std::max<std::uint32_t>(2, config.dynamic_confirm_frames);
...
MotionLayer::VoxelKey MotionLayer::makeKeyForSize(
    float x,
    float y,
    float z,
...
bool MotionLayer::isStale(const Cell& cell, double now_s) const {
  ...
  const double age_source =
      cell.state == CellState::Free || cell.state == CellState::Cleared
          ? cell.last_free_s
          : cell.last_hit_s;
...
void MotionLayer::markHit(const Cell& sample, double stamp_s) {
  ...
  if (cell.hits <= 0 || sample.height > cell.height ||
      cell.state == CellState::Free || cell.state == CellState::Cleared) {
    cell.x = sample.x;
    cell.y = sample.y;
    cell.z = sample.z;
  ...
  if (!inserted) {
    if (!sameFrame(cell.last_free_s, stamp_s)) {
      ...
      cell.free_observations =
          std::min<std::uint32_t>(cell.free_observations + 1, 1000000);
    }
...
void MotionLayer::collectRayFreeKeys(
...
void MotionLayer::update(const std::vector<float>& xyzh, double stamp_s) {
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    if (!std::isfinite(sample.x) || !std::isfinite(sample.y) ||
        !std::isfinite(sample.z) || !std::isfinite(sample.height)) {
      continue;
    }
...
void MotionLayer::updateFromScan(
    const SensorOrigin& origin,
  ...
  const std::size_t ray_stride =
      count > config_.max_clearing_rays
          ...
          : 1;
  const bool run_clearing =
      config_.ray_clearing_enabled && origin.valid &&
  ...
  if (run_clearing) {
    ...
    for (std::size_t i = 0; i < count; i += ray_stride) {
      ...
      if (!std::isfinite(endpoint.x) || !std::isfinite(endpoint.y) ||
          !std::isfinite(endpoint.z) || !std::isfinite(endpoint.height)) {
        continue;
      }
...
void MotionLayer::prune(double now_s) {
  ...
  if (last_prune_s_ >= 0.0 && now_s >= last_prune_s_ &&
      now_s - last_prune_s_ < kPruneIntervalS) {
    return;
  }
  last_prune_s_ = now_s;
  ++stats_.prune_passes;
...
std::vector<float> MotionLayer::snapshot(std::size_t max_points, double now_s) {
  ...
  if (inflation_steps == 0) {
    ...
    std::partial_sort(
        obstacles.begin(),
        obstacles.begin() + static_cast<std::ptrdiff_t>(max_points),
        obstacles.end(),
    ...
    for (int ix = -inflation_steps; ix <= inflation_steps; ++ix) {
      for (int iy = -inflation_steps; iy <= inflation_steps; ++iy) {
        const double offset = std::hypot(
            static_cast<double>(ix) * config_.voxel_size_m,
...
std::vector<DynamicCluster> MotionLayer::dynamicClusters(
  ...
  if (sameFrame(last_cluster_update_s_, current_stamp_s_)) {
    ...
    return std::vector<DynamicCluster>(
        last_dynamic_clusters_.begin(),
        last_dynamic_clusters_.begin() + static_cast<std::ptrdiff_t>(max_clusters));
  }
  ...
  for (const auto& key : current_hit_keys_) {
    ...
    if (cell.state == CellState::Occupied &&
        sameFrame(cell.last_hit_s, current_stamp_s_) &&
  ...
  for (auto& candidate : candidates) {
    ...
    ClusterTrack track;
    if (best_index < cluster_tracks_.size() &&
        best_distance <= config_.dynamic_match_distance_m) {
      ...
      const double direction_cos =
          previous_speed > 1e-6 && raw_speed > 1e-6
              ? (previous.vx * raw_vx + previous.vy * raw_vy) /
                    (previous_speed * raw_speed)
              : 1.0;
      ...
      track.moving_frames = plausible_motion
          ? previous.moving_frames + 1
          : 0;
      const std::uint32_t required_motion_frames = config_.dynamic_confirm_frames - 1;
      track.confirmed =
          track.observations >= config_.dynamic_confirm_frames &&
    ...
    if (track.confirmed) {
      cluster.confidence = std::min(
          1.0,
          0.5 * static_cast<double>(cluster.cells) /
