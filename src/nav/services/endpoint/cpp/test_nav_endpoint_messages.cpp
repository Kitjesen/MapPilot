...
void testTerrainExtDefaultZeroShareDoesNotEnterPlannerObstacles() {
  ...
  lingtu::nav::endpoint::buildPlannerObstacleCloud(
      out, {}, {}, false, ext_only, true, 100, {});
...
void testFreshTerrainExtEntersPlannerObstaclesWhenEnabled() {
  ...
  std::vector<float> out;
  lingtu::nav::endpoint::buildPlannerObstacleCloud(
      out, {}, {}, false, ext_only, true, 100, config);
  require(out.size() == ext_only.size(), "enabled fresh terrain_map_ext must enter planner obstacles");

  lingtu::nav::endpoint::buildPlannerObstacleCloud(
      out, {}, {}, false, ext_only, false, 100, config);
...
lingtu_dds_Header header(const char* frame, double stamp = 10.0) {
  ...
  out.stamp.nanosec = static_cast<std::uint32_t>(
      (stamp - static_cast<double>(out.stamp.sec)) * 1e9);
  return out;
}
...
void testCanonicalFrameDecoders() {
  ...
  require(
      std::abs(goal.value.position.x - 13.0) < 1e-9,
      "odom goal must transform to map");
  require(std::abs(goal.value.yaw - 0.5) < 1e-9, "goal yaw must be preserved");

  path_msg.header = header("camera_link");
  require(
      !lingtu::nav::endpoint::decodePath(path_msg, mapOdom()).ok(),
  ...
  require(
      !lingtu::nav::endpoint::decodeGrid(grid_msg).ok(),
  ...
  require(
      !lingtu::nav::endpoint::decodeTwist(twist_msg).ok(),
...
void testSourceStampValidationRejectsReplayAndFutureCommands() {
  ...
  require(
      sourceStampError("clear_estop", 10.0, 10.1, 0.35, 0.05).empty(),
      "fresh clear-estop timestamp must pass");
  require(
      sourceStampError("clear_estop", 0.0, 10.0, 0.35, 0.05) ==
          "clear_estop_source_stamp_invalid",
      "zero clear-estop timestamp must fail");
  require(
      sourceStampError("clear_estop", 10.2, 10.0, 0.35, 0.05) ==
          "clear_estop_source_stamp_future",
      "future clear-estop timestamp must fail");
  require(
      sourceStampError("clear_estop", 9.0, 10.0, 0.35, 0.05) ==
...
void testResumeBoundaryRejectsOldMotionRequests() {
  ...
  require(
      !sourceStampPredates(1.0, 0.0),
      "requests before the first takeover must not require a resume boundary");
  require(
      sourceStampPredates(0.0, 10.0),
      "unstamped requests must fail after a resume boundary");
  require(
      sourceStampPredates(9.0, 10.0),
      "pre-resume requests must be rejected");
  require(
      sourceStampPredates(10.0, 10.0),
      "requests must be strictly newer than the resume boundary");
  require(
      !sourceStampPredates(10.1, 10.0),
      "fresh post-resume requests must pass");
}
...
void testPointCloudLayoutValidation() {
  ...
  require(
      lingtu::nav::endpoint::cloudToXyzh(msg, 0, std::nullopt, std::nullopt).size() == 4,
      "valid float32 cloud must decode");

  fields[0].datatype = 2;
  require(
      lingtu::nav::endpoint::cloudToXyzh(msg, 0, std::nullopt, std::nullopt).empty(),
  ...
  map_body.valid = true;
  const auto transformed = lingtu::nav::endpoint::cloudToXyzh(
      msg,
      0,
      map_body,
      std::nullopt);
