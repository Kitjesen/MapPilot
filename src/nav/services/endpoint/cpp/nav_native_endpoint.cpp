...
int main(int argc, char** argv) {
  try {
    ...
    GlobalPlanTask global_plan_task(
        [active_octomap_gate](const auto& request, const auto& cancel_check) {
          return runWithActiveOctomap(*active_octomap_gate, request, cancel_check);
        });
    ...
    while (g_running) {
      ...
      dds.drainTf([&](const lingtu_dds_TFMessage& msg) {
        ...
        if (map_odom_epoch_start_s > 0.0 &&
            tf->stamp_s + 1e-9 < map_odom_epoch_start_s) {
          frames.last_error = "map_odom_tf_before_current_epoch";
          ++tf_count;
          return;
        }
        if (last_tf_s > 0.0 && tf->stamp_s + 1e-9 < last_tf_s) {
          frames.last_error = "map_odom_tf_out_of_order";
          ++tf_count;
          return;
        }
        ...
        if (map_frame_jump) {
          ...
          map_odom_epoch_start_s = tf->stamp_s;
          frames.last_error = "map_frame_jump";
          (void)inspection_executor.Pause("map_frame_jump");
          (void)clear_endpoint_motion("map_frame_jump");
        }
      ...
      auto submit_goal = [&](const lingtu_dds_PoseStamped& msg)
        ...
        context.request.map_path = cfg.map_path;
        ...
        std::fprintf(stderr, "nav_native: OctoPlanner3D planning started\n");
      ...
      dds.drainCloud([&](const lingtu_dds_PointCloud2& msg) {
        ...
        const bool cloud_stamp_fresh =
            cloud_stamp_s > 0.0 &&
            (cfg.cloud_max_age_s <= 0.0 ||
             std::abs(cloud_now - cloud_stamp_s) <= cfg.cloud_max_age_s);
        if (!cloud_stamp_fresh) {
      ...
      dds.drainTraversability([&](const lingtu_dds_OccupancyGrid& msg) {
        ...
        last_traversability_s = headerStampSeconds(msg.header);
        ++traversability_count;
      });
      dds.drainLocalizationHealth([&](const lingtu_dds_Text& msg) {
        localization_health = decodeLocalizationHealth(textData(msg));
        if (!localization_health.valid) {
          frames.last_error = localization_health.error;
        } else {
      ...
      const double input_now = nowSeconds();
      ...
      if (auto completion = global_plan_task.poll()) {
        ...
        if (!completion->error.empty()) {
          last_plan.reason = "octoplanner3d_exception";
          ++plan_fail_count;
          std::fprintf(
              stderr,
              "nav_native: OctoPlanner3D exception: %s\n",
      ...
      if (active_teleop_request && !path_active && !input_gate_state.ready) {
        ...
        const double age_s = tick_now - control_authority.teleopStampSeconds();
        ...
        const double publish_age_s = nowSeconds() - control_authority.teleopStampSeconds();
