...
class FinalVelocityReader {
 public:
  ...
  Sample take() {
    ...
    if (count < 0) {
      throw std::runtime_error(
          std::string("dds_take(test_cmd_vel): ") +
          dds_strretcode(-count));
    }
    if (count == 0 || !infos[0].valid_data) {
      if (count > 0) {
        checked(
            dds_return_loan(reader_, samples, count),
            "dds_return_loan(test_cmd_vel)");
      }
      return {};
    }
    const auto& msg =
        *static_cast<lingtu_dds_FinalVelocityCommand*>(samples[0]);
    Sample result;
    result.host_boot_id = msg.host_boot_id ? msg.host_boot_id : "";
    result.producer_boot_id =
...
Sample writeAndTake(
    lingtu::nav::endpoint::DdsRuntime& runtime,
    FinalVelocityReader& reader,
    const nav_kernel::Twist& command) {
  for (int attempt = 0; attempt < 100; ++attempt) {
    require(runtime.writeCmdVel(command), "final velocity write must succeed");
...
void testWriterAddsFreshnessEnvelope() {
  ...
  const Sample first = writeAndTake(
      runtime, reader, nav_kernel::Twist{0.2, -0.1, 0.4});
  ...
  const Sample second = writeAndTake(
      runtime, reader, nav_kernel::Twist{});
  require(
      second.producer_boot_id == first.producer_boot_id,
      "producer id must remain stable for the process");
  require(
      second.output_seq == first.output_seq + 1,
      "output sequence must increase monotonically");
  require(
      second.source_boottime_ns >= first.source_boottime_ns,
      "boottime must not roll back");
}
...
int main() {
  try {
    ...
    std::fprintf(
        stderr, "test_final_velocity_writer: FAIL: %s\n", exc.what());
    return 1;
  }
}
