...
void testPureTeleopNeedsNoPoseButStillLimitsVelocity() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      cfg, {1.0, 0.0, 2.0}, 0.0, std::nullopt, {}, {}, false);
...
void testTeleopAvoidFailsClosedWithoutPose() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      cfg, {0.2, 0.0, 0.0}, 0.0, std::nullopt, {}, {}, false);
...
void testSubthresholdPureTranslationPublishesAnExplicitZero() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      cfg, {0.02, 0.0, 0.0}, 0.0, std::nullopt, {}, {}, false);
  ...
  require(
      lingtu::nav::endpoint::linearSpeed(decision.cmd) == 0.0 && decision.cmd.wz == 0.0,
...
void testSubthresholdTranslationCannotBypassForwardChecksDuringYaw() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      cfg, {0.02, 0.0, 0.20}, 0.0, std::nullopt, {}, {}, false);
  ...
  require(decision.cmd.vx == 0.0 && decision.cmd.vy == 0.0, "translation deadband must be applied independently of yaw");
...
void testAutonomySafetyFollowsLocalPathInsteadOfFrozenTwistArc() {
  ...
  require(
      frozen_twist.stopped || frozen_twist.slowed,
      "the regression fixture must intersect the frozen-Twist sweep");
  require(
      !path_aware.stopped && !path_aware.slowed,
...
void testStraightMotionPreservesObstacleDistanceBands() {
  ...
  require(decision.slowed && !decision.stopped, "straight motion must slow in the configured slow band");
  require(decision.reason == "obstacle_slow", "straight obstacle reason must remain explicit");
  require(
      std::abs(decision.obstacle_distance_m - 0.90) < 0.06,
...
void testMixedTurnDetectsObstacleInsideSweptFootprint() {
  ...
  require(
      decision.obstacle_distance_m > cfg.stop_distance_m,
...
lingtu::nav::endpoint::TraversabilityGrid gridWithCostAt(
    double map_x,
    double map_y,
    float cost) {
  lingtu::nav::endpoint::TraversabilityGrid grid;
...
void testTeleopAvoidSlowsForSoftTerrainCost() {
  ...
  require(
      std::abs(decision.cmd.vx - 0.105) < 1e-9,
...
void testTeleopAvoidStopsForHardTerrainCost() {
  ...
  require(lingtu::nav::endpoint::linearSpeed(decision.cmd) == 0.0, "hard terrain output must be zero");
}
...
void testHardTerrainOutsideStopBandSlowsBeforeStopping() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      ...
      pose,
      {},
      gridWithCostAt(0.75, 0.0, 100.0F),
      true);

  require(
      decision.slowed && !decision.stopped,
...
void testMixedTurnSamplesTerrainAlongArc() {
  ...
  const auto decision = lingtu::nav::endpoint::arbitrateTeleopCommand(
      ...
      pose,
      {},
      gridWithCostAt(0.29, 0.23, 100.0F),
      true);
...
void testTeleopAvoidFailsClosedForStaleTerrainCost() {
  ...
  require(lingtu::nav::endpoint::linearSpeed(decision.cmd) == 0.0, "stale terrain output must be zero");
}
...
void testSafetySlowCannotReintroduceSubthresholdTranslation() {
  ...
  require(
      decision.reason == "terrain_slow_below_min_motion",
      "post-safety deadband reason must preserve the terrain source");
  require(
      lingtu::nav::endpoint::linearSpeed(decision.cmd) == 0.0,
