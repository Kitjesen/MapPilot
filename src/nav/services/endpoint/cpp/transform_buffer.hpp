...
class TransformBuffer {
 public:
  explicit TransformBuffer(double window_s = 2.0)
      : window_s_(std::max(0.1, window_s)) {}
  ...
  std::optional<RigidTransform> sample(double stamp_s, double max_gap_s) const {
    ...
    if (right == entries_.end()) {
      ...
      return std::abs(stamp_s - last.stamp_s) <= gap
          ? std::optional<RigidTransform>(last.transform)
          : std::nullopt;
    }
  ...
  double nearestGap(double stamp_s) const {
    ...
    return std::min(
        std::abs(stamp_s - (right - 1)->stamp_s),
        std::abs(right->stamp_s - stamp_s));
  }
 ...
 private:
  ...
  static RigidTransform interpolate(
      const RigidTransform& before,
      const RigidTransform& after,
      double ratio,
      double stamp_s) {
