...
class FrameContract:
    ...
    def runtime_contract() -> str | None:
        ...
        return os.environ.get("LINGTU_RUNTIME_CONTRACT") or os.environ.get("LINGTU_DATA_SOURCE")

    def expected_frames_for_source(self, source: str) -> tuple[str, ...]:
        """Return the expected frame ids for a given data *source*."""
        planning_frame = normalize_frame_id(self._planning_frame_id) or PLANNING_FRAME_ID
    ...
    def is_frame_mismatch(self, frame_id: str, *, source: str) -> bool:
        ...
        return bool(
            normalized and normalized != "unknown" and normalized not in self.expected_frames_for_source(source)
        )
