...
is delegated to lazy imports from the perception package.
...
class PerceptionModule(Module, layer=3):
    ...
    def setup(self) -> None:
        ...
        Components are created through two paths depending on detector_type:
          - Factory path  (yolo_world / clip):  api/factory.py 鈫?impl/ adapters
    ...
    def _setup_via_factory(self) -> None:
        ...
        try:
            ...
            from perception.api.types import PerceptionConfig
            ...
            logger.warning(
                "Factory setup failed (%s) 鈥?falling back to direct import path", e
            )
            self._setup_direct()
    ...
    def _project_to_3d_fallback(
        ...
        for det2d in detections_2d:
            ...
            # Approximate 3D extent from bbox pixel extent 脳 depth / focal
