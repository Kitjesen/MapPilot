...
RUNTIME_AUDIT_VALIDATE_CHECK_COVERAGE = {
    ...
    "runtime_stage_owner_frame_map_dependency_integrity": ("runtime_contract_integrity",),
...
SAVED_MAP_ARTIFACT_GATE_VALIDATES = (
    ...
    "octomap_source_map_sha256_matches_map_pcd",
...
SAVED_MAP_ARTIFACT_GATE_PROVES = (
    ...
    "octomap_and_occupancy_derive_from_same_map_pcd",
...
_RUNTIME_VALIDATION_GATES: dict[str, dict[str, Any]] = {
    "runtime_audit": {
        ...
        "coverage": {name: list(checks) for name, checks in RUNTIME_AUDIT_VALIDATE_CHECK_COVERAGE.items()},
    },
...
def _gate_acceptance_payload(gate: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "operator_summary_sections": list(gate.get("operator_summary_sections") or ()),
    }
...
def validate_runtime_validation_gates(
    ...
    expected_coverage = {name: list(checks) for name, checks in RUNTIME_AUDIT_VALIDATE_CHECK_COVERAGE.items()}
    ...
    covered_checks = {
        str(check) for checks in runtime_audit_coverage.values() if isinstance(checks, list) for check in checks
    }
    missing_covered_checks = sorted(set(RUNTIME_AUDIT_CHECKS) - covered_checks)
    if missing_covered_checks:
        blockers.append("runtime_audit checks have no validation coverage: " + ", ".join(missing_covered_checks))
    unknown_covered_checks = sorted(covered_checks - set(RUNTIME_AUDIT_CHECKS))
    if unknown_covered_checks:
        blockers.append("runtime_audit coverage references unknown checks: " + ", ".join(unknown_covered_checks))
    ...
    if real_evidence.get("collector_command") != REAL_RUNTIME_EVIDENCE_COLLECTOR_COMMAND:
        blockers.append("real_runtime_evidence validation gate collector command drifted")
    ...
    if real_evidence.get("expected_runtime_contract") != REAL_RUNTIME_CONTRACT:
        blockers.append(f"real_runtime_evidence expected contract is not {REAL_RUNTIME_CONTRACT}")
    if real_evidence.get("requires_ros") is not False:
        blockers.append("real_runtime_evidence validation gate must not require ROS")
