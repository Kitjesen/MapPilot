...
def process_rows() -> list[dict[str, Any]]:
    """Return CPU/RSS for LingTu Host and native SLAM processes."""

    try:
        raw = subprocess.check_output(
            ["ps", "-eo", "comm,pcpu,pmem,rss,args"],
    ...
    for line in raw.splitlines()[1:]:
        ...
        if len(parts) < 5:
            continue
        command, cpu, memory, rss, args = parts
        normalized = args.lower().replace("-", "_")
        label = "host" if "lingtu.real.host" in args else "slamd" if "slamd" in normalized else command
        if label not in {"host", "slamd"}:
            continue
        try:
            rows.append({"label": label, "pcpu": float(cpu), "pmem": float(memory), "rss_kb": int(rss)})
        except ValueError:
            continue
    return rows


def process_summary(samples: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    """Aggregate process CPU and memory observations by runtime label."""
    grouped: dict[str, dict[str, list[float]]] = {}
    for sample in samples:
        for row in sample.get("processes", []):
            bucket = grouped.setdefault(str(row["label"]), {"pcpu": [], "pmem": [], "rss_kb": []})
            for key in bucket:
                bucket[key].append(float(row[key]))
    return {
        label: {
            "avg_pcpu": round(sum(values["pcpu"]) / len(values["pcpu"]), 2),
            "max_pcpu": round(max(values["pcpu"]), 2),
            "avg_rss_mb": round(sum(values["rss_kb"]) / len(values["rss_kb"]) / 1024, 1),
            "max_rss_mb": round(max(values["rss_kb"]) / 1024, 1),
            "avg_pmem": round(sum(values["pmem"]) / len(values["pmem"]), 2),
        }
