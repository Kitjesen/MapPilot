...
class Tomogram(object):
    ...
    def initBuffers(self):
        self.layers_g = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
        self.layers_c = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
        self.grad_mag_sq = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
        self.grad_mag_max = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
        self.trav_cost = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
        self.inflated_cost = np.zeros((self.n_slice_init, self.map_dim_x, self.map_dim_y), dtype=np.float32)
    ...
    def point2map(self, points):
        ...
        t_trav = (time.time() - start_time) * 1e3
