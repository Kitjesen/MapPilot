...
struct NodeConfig
{
    ...
    std::string fixture_export_path;
};
...
public:
    ...
    void loadParameters()
    {
        ...
        this->declare_parameter("fixture_export_path", "");
        std::string config_path;
        this->get_parameter<std::string>("config_path", config_path);
        this->get_parameter<std::string>("fixture_export_path", m_node_config.fixture_export_path);
        ...
        if (config["fixture_export_path"])
            m_node_config.fixture_export_path = config["fixture_export_path"].as<std::string>();
    ...
    void exportFixtureIfConfigured()
    {
        if (m_node_config.fixture_export_path.empty())
            return;
        std::filesystem::path fixture_path(m_node_config.fixture_export_path);
        std::filesystem::path parent_path = fixture_path.parent_path();
        if (!parent_path.empty())
        {
            ...
            if (error)
            {
                RCLCPP_WARN(this->get_logger(), "FAILED TO CREATE FIXTURE DIR %s: %s", parent_path.string().c_str(), error.message().c_str());
                return;
            }
        }
        if (!m_pgo->exportPoseGraphFixture(fixture_path.string()))
        {
            RCLCPP_WARN(this->get_logger(), "FAILED TO EXPORT PGO FIXTURE TO %s", fixture_path.string().c_str());
            return;
        }
        RCLCPP_INFO(this->get_logger(), "EXPORT PGO FIXTURE TO %s", fixture_path.string().c_str());
    }
