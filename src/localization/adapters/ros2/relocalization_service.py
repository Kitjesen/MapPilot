"""ROS relocalization service-call compatibility adapter."""
...
GLOBAL_RELOCALIZE_STATUS_SERVICE = TOPICS.global_relocalize_status_service
...
def global_relocalize_status_command() -> list[str]:
    command = (
        _ROS_SETUP
        + "unset RMW_IMPLEMENTATION; "
        f"ros2 service call {GLOBAL_RELOCALIZE_STATUS_SERVICE} "
        "std_srvs/srv/Trigger '{}'"
    )
    return ["bash", "-c", command]
...
def query_global_relocalize_status(*, timeout_s: float = 5.0) -> RelocalizationResult:
    return _run(global_relocalize_status_command(), timeout_s=timeout_s)
