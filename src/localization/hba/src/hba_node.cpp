...
public:
    ...
    void exportFixtureIfConfigured()
    {
        ...
        if (!m_hba->exportPoseGraphFixture(fixture_path.string()))
        {
            RCLCPP_WARN(this->get_logger(), "FAILED TO EXPORT HBA FIXTURE TO %s", fixture_path.string().c_str());
            return;
        }
        RCLCPP_INFO(this->get_logger(), "EXPORT HBA FIXTURE TO %s", fixture_path.string().c_str());
    }
