"""Typed DDS topic registry for LingTu runtime streams."""

from __future__ import annotations

from dataclasses import dataclass
from importlib import import_module
from typing import Any

from runtime.runtime_interface import TOPICS


@dataclass(frozen=True)
class TopicSpec:
    topic: str
    type_name: str
    import_path: str
    idl_type: str
    cpp_type: str
    ros_compatible: bool = True
    dds_topic: str | None = None

    def dds_type(self) -> Any:
        module_name, attr = self.import_path.rsplit(".", 1)
        return getattr(import_module(module_name), attr, None)


TOPIC_SPECS: dict[str, TopicSpec] = {
    "/tf": TopicSpec(
        "/tf",
        "TFMessage",
        "message.dds_types.geometry.TFMessage",
        "lingtu.dds.TFMessage",
        "lingtu::dds::TFMessage",
        ros_compatible=False,
    ),
    "/tf_static": TopicSpec(
        "/tf_static",
        "TFMessage",
        "message.dds_types.geometry.TFMessage",
        "lingtu.dds.TFMessage",
        "lingtu::dds::TFMessage",
        ros_compatible=False,
    ),
    # Native hardware wire. ROS2/Livox-compatible DDS is adapter-only and must
    # translate into this schema before entering the SLAM hot path.
    TOPICS.lidar_scan: TopicSpec(
        TOPICS.lidar_scan,
        "LivoxFrame",
        "message.dds_types.livox.LivoxFrame",
        "lingtu.dds.LivoxFrame",
        "lingtu::dds::LivoxFrame",
        ros_compatible=False,
    ),
    TOPICS.raw_lidar_packet: TopicSpec(
        TOPICS.raw_lidar_packet,
        "LivoxFrame",
        "message.dds_types.livox.LivoxFrame",
        "lingtu.dds.LivoxFrame",
        "lingtu::dds::LivoxFrame",
        ros_compatible=False,
    ),
    TOPICS.imu: TopicSpec(
        TOPICS.imu,
        "Imu",
        "message.dds_types.imu.Imu",
        "lingtu.dds.Imu",
        "lingtu::dds::Imu",
        ros_compatible=False,
    ),
    TOPICS.raw_imu: TopicSpec(
        TOPICS.raw_imu,
        "Imu",
        "message.dds_types.imu.Imu",
        "lingtu.dds.Imu",
        "lingtu::dds::Imu",
        ros_compatible=False,
    ),
    TOPICS.odom_prior: TopicSpec(
        TOPICS.odom_prior,
        "Odometry",
        "message.dds_types.nav.Odometry",
        "lingtu.dds.Odometry",
        "lingtu::dds::Odometry",
        ros_compatible=False,
    ),
    TOPICS.driver_odometry: TopicSpec(
        TOPICS.driver_odometry,
        "Odometry",
        "message.dds_types.nav.Odometry",
        "lingtu.dds.Odometry",
        "lingtu::dds::Odometry",
        ros_compatible=False,
    ),
    TOPICS.odometry: TopicSpec(
        TOPICS.odometry,
        "Odometry",
        "message.dds_types.nav.Odometry",
        "lingtu.dds.Odometry",
        "lingtu::dds::Odometry",
        ros_compatible=False,
    ),
    TOPICS.state_estimation_at_scan: TopicSpec(
        TOPICS.state_estimation_at_scan,
        "Odometry",
        "message.dds_types.nav.Odometry",
        "lingtu.dds.Odometry",
        "lingtu::dds::Odometry",
        ros_compatible=False,
    ),
    TOPICS.registered_cloud: TopicSpec(
        TOPICS.registered_cloud,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.map_observation: TopicSpec(
        TOPICS.map_observation,
        "MapObservation",
        "message.dds_types.nav.MapObservation",
        "lingtu.dds.MapObservation",
        "lingtu::dds::MapObservation",
        ros_compatible=False,
    ),
    TOPICS.map_cloud: TopicSpec(
        TOPICS.map_cloud,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.cumulative_map_cloud: TopicSpec(
        TOPICS.cumulative_map_cloud,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.saved_map_cloud: TopicSpec(
        TOPICS.saved_map_cloud,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.maps_activation_request: TopicSpec(
        TOPICS.maps_activation_request,
        "MapActivationRequest",
        "message.dds_types.nav.MapActivationRequest",
        "lingtu.dds.MapActivationRequest",
        "lingtu::dds::MapActivationRequest",
        ros_compatible=False,
    ),
    TOPICS.maps_activation_ack: TopicSpec(
        TOPICS.maps_activation_ack,
        "MapActivationAck",
        "message.dds_types.nav.MapActivationAck",
        "lingtu.dds.MapActivationAck",
        "lingtu::dds::MapActivationAck",
        ros_compatible=False,
    ),
    TOPICS.maps_state: TopicSpec(
        TOPICS.maps_state,
        "MapRuntimeState",
        "message.dds_types.nav.MapRuntimeState",
        "lingtu.dds.MapRuntimeState",
        "lingtu::dds::MapRuntimeState",
        ros_compatible=False,
    ),
    TOPICS.maps_live_cloud: TopicSpec(
        TOPICS.maps_live_cloud,
        "MapCloudLayer",
        "message.dds_types.nav.MapCloudLayer",
        "lingtu.dds.MapCloudLayer",
        "lingtu::dds::MapCloudLayer",
        ros_compatible=False,
    ),
    TOPICS.maps_voxel_cloud: TopicSpec(
        TOPICS.maps_voxel_cloud,
        "MapCloudLayer",
        "message.dds_types.nav.MapCloudLayer",
        "lingtu.dds.MapCloudLayer",
        "lingtu::dds::MapCloudLayer",
        ros_compatible=False,
    ),
    TOPICS.maps_accumulated_cloud: TopicSpec(
        TOPICS.maps_accumulated_cloud,
        "MapCloudLayer",
        "message.dds_types.nav.MapCloudLayer",
        "lingtu.dds.MapCloudLayer",
        "lingtu::dds::MapCloudLayer",
        ros_compatible=False,
    ),
    TOPICS.maps_occupancy: TopicSpec(
        TOPICS.maps_occupancy,
        "MapGrid",
        "message.dds_types.nav.MapGrid",
        "lingtu.dds.MapGrid",
        "lingtu::dds::MapGrid",
        ros_compatible=False,
    ),
    TOPICS.maps_elevation: TopicSpec(
        TOPICS.maps_elevation,
        "MapGrid",
        "message.dds_types.nav.MapGrid",
        "lingtu.dds.MapGrid",
        "lingtu::dds::MapGrid",
        ros_compatible=False,
    ),
    TOPICS.maps_esdf: TopicSpec(
        TOPICS.maps_esdf,
        "MapGrid",
        "message.dds_types.nav.MapGrid",
        "lingtu.dds.MapGrid",
        "lingtu::dds::MapGrid",
        ros_compatible=False,
    ),
    TOPICS.maps_scene: TopicSpec(
        TOPICS.maps_scene,
        "MapScene",
        "message.dds_types.nav.MapScene",
        "lingtu.dds.MapScene",
        "lingtu::dds::MapScene",
        ros_compatible=False,
    ),
    TOPICS.camera_color: TopicSpec(
        TOPICS.camera_color,
        "Image",
        "message.dds_types.camera.Image",
        "lingtu.dds.Image",
        "lingtu::dds::Image",
        ros_compatible=True,
        dds_topic="rt/camera/color",
    ),
    TOPICS.camera_depth: TopicSpec(
        TOPICS.camera_depth,
        "Image",
        "message.dds_types.camera.Image",
        "lingtu.dds.Image",
        "lingtu::dds::Image",
        ros_compatible=True,
        dds_topic="rt/camera/depth",
    ),
    TOPICS.camera_info: TopicSpec(
        TOPICS.camera_info,
        "CameraInfo",
        "message.dds_types.camera.CameraInfo",
        "lingtu.dds.CameraInfo",
        "lingtu::dds::CameraInfo",
        ros_compatible=True,
        dds_topic="rt/camera/info",
    ),
    TOPICS.gnss_fix: TopicSpec(
        TOPICS.gnss_fix,
        "GnssFix",
        "message.dds_types.gnss.GnssFix",
        "lingtu.dds.GnssFix",
        "lingtu::dds::GnssFix",
        ros_compatible=False,
        dds_topic="rt/gnss/fix",
    ),
    TOPICS.gnss_status: TopicSpec(
        TOPICS.gnss_status,
        "GnssStatus",
        "message.dds_types.gnss.GnssStatus",
        "lingtu.dds.GnssStatus",
        "lingtu::dds::GnssStatus",
        ros_compatible=False,
        dds_topic="rt/gnss/status",
    ),
    TOPICS.gnss_odom: TopicSpec(
        TOPICS.gnss_odom,
        "Odometry",
        "message.dds_types.nav.Odometry",
        "lingtu.dds.Odometry",
        "lingtu::dds::Odometry",
        ros_compatible=False,
        dds_topic="rt/gnss/odom",
    ),
    TOPICS.slam_map_command: TopicSpec(
        TOPICS.slam_map_command,
        "Text",
        "message.dds_types.scalar.Text",
        "lingtu.dds.Text",
        "lingtu::dds::Text",
        ros_compatible=False,
    ),
    TOPICS.slam_map_event: TopicSpec(
        TOPICS.slam_map_event,
        "Text",
        "message.dds_types.scalar.Text",
        "lingtu.dds.Text",
        "lingtu::dds::Text",
        ros_compatible=False,
    ),
    TOPICS.slam_relocalization_request: TopicSpec(
        TOPICS.slam_relocalization_request,
        "RelocalizationRequest",
        "message.dds_types.slam.RelocalizationRequest",
        "lingtu.dds.RelocalizationRequest",
        "lingtu::dds::RelocalizationRequest",
        ros_compatible=False,
    ),
    TOPICS.slam_relocalization_response: TopicSpec(
        TOPICS.slam_relocalization_response,
        "RelocalizationResponse",
        "message.dds_types.slam.RelocalizationResponse",
        "lingtu.dds.RelocalizationResponse",
        "lingtu::dds::RelocalizationResponse",
        ros_compatible=False,
    ),
    TOPICS.localization_quality: TopicSpec(
        TOPICS.localization_quality,
        "Float32",
        "message.dds_types.scalar.Float32",
        "lingtu.dds.Float32",
        "lingtu::dds::Float32",
        ros_compatible=False,
    ),
    TOPICS.localization_health: TopicSpec(
        TOPICS.localization_health,
        "Text",
        "message.dds_types.scalar.Text",
        "lingtu.dds.Text",
        "lingtu::dds::Text",
        ros_compatible=False,
    ),
    TOPICS.global_path: TopicSpec(
        TOPICS.global_path,
        "Path",
        "message.dds_types.nav.Path",
        "lingtu.dds.Path",
        "lingtu::dds::Path",
        ros_compatible=False,
    ),
    TOPICS.local_path: TopicSpec(
        TOPICS.local_path,
        "Path",
        "message.dds_types.nav.Path",
        "lingtu.dds.Path",
        "lingtu::dds::Path",
        ros_compatible=False,
    ),
    TOPICS.nav_way_point: TopicSpec(
        TOPICS.nav_way_point,
        "PoseStamped",
        "message.dds_types.geometry.PoseStamped",
        "lingtu.dds.PoseStamped",
        "lingtu::dds::PoseStamped",
        ros_compatible=False,
    ),
    TOPICS.nav_command_request: TopicSpec(
        TOPICS.nav_command_request,
        "NavigationCommandRequest",
        "message.dds_types.nav.NavigationCommandRequest",
        "lingtu.dds.NavigationCommandRequest",
        "lingtu::dds::NavigationCommandRequest",
        ros_compatible=False,
    ),
    TOPICS.nav_command_ack: TopicSpec(
        TOPICS.nav_command_ack,
        "NavigationCommandAck",
        "message.dds_types.nav.NavigationCommandAck",
        "lingtu.dds.NavigationCommandAck",
        "lingtu::dds::NavigationCommandAck",
        ros_compatible=False,
    ),
    TOPICS.operator_motion_control: TopicSpec(
        TOPICS.operator_motion_control,
        "OperatorMotionControl",
        "message.dds_types.nav.OperatorMotionControl",
        "lingtu.dds.OperatorMotionControl",
        "lingtu::dds::OperatorMotionControl",
        ros_compatible=False,
    ),
    TOPICS.operator_motion_sample: TopicSpec(
        TOPICS.operator_motion_sample,
        "OperatorMotionSample",
        "message.dds_types.nav.OperatorMotionSample",
        "lingtu.dds.OperatorMotionSample",
        "lingtu::dds::OperatorMotionSample",
        ros_compatible=False,
    ),
    TOPICS.operator_motion_ack: TopicSpec(
        TOPICS.operator_motion_ack,
        "OperatorMotionAck",
        "message.dds_types.nav.OperatorMotionAck",
        "lingtu.dds.OperatorMotionAck",
        "lingtu::dds::OperatorMotionAck",
        ros_compatible=False,
    ),
    TOPICS.operator_motion_status: TopicSpec(
        TOPICS.operator_motion_status,
        "OperatorMotionStatus",
        "message.dds_types.nav.OperatorMotionStatus",
        "lingtu.dds.OperatorMotionStatus",
        "lingtu::dds::OperatorMotionStatus",
        ros_compatible=False,
    ),
    TOPICS.nav_goal_status: TopicSpec(
        TOPICS.nav_goal_status,
        "NavigationGoalStatus",
        "message.dds_types.nav.NavigationGoalStatus",
        "lingtu.dds.NavigationGoalStatus",
        "lingtu::dds::NavigationGoalStatus",
        ros_compatible=False,
    ),
    TOPICS.nav_state: TopicSpec(
        TOPICS.nav_state,
        "NavigationState",
        "message.dds_types.nav.NavigationState",
        "lingtu.dds.NavigationState",
        "lingtu::dds::NavigationState",
        ros_compatible=False,
    ),
    TOPICS.exploration_command: TopicSpec(
        TOPICS.exploration_command,
        "ExplorationCommandRequest",
        "message.dds_types.nav.ExplorationCommandRequest",
        "lingtu.dds.ExplorationCommandRequest",
        "lingtu::dds::ExplorationCommandRequest",
        ros_compatible=False,
    ),
    TOPICS.exploration_ack: TopicSpec(
        TOPICS.exploration_ack,
        "ExplorationCommandAck",
        "message.dds_types.nav.ExplorationCommandAck",
        "lingtu.dds.ExplorationCommandAck",
        "lingtu::dds::ExplorationCommandAck",
        ros_compatible=False,
    ),
    TOPICS.exploration_run_event: TopicSpec(
        TOPICS.exploration_run_event,
        "ExplorationRunEvent",
        "message.dds_types.nav.ExplorationRunEvent",
        "lingtu.dds.ExplorationRunEvent",
        "lingtu::dds::ExplorationRunEvent",
        ros_compatible=False,
        dds_topic="rt/nav/exploration/run/event",
    ),
    TOPICS.exploration_segment_request: TopicSpec(
        TOPICS.exploration_segment_request,
        "ExplorationSegmentRequest",
        "message.dds_types.nav.ExplorationSegmentRequest",
        "lingtu.dds.ExplorationSegmentRequest",
        "lingtu::dds::ExplorationSegmentRequest",
        ros_compatible=False,
    ),
    TOPICS.exploration_segment_ack: TopicSpec(
        TOPICS.exploration_segment_ack,
        "ExplorationSegmentAck",
        "message.dds_types.nav.ExplorationSegmentAck",
        "lingtu.dds.ExplorationSegmentAck",
        "lingtu::dds::ExplorationSegmentAck",
        ros_compatible=False,
    ),
    TOPICS.exploration_segment_status: TopicSpec(
        TOPICS.exploration_segment_status,
        "ExplorationSegmentStatus",
        "message.dds_types.nav.ExplorationSegmentStatus",
        "lingtu.dds.ExplorationSegmentStatus",
        "lingtu::dds::ExplorationSegmentStatus",
        ros_compatible=False,
    ),
    TOPICS.inspection_task_request: TopicSpec(
        TOPICS.inspection_task_request,
        "InspectionTaskRequest",
        "message.dds_types.nav.InspectionTaskRequest",
        "lingtu.dds.InspectionTaskRequest",
        "lingtu::dds::InspectionTaskRequest",
        ros_compatible=False,
        dds_topic="rt/nav/inspection/task/request",
    ),
    TOPICS.inspection_task_ack: TopicSpec(
        TOPICS.inspection_task_ack,
        "InspectionTaskAck",
        "message.dds_types.nav.InspectionTaskAck",
        "lingtu.dds.InspectionTaskAck",
        "lingtu::dds::InspectionTaskAck",
        ros_compatible=False,
        dds_topic="rt/nav/inspection/task/ack",
    ),
    TOPICS.inspection_evidence_request: TopicSpec(
        TOPICS.inspection_evidence_request,
        "InspectionEvidenceRequest",
        "message.dds_types.nav.InspectionEvidenceRequest",
        "lingtu.dds.InspectionEvidenceRequest",
        "lingtu::dds::InspectionEvidenceRequest",
        ros_compatible=False,
        dds_topic="rt/nav/inspection/evidence/request",
    ),
    TOPICS.inspection_evidence_result: TopicSpec(
        TOPICS.inspection_evidence_result,
        "InspectionEvidenceResult",
        "message.dds_types.nav.InspectionEvidenceResult",
        "lingtu.dds.InspectionEvidenceResult",
        "lingtu::dds::InspectionEvidenceResult",
        ros_compatible=False,
        dds_topic="rt/nav/inspection/evidence/result",
    ),
    TOPICS.inspection_status: TopicSpec(
        TOPICS.inspection_status,
        "InspectionStatus",
        "message.dds_types.nav.InspectionStatus",
        "lingtu.dds.InspectionStatus",
        "lingtu::dds::InspectionStatus",
        ros_compatible=False,
    ),
    TOPICS.inspection_task_event: TopicSpec(
        TOPICS.inspection_task_event,
        "InspectionTaskEvent",
        "message.dds_types.nav.InspectionTaskEvent",
        "lingtu.dds.InspectionTaskEvent",
        "lingtu::dds::InspectionTaskEvent",
        ros_compatible=False,
        dds_topic="rt/nav/inspection/task/event",
    ),
    TOPICS.goal_pose: TopicSpec(
        TOPICS.goal_pose,
        "PoseStamped",
        "message.dds_types.geometry.PoseStamped",
        "lingtu.dds.PoseStamped",
        "lingtu::dds::PoseStamped",
        ros_compatible=False,
    ),
    TOPICS.cancel: TopicSpec(
        TOPICS.cancel,
        "Text",
        "message.dds_types.scalar.Text",
        "lingtu.dds.Text",
        "lingtu::dds::Text",
        ros_compatible=False,
    ),
    TOPICS.semantic_instruction: TopicSpec(
        TOPICS.semantic_instruction,
        "Text",
        "message.dds_types.scalar.Text",
        "lingtu.dds.Text",
        "lingtu::dds::Text",
        ros_compatible=False,
    ),
    TOPICS.traversability: TopicSpec(
        TOPICS.traversability,
        "OccupancyGrid",
        "message.dds_types.nav.OccupancyGrid",
        "lingtu.dds.OccupancyGrid",
        "lingtu::dds::OccupancyGrid",
        ros_compatible=False,
    ),
    TOPICS.terrain_map: TopicSpec(
        TOPICS.terrain_map,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.terrain_map_ext: TopicSpec(
        TOPICS.terrain_map_ext,
        "PointCloud2",
        "message.dds_types.pointcloud.PointCloud2",
        "lingtu.dds.PointCloud2",
        "lingtu::dds::PointCloud2",
        ros_compatible=False,
    ),
    TOPICS.map_clearing: TopicSpec(
        TOPICS.map_clearing,
        "Bool",
        "message.dds_types.scalar.Bool",
        "lingtu.dds.Bool",
        "lingtu::dds::Bool",
        ros_compatible=False,
    ),
    TOPICS.cloud_clearing: TopicSpec(
        TOPICS.cloud_clearing,
        "Bool",
        "message.dds_types.scalar.Bool",
        "lingtu.dds.Bool",
        "lingtu::dds::Bool",
        ros_compatible=False,
    ),
    TOPICS.cmd_vel: TopicSpec(
        TOPICS.cmd_vel,
        "FinalVelocityCommand",
        "message.dds_types.geometry.FinalVelocityCommand",
        "lingtu.dds.FinalVelocityCommand",
        "lingtu::dds::FinalVelocityCommand",
        ros_compatible=False,
    ),
    TOPICS.teleop_cmd_vel: TopicSpec(
        TOPICS.teleop_cmd_vel,
        "TwistStamped",
        "message.dds_types.geometry.TwistStamped",
        "lingtu.dds.TwistStamped",
        "lingtu::dds::TwistStamped",
        ros_compatible=False,
    ),
    TOPICS.exploration_grid: TopicSpec(
        TOPICS.exploration_grid,
        "OccupancyGrid",
        "message.dds_types.nav.OccupancyGrid",
        "lingtu.dds.OccupancyGrid",
        "lingtu::dds::OccupancyGrid",
        ros_compatible=False,
    ),
    TOPICS.exploration_execution_snapshot: TopicSpec(
        TOPICS.exploration_execution_snapshot,
        "ExplorationExecutionGrid",
        "message.dds_types.nav.ExplorationExecutionGrid",
        "lingtu.dds.ExplorationExecutionGrid",
        "lingtu::dds::ExplorationExecutionGrid",
        ros_compatible=False,
    ),
    TOPICS.exploration_snapshot: TopicSpec(
        TOPICS.exploration_snapshot,
        "ExplorationGrid",
        "message.dds_types.nav.ExplorationGrid",
        "lingtu.dds.ExplorationGrid",
        "lingtu::dds::ExplorationGrid",
        ros_compatible=False,
    ),
}


def topic_spec(topic: str) -> TopicSpec | None:
    return TOPIC_SPECS.get(str(topic))


def dds_type_for_topic(topic: str) -> Any | None:
    spec = topic_spec(topic)
    return spec.dds_type() if spec else None


def to_dds_message(topic: str, msg: Any) -> Any:
    """Convert a runtime message to the registered DDS payload for a topic."""

    topic = str(topic)
    if topic in {TOPICS.raw_lidar_points, TOPICS.raw_lidar_packet}:
        from message.dds_types.livox import livox_frame_to_msg

        return livox_frame_to_msg(msg)
    from message.dds_codec import to_dds_message as convert

    return convert(topic, msg)


def from_dds_message(topic: str, msg: Any) -> Any:
    """Convert a registered DDS payload back to a runtime message."""

    topic = str(topic)
    if topic in {TOPICS.raw_lidar_points, TOPICS.raw_lidar_packet}:
        from message.dds_types.livox import livox_msg_to_frame

        return livox_msg_to_frame(msg)
    if topic == TOPICS.raw_imu:
        from message.dds_types.imu import dds_imu_to_imu

        return dds_imu_to_imu(msg)
    from message.dds_codec import from_dds_message as convert

    return convert(topic, msg)


def dds_topic_name(topic: str, *, typed: bool) -> str:
    name = str(topic)
    spec = topic_spec(name)
    if typed and spec and spec.dds_topic:
        return spec.dds_topic
    if typed and name.startswith("/"):
        return "rt" + name
    return name
