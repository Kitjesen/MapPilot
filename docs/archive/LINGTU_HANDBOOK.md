...
Positioning:
- Not a SLAM library, not a planner library ï¿?it is the complete pipeline from sensors to motors.
- Not a teleop tool ï¿?it is an autonomous decision system. Give it a goal and it plans, drives, avoids and recovers on its own.
- Not a simulation toy ï¿?it runs on real hardware (S100P / RDK X5).

Target hardware:
- Compute: S100P (Nash BPU 128 TOPS, aarch64, Ubuntu 22.04 + ROS 2 Humble)
- LiDAR: Livox MID-360 (10 Hz, 360 deg x 59 deg)
- Camera: Orbbec Gemini 335 (color + depth, mounted vertically ï¿?see camera.rotate)
...
Design principles:
1. Module-First ï¿?Module is the only runtime unit, Blueprint is the only assembly unit.
2. Composition over inheritance ï¿?nine stack factory functions assemble the full system.
3. Pluggable backends via Registry ï¿?algorithm selection is `@register("category", "name")`, no `if/else` branching.
4. C++ owns the hot paths ï¿?Python orchestrates, C++ does the heavy lifting through nanobind.
5. Raw DDS instead of rclpy ï¿?production avoids the ROS 2 Python runtime; rclpy is only a fallback.
...
| Blueprint | DSL for assembling modules ï¿?supports `autoconnect`, explicit `wire`, multiple transports | `runtime.blueprint.Blueprint` |
...
## Part II ï¿?Sensing and Localization
...
- DDS-first via raw cyclonedds ï¿?runs without a full ROS 2 environment, this is the production path.
- rclpy fallback ï¿?used when ROS 2 Python is available; adds watchdog + USB reset + L1/L2/L3 self-recovery.
...
3. DUFOMap offline filter (Phase 2) ï¿?ray-casting + void detection on `<map>/patches/*.pcd`. Writes a clean `map.pcd`, backs the original up as `map.pcd.predufo`. Gated by `LINGTU_SAVE_DYNAMIC_FILTER=1`. Binary at `~/src/dufomap/build/dufomap_run`, config `config/dufomap.toml`.
...
## Part III ï¿?Planning and Control

### 8. Global Planner (PCT)

`pct_planner` from HKU/HKUST (GPLv2). Paper: "Efficient Trajectory Planning for Off-Road" (arXiv 2310.07780).

Artifacts (aarch64 only ï¿?x86 dev boxes auto-fall-back to `_AStarBackend`):
- `ele_planner.so` ï¿?3D A* on a hex grid with multi-slice height handling.
- `traj_opt.so` ï¿?GPMP (Gaussian-process motion planning) for jerk-minimal smoothing.
- `a_star.so`, `libele_planner_lib.so`.

Steps in `planner_wrapper.py::TomogramPlanner.plan()`:
1. World -> grid index (`pos2idx`).
2. Height -> slice index (`pos2slice`) ï¿?handles stairs / multi-storey.
...
- Library: `src/base_autonomy/local_planner/paths/*.ply` ï¿?7 groups, ~150 paths each, MATLAB-generated arcs of 1-2 m.
- Online (10 Hz):
...
## Part IV ï¿?Semantics and Intelligence

### 13. SemanticPlannerModule ï¿?five-level goal resolver
...
### 14. VisualServoModule ï¿?visual servo + person follow
...
Components: `BBoxNavigator` (bbox + depth -> 3D -> PD), `PersonTracker` (VLM select + CLIP Re-ID for occlusion robustness), `vlm_bbox_query` (open-vocab detection ï¿?Grounding-DINO or YoloE).
...
### 16. AgentLoop ï¿?multi-step LLM tool use
...
| `mock` | none ï¿?keyword path |

---

## Part V ï¿?Safety and Interfaces
...
`GatewayModule` (L6, `src/gateway/gateway_module.py`) ï¿?FastAPI + uvicorn on port 5050.
...
Key endpoints:
- `GET  /api/v1/session` ï¿?current mode + map + ICP quality.
- `POST /api/v1/session/start {mode, map_name}` ï¿?start mode (auto-launches slam / localizer services).
- `POST /api/v1/session/end` ï¿?return to idle.
- `POST /api/v1/goal {x, y, z?}` ï¿?send a navigation goal.
- `POST /api/v1/slam/relocalize {map_name, x, y, yaw}` ï¿?manual relocalization.
- `POST /api/v1/cmd/stop` ï¿?emergency stop.
- `POST /api/v1/bag/start {duration, prefix}` ï¿?start rosbag recording.
- `POST /api/v1/webrtc/bitrate {bps}` ï¿?adjust video bitrate.
- `GET  /api/v1/webrtc/stats` ï¿?video stats.
- `POST /api/v1/map_cloud/reset` ï¿?clear the browser-side cumulative point cloud.
...
Bag recording (`scripts/record_bag.sh`): `ros2 bag record` over 13 topics (no raw camera ï¿?payloads explode) into `~/data/bags/<prefix>_<ts>/`:
...
## Part VI ï¿?Operations and Field
...
~/data/inovxio/lingtu/                   source root
  éˆ¹æº¾æ”¢éˆ¹ï¿?lingtu.py                          CLI entry
  éˆ¹æº¾æ”¢éˆ¹ï¿?cli/profiles_data.py               profile + robot presets
  éˆ¹æº¾æ”¢éˆ¹ï¿?src/                               Python source
  éˆ¹æº¾æ”¢éˆ¹ï¿?install/                           colcon build (ROS 2 packages)
  éˆ¹æ–ºæ”¢éˆ¹ï¿?web/dist/                          dashboard static assets
/tmp/lingtu_nav.log                      tmux runtime log
~/data/lingtu/maps/                      maps (legacy ~/data/nova/maps still honoured)
  éˆ¹æ–ºæ”¢éˆ¹ï¿?<map_name>/
       éˆ¹æº¾æ”¢éˆ¹ï¿?map.pcd                       SLAM point cloud
       éˆ¹æº¾æ”¢éˆ¹ï¿?tomogram.pickle               PCT input
       éˆ¹æº¾æ”¢éˆ¹ï¿?map.pgm + map.yaml            nav2-compatible
       éˆ¹æ–ºæ”¢éˆ¹ï¿?poses.txt                     PGO output
...
- `localization.service` ï¿?Fast-LIO2 + Livox driver
- `localizer.service` ï¿?ICP localizer (loads saved map)
- `camera.service` ï¿?Orbbec launch
- `lingtu.service` ï¿?currently unused; field operations launch via tmux
...
Firewall: `iptables ROBOT_REMOTE` chain DROPs by default ï¿?open `5050` and `8090` after `lt restart`.
...
| localizer_ready | true | ï¿?| false |
| /nav/odometry | 8-10 Hz | 3-8 | < 3 / empty |
| /localization_quality | ~10 Hz | ï¿?| empty |
...
# Step 4 ï¿?monitor or stop
lt                             # mission status
lt stop                        # estop, cmd_vel zeroed

# Step 5 ï¿?record a bag for failure repro
lt record start
lt record
lt record stop
```
...
Obstacle avoidance is automatic ï¿?no toggle. As long as session is `navigating`, ICP quality is good and a goal is set, LocalPlanner re-scores 1000 candidates per frame, Terrain emits traversability, and CmdVelMux selects the final velocity.
...
| `lingtu watch` | `watch -c -n 1` continuous refresh ï¿?keep on a side monitor while mapping or navigating |
...
- Orbbec `color_fps:=30` locks the bootloader ï¿?never pass it to `gemini_330_series.launch.py`; you will physically need to replug.
- rclpy + uvicorn + aiortc together break thread scheduling ï¿?stay on the DDS fallback.
- `iptables ROBOT_REMOTE` DROPs `5050` / `8090` by default ï¿?open both after every deploy / reboot.
- SSH command lines with semicolons return exit 255 ï¿?wrap in a heredoc script.
...
| Y2 | PCT only on aarch64 | Accepted ï¿?A* fallback |
| Y3 | ChromaDB optional | Accepted ï¿?VectorMemory soft-fails to numpy |
| Y4 | All 1226 framework tests are mock-based | Roadmap ï¿?real ROS 2 integration tests |

Green ï¿?optimisation:

| # | Issue | Status |
|---|---|---|
| G1 | `gateway_module.py` exceeds 2700 lines, tightly coupled | Roadmap ï¿?split Session / SSE / HTTP |
| G2 | No monitoring / alerting | Roadmap ï¿?Prometheus + WeChat Work |
...
A: nav2 is heavy; quadrupeds don't need its behaviour-tree machinery; PCT's 3D terrain-aware planning has no nav2 equivalent; we want Module-First, not nav2 lifecycle nodes. OccupancyGrid is still nav2-compatible (`map.pgm` + `map.yaml`) ï¿?maps remain interoperable.
...
A: Yes. Use `--llm mock` ï¿?Fast Path is enough for the keyword cases. Only the multi-step Slow Path needs cloud LLM.
...
A: No ï¿?`~/.lingtu/last_nav_pose.json` persists the last successful relocalization. The next `session/start navigating` background worker restores it automatically.
