...
Current target: a ROS-free, typed-DDS field runtime for Thunder with one native
navigation writer and one remote Brainstem driver boundary. Local DDS/driver
contracts are now locked; remaining closure is target-side no-motion evidence,
fault injection, and supervised motion evidence.
...
2. Prove no-motion route preview on the deployed target with the current native
   endpoint and active map.
3. Run target-side fault injection before claiming field motion closure: kill
   DDS/nav/driver processes, drop Brainstem connectivity, stale localization,
   stale traversability, and lease preemption.
4. Keep `lingtu-driver` as the unique hardware consumer for `/nav/cmd_vel`.
5. Add typed command IDs and command acceptance/rejection acknowledgements;
   goal/cancel/teleop writers are already persistent C++ DDS clients.
6. Keep Python DDS adapters out of the field default path.
7. Keep ROS2 only as explicit compatibility adapter code.
8. Validate MapService save -> octomap artifact -> OctoPlanner3D preview.
9. Validate `tare_explore` as exploration target generation, not direct chassis control.
10. Replace Python camera DDS frame ingestion with a versioned C++ SHM image
   ring while retaining typed DDS metadata/health.
11. Make MuJoCo consume final native `/nav/cmd_vel` through the learned
...
| Active | `nav` | Re-run field evidence with the current native command output path: `lingtu-nav-dds -> /nav/cmd_vel -> lingtu-driver -> remote Brainstem`. Current local code has the command path and gates; target evidence still needs no-motion preview, DDS sampling, and fault injection. | `src/nav/cpp/endpoint/test_input_gate.cpp`; `src/nav/cpp/endpoint/test_nav_endpoint_config.cpp`; `src/drivers/real/thunder/native/test_core.cpp`; `src/drivers/real/thunder/native/test_io.cpp`; `scripts/deploy/thunder/lingtu-driver.service`. |
| Done locally | `driver` | Unique native `/nav/cmd_vel` consumer, remote Brainstem requirement, mTLS requirement for remote targets, initial zero before ready, checked Walk RPC, freshness/dedup/rate-limit handling, 200 ms command timeout, and status-file watchdog are implemented locally. | `src/drivers/real/thunder/native/test_core.cpp`; `src/drivers/real/thunder/native/test_io.cpp`; `scripts/deploy/thunder/lingtu-driver.service`; `scripts/deploy/thunder/run_driver.sh`; `scripts/deploy/thunder/install_driver_service.sh`. |
...
| Active | Prove no-motion navigation preview. | Gateway goal API, MapService, OctoPlanner3D, native nav endpoint. | `validate_plan` returns an OctoPlanner3D path while no motion command is submitted. |
...
| Done locally | Lock final autonomous command gate. | Driver readiness/lease freshness, localization/traversability freshness, mode ownership, and command publication checks gate the final `/nav/cmd_vel` writer. | `test_input_gate`, `test_nav_endpoint_config`; C++ endpoint build/tests. |
| Active | Prove final command safety on target. | Same gate under process kill, DDS loss, Brainstem loss, stale localization/traversability, and lease preemption on the deployed robot. | Field fault-injection report under `docs/07-testing/field-runs/`. |
...
- `lingtu-driver` is the only observed hardware consumer for `/nav/cmd_vel`.
- Fault injection proves unsafe/stale conditions produce zero, rejection, or no
  publish before supervised motion smoke.
