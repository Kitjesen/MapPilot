...
- Lightweight deployment and full field navigation share too many defaults.
...
- `src/core/blueprints/catalog/products.py`
...
- `src/core/blueprints/runtime_endpoint.py`
...
- Product-facing endpoint selectors should use aliases such as
  ...
  aliases only.
- `localization_adapter=lcm_endpoint` is the field localization ingress for
  Thunder LCM odometry, map cloud, registered cloud, and health streams.
- `endpoint_egress_adapter=lcm_endpoint` is the field navigation egress for
  Thunder LCM global path, local path, active waypoint, and muxed command
  velocity streams.
- `command_output_mode=endpoint_only` means field navigation does not keep an
  in-process `ThunderDriver`; the endpoint source owns hardware actuation.
- `endpoint_ingress_adapter=lcm_endpoint` is the field navigation command
...
- LCM localization ingress is implemented by
  `src/compat/lcm/localization_adapter.py` and keeps the legacy
...
- LCM path, waypoint, and velocity command egress is implemented by
  `src/compat/lcm/path_command_adapter.py` and keeps the existing
  `EndpointPathBridgeModule` blueprint alias as a compatibility name.
- LCM goal, cancel, and semantic instruction ingress is implemented by
  `src/compat/lcm/navigation_command_adapter.py` and keeps the
  `EndpointCommandBridgeModule` blueprint alias as a compatibility name.
- Standalone Thunder endpoint processes should use
  `src/compat/lcm/endpoint_service.py` and the runnable
  `src/compat/lcm/endpoint_runner.py` entrypoint to publish normalized
...
- Endpoint sources must implement the
  `src/compat/lcm/source.py` plugin protocol. Real hardware adapters and
  smoke/replay sources attach there, not inside product modules.
- `--source thunder_field` is the product source group for field deployment.
...
- The endpoint runner accepts comma-separated sources. Use this to compose
  ...
  one large adapter.
- `src/compat/lcm/sources/smoke.py` is the built-in no-ROS source for endpoint
  data-flow smoke checks. It is not a hardware adapter.
- `src/compat/lcm/sources/jsonl.py` is the built-in no-ROS normalized
  ...
  Use it for replay, external non-ROS SLAM/localization sidecars, and field
  provider bring-up.
- `tools/validate/validate_lcm_jsonl_feed.py` is the deployment gate for JSONL
  ...
  enforce the minimal field sensor inputs with `--require-field-inputs`.
- `src/compat/lcm/sources/thunder_brainstem.py` is the built-in no-ROS
  Brainstem command sink. It consumes muxed `/nav/cmd_vel` from the endpoint
  contract and forwards it to the Thunder hardware driver outside the LingTu
  module graph.
- Endpoint bridge module names should be transport-neutral where possible:
  `EndpointPathBridgeModule`, not product logic named after ROS.

## Deployment Targets

### `thunder-lite`

Purpose: fast local field bring-up and cross-platform smoke deployment.
...
Purpose: full field navigation on Thunder.
...
- The resolved `thunder-nav` graph does not include `ThunderDriver` by
  ...
  `CmdVelMux.driver_cmd_vel -> EndpointPathBridgeModule.cmd_vel`, then the
  endpoint source translates that command to the robot hardware.
- The field endpoint process runs separately through
...
- The default field command source is `thunder_brainstem`. It owns the
...
- `tools/validate/validate_thunder_field_deployment.py` is the deployment
  gate for the default field path. It checks the resolved `thunder-nav`
...
- Move remaining ROS-facing implementations into `src/compat/ros2/`.
...
- Keep `thunder_field_lcm_v1` as the visible field contract for Thunder
...
- Use `LCMPathCommandBridgeModule` for global path, local path, active
  waypoint, and muxed command velocity egress.
- Keep `command_output_mode=endpoint_only` for Thunder field endpoints so the
  module graph has one command sink and no duplicate in-process hardware
  driver.
- Use `LCMNavigationCommandBridgeModule` for goal, cancel, and semantic
  instruction ingress.
- Use `LCMEndpointService` as the standalone endpoint-side publisher/consumer
  ...
  streams.
- Use `python -m compat.lcm.endpoint_runner` or
...
- Use `--source smoke` for no-ROS endpoint data-flow checks and
  `--source thunder_field` for the default product field source group. Use
  ...
  `LINGTU_ENDPOINT_JSONL_COMMAND` when the field source group should include
  normalized no-ROS sensor/localization ingress.
- Validate any JSONL field provider before deployment:
...
- Keep optional extras for full field navigation and simulation.
...
- Thunder field deployment validator.
- Thunder Lite package validator and packager dry-run.
- JSONL endpoint feed validator for any no-ROS field provider.
