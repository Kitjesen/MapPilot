...
5. **`docs/archive/AGENTS.md` reviewed, deliberately left unchanged**: it is a frozen historical snapshot (still describes `src/core/`, `src/base_autonomy/`, `nav_core`, an un-prefixed `docs/MODULE_FIRST_GUIDELINE.md` path, ~1000-test-era counts, and a pre-reorg 9-factory/profile list) — stale on dozens of dimensions unrelated to `base_autonomy`, not just this one topic. It lives under `docs/archive/` as archived material, distinct from `docs/archive/MODULE_FIRST_GUIDELINE.md`, which the live `AGENTS.md` still cites as canonical and is presumably kept current on its own. Patching only the `base_autonomy` lines here would misrepresent the rest of the file as maintained, so it was left as-is.
