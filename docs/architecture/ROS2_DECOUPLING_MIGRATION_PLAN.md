...
Status: historical migration plan, partially superseded by the current native DDS roadmap
...
Builds on: [`LINGTU_RUNTIME_BUS_DECISION.md`](./LINGTU_RUNTIME_BUS_DECISION.md), [`ROS_ROLE_REPLACEMENT_MAP.md`](./ROS_ROLE_REPLACEMENT_MAP.md), [`ros_frame_contract.md`](./ros_frame_contract.md), [`PORTABLE_LEAN_PACKAGE_MATRIX.md`](./PORTABLE_LEAN_PACKAGE_MATRIX.md), [`NAVIGATION_RUNTIME_DATAFLOW.md`](./NAVIGATION_RUNTIME_DATAFLOW.md), [`PRODUCT_MODE_RUNTIME_CONTRACT.md`](./PRODUCT_MODE_RUNTIME_CONTRACT.md), and [`docs/plans/current-roadmap.md`](../plans/current-roadmap.md).
Does not modify: any SLAM/perception/planning/build code. This document only.

Current update: `thunder_field` still uses DDS for the field transport boundary,
but the old Python nav DDS adapters have been removed.
The field navigation DDS owner is the C++ `lingtu-nav-dds` service.

## 0. Executive Summary

LingTu's Python layer is architecturally ROS2-independent by design: Modules talk through `In[T]`/`Out[T]` ports over a pluggable Transport (Local callback, DDS/CycloneDDS, SHM, Adapter), and the field DDS baseline is the C++ CycloneDDS C API plus `idlc` from apt `cyclonedds-dev`/`cyclonedds-tools` on S100P. `cyclonedds-python` is optional for development diagnostics and must not be a robot startup dependency. **Current-state caveat**: the architecture itself is sound, but the endpoint-transport/evidence layer that implements it is mid-refactor today, not settled — a full run of `python -m pytest src/runtime/tests/ -q` on the current working tree reports 113 failed / 2636 passed / 19 skipped, with roughly 40 of those failures concentrated in exactly this layer (`test_runtime_evidence.py`, `test_transport.py`, `test_transport_wiring.py`, `test_lcm_endpoint_contracts.py`, `test_cli_no_repl.py`, `test_runtime_display.py`, `test_runtime_catalogs.py`, `test_deployment_service_contracts.py`, `test_run_state.py`, `test_module_boundaries.py`). See §2.6a for the concrete evidence and root-cause breakdown; it does not change the target architecture, the technical approach, or the phase order below, but it does add a stabilization work item to Phase 0 (§7) before Phase 2+ builds native C++ output paths on top of this layer. The remaining hard ROS 2 dependency — the thing that pins the robot to Ubuntu 22.04 / Humble — lives entirely in the C++ layer (`rclcpp` SLAM and base-autonomy nodes), two vendored third-party ROS2 driver packages (Livox, Orbbec), a handful of Python `rclpy`/`ros2`-CLI bridge and compatibility-adapter files, and the `colcon`/`ament_cmake` build graph that ties all of it together.
...
**Already-native counterpart** (not yet default everywhere): `src/localization/slam/cpp/` contains `cyclone_runtime.cpp` (pure CycloneDDS, no ROS 2 dependency) alongside `dds_runtime.cpp` (a ROS2-interoperable DDS runtime used for incremental rollout). `src/localization/slam/cpp/CMakeLists.txt` already gates these behind independent build flags: `LINGTU_SLAM_FASTLIO2_BACKEND`, `LINGTU_SLAM_BUILD_CPP_DDS_RUNTIME` (ROS2-adjacent), `LINGTU_SLAM_BUILD_CYCLONE_DDS_RUNTIME` (ROS-free). `scripts/build/build_slam_core.sh` already drives this. The native SLAM boundary is tracked by the C++ code contract under `src/localization/slam/cpp/` and by the current roadmap, not by the retired PRD snapshots.
...
| `src/nav/adapters/ros2/*` (`nav_in.py`, `nav_out.py`, `map_out.py` and siblings) | `rclpy` publish/subscribe adapters for nav-plane compatibility (`goal_pose`, `global_path`, `local_path`, `way_point`, `cmd_vel`) |
...
- **`thunder_field` now has one field nav DDS owner.** `RUNTIME_ENDPOINTS["thunder_field"]` in `src/runtime/profiles/catalog/endpoints.py` uses `endpoint_transport="dds"`, `localization_adapter="cpp_slam_status"`, `native_navigation_endpoint="lingtu-nav-dds"`, and `hardware_control_boundary="dds_endpoint_source"`; `enable_nav_in=false` and `enable_nav_out=false` keep Python nav adapters out of the field default. The Python DDS nav adapters have been removed; a field goal/path/cmd_vel must not be written by both Python nav.in/out and the C++ service.
...
Per `docs/04-deployment/README.md`, the current product systemd path is native
DDS first: `lingtu-livox-dds`, `lingtu-slam-dds`, `lingtu-nav-dds`, and
`lingtu`. The old `robot-lidar`, `robot-fastlio2`, and `robot-localizer` units
are compatibility services and must not be described as production defaults.
`scripts/lingtu` keeps explicit legacy aliases only for comparison/debug work.
...
- **Native SLAM boundary** — `src/localization/slam/cpp/` and `src/message/idl/` define the current C++/typed-DDS contract. The product path is ROS-free; ROS2 remains an explicit compatibility path only.
- **Transport dataflow contract** — `docs/architecture/NAVIGATION_RUNTIME_DATAFLOW.md`, `docs/architecture/LINGTU_RUNTIME_BUS_DECISION.md`, and `src/message/idl/lingtu_slam.idl` are the current source of truth for typed DDS/IDL runtime flow.
...
#### Phase 0 completion notes (2026-07-02)

**Done in this pass**

1. **`thunder_field` transport decision (§12.8 resolved)**: production field endpoint is **typed CycloneDDS** (`endpoint_transport="dds"`, contract `thunder_field_dds_v1`). Rationale: native Livox SDK2 ingest, C++ `lingtu_slam_cyclone_runtime`, and Thunder deployment systemd units already publish/consume the typed DDS/IDL layer (`src/message/idl/lingtu_slam.idl`); LCM remains for smoke/replay and optional `lcm-endpoint` operator installs. Updated `LINGTU_RUNTIME_BUS_DECISION.md`, `src/runtime/blueprints/products/thunder.py` base config (`dds_endpoint`), added `test_dds_endpoint_contracts.py`, and adjusted `test_lcm_endpoint_contracts.py` so LCM contract tests no longer assert the field default.
2. **Adapter package roles clarified**: `src/runtime/adapters/dds/` = production typed DDS field boundary; `src/runtime/adapters/lcm/` = smoke/replay/optional lcm-endpoint; `src/runtime/adapters/native/` = native SLAM map-save (non-ROS); `src/runtime/adapters/ros2/` = explicit ROS2 compat only. Parallel file names are intentional transport variants, not duplicate ownership of `thunder_field`.
3. **Module boundary fixes verified**: `src/drivers/real/lidar/native_factory.py` is a lazy ROS2 compat shim (no top-level `runtime.native_*` imports); DDS adapters no longer import `drivers.real.lidar`; `src/message/` is registered under `L0_core_contract` in `config/architecture_layers.yaml`.
4. **ROS2 coupling guardrail**: `validate_ros_coupling_touchpoints()` in `tools/validate/validate_architecture_boundaries.py` plus `src/runtime/tests/test_ros2_coupling_guardrail.py` — flags new `ros2` CLI subprocess and `/opt/ros/` references outside explicit compat allowlists (alongside existing `validate_ros_import_boundaries()`).
5. **Stale CI retired**: `.github/workflows/portable-fastlio2-windows-gate.yml` limited to `workflow_dispatch` with job `if: false` (paths stale vs `src/localization/slam/cpp/` native runtime direction).

**Already-native toolchain inventory (Phase 8 input)**

| Binary / library | Build entry | Toolchain observed | Ubuntu 22.04 pin? |
| --- | --- | --- | --- |
| `nav_kernel` (nanobind `.so`) | `scripts/build/build_nav_kernel.sh`, standalone CMake under `src/nav/kernel/` | GCC/Clang, CMake ≥3.16, OpenMP optional, Python dev headers for nanobind | Build scripts assume Linux/aarch64; no ROS2 dependency |
| `local_planner` C++ core | `src/nav/services/plan/local_planner/cpp/` standalone CMake | GCC/Clang, CMake, gtest when tests enabled | Same as nav_kernel |
| DUFOMap `dufomap_run` | `scripts/build/build_dufomap.sh` | apt build-deps on target, patched upstream, aarch64 release binary | Robot build doc targets Ubuntu 22.04; binary is plain C++, not ROS2 |
| Livox SDK2 DDS stream | `scripts/build/build_livox_sdk2_stream.sh` | Livox-SDK2 + CycloneDDS, plain CMake | No ROS2; CycloneDDS install path on S100P |
| Orbbec native capture | `scripts/build/build_orbbec_native.sh` | Vendor Orbbec SDK, plain CMake | No ROS2 |
| C++ SLAM cyclone runtime | `scripts/build/build_slam_core.sh` | CycloneDDS, Fast-LIO2 sources, `LINGTU_SLAM_BUILD_CYCLONE_DDS_RUNTIME` | No ROS2 on native path; colcon path remains compat |

**Still open after Phase 0** (not blocking Phase 1, but tracked): remaining failures in the §2.6a ten-module cluster outside endpoint-transport scope (CLI topic naming drift, runtime catalog profile enumeration, deployment contract strings). Full `src/runtime/tests/` baseline was ~113 failed / 2636 passed before this pass; re-run after merge to measure delta.
...
#### Phase 1 completion notes (2026-07-02)

**Done, see findings** — this phase was already substantially executed by an earlier workspace-wide runtime reorganization pass on this branch; this pass formalizes and verifies that fact rather than performing a fresh deletion.

1. **Zero production-profile dependency confirmed**: grepped `cli/profiles_data.py` and every file under `src/runtime/profiles/` for `legacy_ros`, `terrain_analysis`, `terrain_analysis_ext`, `sensor_scan_generation`, and `base_autonomy` — zero references (the only `local_planner`-adjacent hits are the unrelated Python `local_planner_backend`/`nav.local_planner` registry keys for the already-native `LocalPlannerModule`).
2. **The four `legacy_ros` packages no longer exist anywhere in this working tree**: not on disk, not in `git ls-tree HEAD`, and not in the history of any local branch (`codex/runtime-boundary-cleanup`, its parent `codex/simulation-architecture-hardening`, or others) — they were removed outright, not moved to a quarantine directory. This matches the repo's actual precedent for retired code: `src/legacy/` (the pre-v1.7 retirement location this plan originally pointed at) was itself deleted outright in commit `20c71e56` ("dead file removal"), not kept as a permanent archive — "delete, rely on git history" is the established convention here, not "move to `docs/archive`." A regression test already locks this in: `src/nav/tests/local/test_pure_language_fallback_contract.py::test_legacy_ros2_local_autonomy_packages_have_been_removed` asserts none of the four package directories exist on disk. `src/nav/local/README.md` already documents the retirement and cites this plan's Phase 1 by name.
3. **Native replacement independently verified with a real build, not just grep**: both `src/nav/kernel` and `src/nav/services/plan/local_planner/cpp` (standalone CMake, no ROS2/colcon) were configured and built from a clean `build/` directory on this Windows box using the MSYS2 Clang 21 toolchain already installed, and all three requested test binaries ran green: `test_path_follower_core` 17/17, `test_benchmark` 12/12, `test_local_planner_core` 35/35. Source-level grep of both trees for `legacy_ros`/`terrain_analysis_ext`/`sensor_scan_generation` is also clean (zero hits), confirming the earlier assumption that a full C++ build would likely be infeasible on Windows was overly conservative.
4. **Adjacent stale-doc fix**: `CLAUDE.md`'s Source Directory table still carried a `src/legacy/` row ("Retired code from pre-v1.7 refactor...") — that directory was deleted in June 2026 (commit `20c71e56`) and no longer exists on this branch; the row was removed. `CLAUDE.md`/`AGENTS.md` (repo root) were otherwise already correct on `base_autonomy/`: both already state it "no longer exists as a path" and point at `nav_kernel`/`src/nav/services/plan/local_planner/cpp`, so no further edit was needed there.
5. **Historical archive snapshots removed**: stale archive snapshots conflicted with the current runtime contracts and were removed along with the rest of the archive material. Use git history for old files; live guidance now comes from root `AGENTS.md`, `docs/architecture/README.md`, and the current contract documents.
6. **Test suite**: full `python -m pytest src/runtime/tests/ -q` (2768 tests at Phase 0 exit: 113 failed / 2636 passed / 19 skipped) re-run after this pass' doc-only edit (`CLAUDE.md`; no production `src/` code was changed): **168 of 172 test files completed in 826s at 108 failed / 2532 passed / 19 skipped (2659 collected)**. The remaining 4 files (119 tests: `test_kernel_migration_status.py`, `test_pose_graph_opt_coverage_audit.py`, `test_compute_kernel_migration_catalog.py`, `test_deployment_service_contracts.py`) were excluded from this run because they are extremely slow-to-non-terminating on this Windows sandbox for two distinct, pre-existing, environment-specific reasons confirmed via `pytest-timeout` thread-dumps (not related to this phase's changes): (a) the first three each drive `tools/validate/validate_pose_graph_opt_coverage.py::scan_repository()`, a pure-Python full-repository content walk (single tests observed exceeding 5-10 minutes on this filesystem); (b) `test_deployment_service_contracts.py` shells out via `_wsl_path()` to `bash -lc "wslpath -a ..."`, which hangs indefinitely on a Windows box without a working WSL install. Reconciliation: 2659 (run) + 119 (excluded, collected separately via `--collect-only`) = 2778, close to the Phase 0 baseline's 2768 — the small residual delta (~10 tests) and the 5-fewer-failures observed (108 vs 113 on a smaller set) are consistent with ongoing unrelated in-flight work on this branch, not with anything in this phase's two doc/comment-only edits. Spot-checked several of the 108 failures (including the GNSS-named `test_localization_health.py::TestNavigationDegeneracyResponse::*` cases) — root cause in every case sampled is unrelated pre-existing drift already described in §2.6a/Phase 0 (e.g. `ValueError: Unknown planner: 'astar'. Available: ['octoplanner3d', 'pct', 'direct']` from the in-flight planner-backend rename), not GNSS or legacy_ros. Net: no regression attributable to this phase. See §7 Phase 4 notes for the separate `config/robot_config.yaml` comment fix and the C++ build/test results in item 3 above for `nav_kernel`/`local_planner/cpp`.
7. **Nothing left to do in this phase**: no code changes were required or made in `src/nav/kernel/`, `src/nav/services/plan/local_planner/cpp/`, or anywhere in the Python runtime; the one edit made (`CLAUDE.md`) is comment/doc-only.
...
#### Phase 4 completion notes (2026-07-02)

**Done, see findings** — already native by default; documentation-only confirmation, exactly as anticipated by §2.5/§6. No profile cutover was needed.

1. **Prior art already said this was resolved**: `docs/architecture/ROS_ROLE_REPLACEMENT_MAP.md` (§3 prior art, not re-litigated here) already states the GNSS/RTK acquisition default is `GnssModule` reading `/dev/wtrtk980` through `GnssSerialDriver`, with DDS `/gps/fix` (what `wtrtk980_ros2_reader` would publish) as an explicit compatibility fallback only.
2. **Verified directly in code and config, not just in the role-replacement doc**: the `gnss()` stack factory (`src/runtime/blueprints/stacks/system.py`) reads `config/robot_config.yaml`'s `gnss:` section. `device: "/dev/wtrtk980"` is set there (truthy), and `device_manager_bridge` is not set anywhere under `config/`, so `use_device_manager_bridge` defaults to `not bool(serial_port)` = `False`; `GnssModule` therefore receives `serial_port="/dev/wtrtk980"` and `GnssModule.setup()` tries the native `GnssSerialDriver` (`src/localization/gnss_serial_driver.py`) first, only falling back to the DDS `/gps/fix` subscription if the serial driver raises. `config/devices.yaml`'s `wtrtk980_main` device entry resolves to the native `serial_nmea0183` driver (`src/runtime/devices/drivers/serial_nmea0183.py`) for the alternate `GnssBridgeModule` path — also fully native, not ROS2.
3. **No profile overrides this**: `cli/profiles_data.py` and everything under `src/runtime/profiles/` set no GNSS-specific override. The only `enable_gnss=False` occurrences are on Thunder-Lite-style minimal product intents, which disable GNSS entirely rather than switching it to the ROS2 reader.
4. **No systemd/ops-tooling path defaults to the ROS2 reader either**: `scripts/lingtu`, `docs/04-deployment/`, and the deploy scripts have no `gnss.service`/ROS2-GNSS-node wiring. `scripts/deploy/cut_release.sh` lists `wtrtk980_ros2_reader` only inside the explicit opt-in `ROS2_COMPAT_PKGS` bundle (gated by `REQUIRE_ROS2_COMPAT`) — matching this plan's target architecture (§4: ROS2 as an explicit compatibility profile), not a default-path concern.
5. **Small doc-only fix made** (config comment, zero behavior change): `config/robot_config.yaml`'s `gnss:` block header comment claimed the driver was `ironoa/um982_ros2_driver (systemd: gnss.service)` — stale and factually wrong relative to the code path verified above (no such systemd unit exists anywhere in the repo; it likely predates the current `GnssModule`/`GnssSerialDriver` architecture). Corrected in place to describe the actual native-first / DDS-fallback behavior and point at `ROS_ROLE_REPLACEMENT_MAP.md`; the adjacent `topic_fix` comment was corrected the same way.
6. **Conclusion**: no production profile defaults to the ROS2 GNSS reader today. Validation gate is satisfied as-is; no cutover code change was required.
...
8. **Are `src/runtime/adapters/dds/` and `src/runtime/adapters/lcm/` a deliberate boundary-type split, or two competing implementations that need to converge on one?** **Resolved (Phase 0, 2026-07-02)**: deliberate split. `dds/` owns the production `thunder_field` typed CycloneDDS boundary (`thunder_field_dds_v1`); `lcm/` owns smoke/replay and optional `lcm-endpoint` deployments (`thunder_field_lcm_v1`). Parallel adapter file names are transport variants sharing the same canonical topic contract, not duplicate owners of the field default.
...
- `docs/plans/current-roadmap.md` — current product/runtime roadmap and remaining validation gates
- `docs/architecture/NAVIGATION_RUNTIME_DATAFLOW.md` — typed runtime dataflow and navigation boundary
