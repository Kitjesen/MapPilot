...
状态：研究快照，不是当前运行时合同。
读者：感知路线规划和算法评估。
替代文档：`semantic_layer_contract.md` 记录当前运行时边界。
...
- **核心竞争力**:Product/Host 解耦架构 + 在 RDK X5 边缘算力约束下的**确定性性能保证**。
...
1. **Host 内 Module 解耦**:perception ↔ memory 依赖反转。`room_inferencer` 单向消费 scene graph,`scene_graph_builder` 不 import memory,保证层次边界清晰。
