...
Replaced by: `FIELD_PRODUCTS.md`, `SYSTEM_DESIGN.md`, binding-policy tests
...
| `lingtu[lite]` | `lite` Profile compatibility extra | Empty marker; The `lite` Profile uses base dependencies only. |
| `lingtu[core-portable]` | Generic Windows/Ubuntu core portable marker | Empty marker; mirrors the base dependency set. |
...
- `lingtu[lite]` and `lingtu[core-portable]` extras mirror the base dependency set;
- `lingtu[sim-mujoco]` layers only MuJoCo on top of core portable;
