...
CORE_RUNTIME_BOUNDARY_SOURCES = {
    ...
    "runtime_profile_adapter_config": SRC
    / "runtime"
    / "profiles"
    / "profile_adapter_config.py",
    "runtime_catalog": SRC / "runtime" / "profiles" / "catalog",
    "runtime_profile_adapters": SRC / "runtime" / "profiles" / "profile_adapters.py",
}
...
RUNTIME_MODEL_OWNER_SOURCES = {
    ...
    "runtime_catalog_profile_adapters": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "profile_adapters.py",
    "runtime_catalog_profile_adapter_configs": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "profile_adapter_configs.py",
    "runtime_catalog_host_defaults": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "host_defaults.py",
    ...
    / "catalog"
    / "host_defaults.py",
    "runtime_catalog_local_host_defaults": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "local_host_defaults.py",
    "runtime_catalog_navigation_defaults": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "navigation_defaults.py",
    "runtime_catalog_driver_backends": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "driver_backends.py",
    "runtime_catalog_driver_runtime_defaults": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "driver_runtime_defaults.py",
    "runtime_catalog_driver_catalog": SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / "driver_catalog.py",
...
ALGORITHM_KERNEL_SEAM_SOURCES = {
    ...
    "nav_kernel_loader": SRC / "nav" / "kernel" / "loader.py",
}
...
REMOVED_DRIVER_CATALOG_PATHS = (
    ...
    SRC / "runtime" / "profiles" / "catalog" / ("robot" + "_archives.py"),
    SRC
    / "runtime"
    / "profiles"
    / "catalog"
    / ("robot" + "_runtime_defaults.py"),
)

CANONICAL_DRIVER_CATALOG_PATHS = (
    ROOT / "config" / "driver_backends" / "thunder.yaml",
    SRC / "runtime" / "profiles" / "catalog" / "driver_backends.py",
    SRC / "runtime" / "profiles" / "catalog" / "driver_catalog.py",
    SRC / "runtime" / "profiles" / "catalog" / "driver_runtime_defaults.py",
)
...
def test_runtime_contract_imports_through_core_boundary() -> None:
    ...
    assert THUNDER_DATA_SOURCE in manifest["resolved_runtime_data_flow"]
    assert tuple(manifest["runtime_data_flow_topics"][THUNDER_DATA_SOURCE]) == runtime_data_flow_topics(
        THUNDER_DATA_SOURCE
    )
...
def test_driver_backend_catalog_replaces_retired_hardware_catalog() -> None:
    ...
    for key in ("backends", "protocols", "modules", "runtime_defaults"):
        assert f"{key}:" in source


def test_resolver_profile_adapter_layer_uses_profile_adapter_config_helper() -> None:
    resolver_source = (SRC / "runtime" / "profiles" / "resolver.py").read_text(
        encoding="utf-8"
    )
    adapter_source = (SRC / "runtime" / "profiles" / "profile_adapters.py").read_text(
        encoding="utf-8"
    )
    catalog_source = (
        SRC / "runtime" / "profiles" / "catalog" / "profile_adapters.py"
    ).read_text(encoding="utf-8")

    assert "from runtime.profiles.profile_adapter_config import" in resolver_source
    assert "profile_adapter_config_for_profile(" in resolver_source
    assert "merge_profile_adapter_config(" in resolver_source
    assert "apply_profile_adapter_config(" not in resolver_source

    assert "from runtime.profiles.profile_adapter_config import" in adapter_source
    assert "profile_adapter_config_for_profile(adapter, profile)" in adapter_source
    assert "merge_profile_adapter_config(config, adapter_config)" in adapter_source

    assert "profile_adapter_config_for_profile(self, profile)" in catalog_source
    assert 'merged["_profile_adapter"]' not in catalog_source
...
def test_runtime_shared_defaults_are_not_duplicated_outside_runtime_paths() -> None:
    ...
    forbidden_sources = {
        "host_defaults": SRC / "runtime" / "profiles" / "catalog" / "host_defaults.py",
        "local_host_defaults": SRC
        / "runtime"
        / "profiles"
        / "catalog"
        / "local_host_defaults.py",
        "navigation_defaults": SRC
        / "runtime"
        / "profiles"
        / "catalog"
        / "navigation_defaults.py",
        ...
        "profile_adapter_configs": SRC
        / "runtime"
        / "profiles"
        / "catalog"
        / "profile_adapter_configs.py",
        "profile_adapters": SRC
        / "runtime"
        / "profiles"
        / "catalog"
        / "profile_adapters.py",
...
def test_retired_runtime_vocabulary_does_not_return() -> None:
    scan_roots = (
        ROOT / "cli",
        ROOT / "config",
        ROOT / "src",
    ...
    skipped_parts = {"archive", "node_modules", "superpowers", "third_party", "vendor"}
    forbidden = (
        "module" + "-first",
        "module" + "_first_dataflow",
        "module" + "first",
        "robot" + "_preset",
        "robot " + "preset",
        "robot-" + "preset",
        ...
        "robot " + "archive",
        "robot-" + "archive",
        "robot" + "_runtime_defaults",
        "robot" + "_driver_profile",
        "robot" + "_catalog",
        "robot " + "catalog",
        "robot-" + "catalog",
        "robot" + "profile",
        "runtime" + "manifest",
        "lingtu_runtime" + "_manifest",
        "active" + "-product.json",
        "lingtu_active" + "_runtime_file",
        "reapply" + "-active",
        "restart" + "-active",
        "stop" + "-active",
    )
    ...
    for root in scan_roots:
        for path in root.rglob("*"):
            ...
            for token in forbidden:
                if token in source:
                    offenders.append(f"{relative.as_posix()} contains retired token {token}")

    assert offenders == [], "Retired runtime vocabulary found:\n" + "\n".join(offenders)


def test_retired_product_runtime_modules_stay_removed() -> None:
    retired_modules = {
        "lingtu." + "launcher",
        "lingtu." + "product",
        "lingtu." + "manifest_contract",
    }
    retired_paths = (
        SRC / "lingtu" / ("launcher" + ".py"),
        SRC / "lingtu" / ("product" + ".py"),
        SRC / "lingtu" / ("manifest_contract" + ".py"),
    )

    assert [_repo_rel(path) for path in retired_paths if path.exists()] == []

    offenders: list[str] = []
    for root in (ROOT / "cli", ROOT / "scripts", ROOT / "sim", ROOT / "src", ROOT / "tests"):
        if not root.exists():
            continue
        for path in root.rglob("*.py"):
            try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    continue
                for module in sorted(imported.intersection(retired_modules)):
                    offenders.append(
                        f"{_repo_rel(path)}:{getattr(node, 'lineno', 0)} imports {module}"
                    )

    assert offenders == [], "Retired runtime module import(s) found:\n" + "\n".join(offenders)
