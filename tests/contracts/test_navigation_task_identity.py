...
from runtime.runtime_interface import NAVIGATION_TASK_IDENTITY_WIRE_CONTRACT, TOPICS
...
def test_navigation_task_identity_uses_existing_dds_topics() -> None:
    ...
    assert TOPICS.nav_command_request == "/nav/command/request"
    assert TOPICS.nav_command_ack == "/nav/command/ack"
    assert TOPICS.nav_goal_status == "/nav/goal/status"
    assert TOPICS.nav_state == "/nav/state"
    assert all(topic in topics for topic in NAVIGATION_TASK_IDENTITY_WIRE_CONTRACT.topics)
    assert "/navigation/tasks/request" not in topics
    assert "/navigation/tasks/ack" not in topics


def test_final_idl_task_identity_requires_a_cold_switch() -> None:
    contract = NAVIGATION_TASK_IDENTITY_WIRE_CONTRACT

    assert contract.schema_revision == "lingtu.dds.navigation_task_identity.v2"
    assert contract.deployment_policy == "cold_switch"
    assert contract.mixed_versions_supported is False
    ...
    assert "mixed-version operation is unsupported" in contract.note
...
def test_navigation_command_receipt_keeps_legacy_cancel_current_compatible() -> None:
    receipt = NavigationCommandReceipt(
        accepted=True,
        kind=int(NavigationCommandKind.CANCEL),
        task_id="",
        request_id="cancel-current-attempt-1",
        endpoint_timestamp_s=4.0,
        reason="cancel_requested",
    )

    assert receipt.task_id == ""
    assert receipt.to_dict()["kind_name"] == "CANCEL"
