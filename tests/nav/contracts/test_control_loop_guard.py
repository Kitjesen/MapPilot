...
GUARD_TEST = ROOT / "tests/nav/cpp/endpoint/test_control_loop_runtime_guard.cpp"
...
def test_cmake_and_linux_build_gate_require_runtime_guard_test() -> None:
    ...
    assert "test_control_loop_runtime_guard.cpp" in tests
    assert "control/guard.cpp" in tests
    assert re.search(r"add_test\s*\(\s*NAME\s+test_control_loop_runtime_guard\b", tests)
