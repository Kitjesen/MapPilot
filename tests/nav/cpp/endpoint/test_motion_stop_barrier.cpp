...
using lingtu::nav::endpoint::ResumeTeleopRequest;
...
struct Fixture {
  ...
  Fixture() {
    ...
    actions.resume_control = [&] {
      order.emplace_back("resume_control");
    ...
    actions.set_teleop_request_not_before = [&](double stamp_s) {
      order.emplace_back("set_teleop_stamp:" + std::to_string(stamp_s));
    };
...
void testClearEndpointMotionBranches() {
  Fixture inactive;
  MotionStopBarrier inactive_coordinator(true, inactive.actions);
  ...
  MotionStopBarrier active_coordinator(true, active.actions);
...
void testEndpointHoldPreservesActiveRoute() {
  Fixture inactive;
  MotionStopBarrier inactive_coordinator(true, inactive.actions);
  require(inactive_coordinator.holdEndpointMotion("driver_control_timeout"),
          "inactive hold must succeed");
  orderIs(inactive, {"suspend_outputs:driver_control_timeout"});

  Fixture rolling;
  rolling.rolling_active = true;
  MotionStopBarrier rolling_coordinator(true, rolling.actions);
  require(rolling_coordinator.holdEndpointMotion("driver_control_timeout"),
          "rolling hold must preserve the finite segment until recovery revalidation");
  orderIs(rolling, {"suspend_outputs:driver_control_timeout"});
}

void testTaskPausePublishesStateOnlyAfterConfirmedStop() {
  Fixture confirmed;
  MotionStopBarrier coordinator(true, confirmed.actions);
  const auto result =
  ...
  MotionStopBarrier timed_out_coordinator(true, timed_out.actions);
...
void testCancelStopAndConfirmationFailures() {
  Fixture cancelled;
  MotionStopBarrier cancel_coordinator(true, cancelled.actions);
  ...
  Fixture stopped;
  MotionStopBarrier stopped_coordinator(true, stopped.actions);
  ...
  MotionStopBarrier zero_failure_coordinator(true, zero_failure.actions);
  ...
  MotionStopBarrier rejected_coordinator(true, rejected.actions);
  ...
  Fixture unavailable;
  MotionStopBarrier unavailable_coordinator(false, unavailable.actions);
...
void testCancelTerminalFollowsConfirmedStopEvidence() {
  Fixture confirmed;
  MotionStopBarrier confirmed_coordinator(true, confirmed.actions);
  ...
  MotionStopBarrier zero_failed_coordinator(true, zero_failed.actions);
...
void testStopPreservingGoalTerminalCommitsAfterConfirmedZero() {
  Fixture fixture;
  MotionStopBarrier coordinator(true, fixture.actions);
...
void testStopPreservingGoalTerminalDoesNotCommitWithoutConfirmedZero() {
  ...
  MotionStopBarrier no_sequence_coordinator(true, no_sequence.actions);
...
void testEstopPersistFailureStillZerosButDoesNotCommitTerminal() {
  ...
  MotionStopBarrier persist_failed_coordinator(true, persist_failed.actions);
...
void testGoalReplanStopConfirmsWithoutTerminalSideEffects() {
  ...
  Fixture disabled;
  MotionStopBarrier disabled_coordinator(false, disabled.actions);
...
void testClearEstopRequiresConfirmedZero() {
  Fixture invalid;
  MotionStopBarrier invalid_coordinator(true, invalid.actions);
  ...
  MotionStopBarrier persist_failure_coordinator(true, persist_failure.actions);
...
void testResumeOnlyUnlatchesAfterConfirmation() {
  ...
  orderIs(confirmed, {"defer_abort:autonomy_resume_ready", "sync", "rolling_active",
                      "clear_outputs:autonomy_resume_ready", "last_sequence", "confirm_zero:17",
                       "abort:autonomy_resume_ready", "resume_control",
                      "set_resume_stamp:42.500000", "clear_resume_required"});

  Fixture already_ready;
  MotionStopBarrier already_ready_coordinator(true, already_ready.actions);
...
void testTeleopResumeConfirmsZeroWithoutTouchingAutonomyGoal() {
  const ResumeTeleopRequest request{{}, true, 42.5};

  Fixture rejected;
  rejected.confirmation = StopConfirmationState::TimedOut;
  MotionStopBarrier rejected_coordinator(true, rejected.actions);
  const auto rejected_result = rejected_coordinator.resumeTeleop(request);
  require(!rejected_result.accepted &&
              rejected_result.reason == "stop_confirmation_timeout_takeover_remains_latched",
          "teleop resume timeout mismatch");
  require(rejected.takeover_latched, "unconfirmed teleop resume must preserve the runtime hold");
  orderIs(rejected, {"rolling_active", "clear_outputs:teleop_resume_ready", "last_sequence",
                     "confirm_zero:17"});

  Fixture confirmed;
  MotionStopBarrier confirmed_coordinator(true, confirmed.actions);
  const auto confirmed_result = confirmed_coordinator.resumeTeleop(request);
  require(confirmed_result.accepted &&
              confirmed_result.reason == "teleop_resume_ready_reassert_command",
          "teleop resume success mismatch");
  require(!confirmed.takeover_latched, "confirmed teleop resume must release the runtime hold");
  orderIs(confirmed, {"rolling_active", "clear_outputs:teleop_resume_ready", "last_sequence",
                      "confirm_zero:17", "set_teleop_stamp:42.500000", "resume_control",
                      "clear_resume_required"});

  Fixture already_ready;
  MotionStopBarrier already_ready_coordinator(true, already_ready.actions);
  const auto ready_result = already_ready_coordinator.resumeTeleop({{}, false, 1.0});
...
void testShutdownPhysicalOnlyAndTerminalPreservingPipelines() {
  ...
  MotionStopBarrier physical_only_coordinator(true, physical_only.actions);
  ...
  MotionStopBarrier preserving_coordinator(true, preserving.actions);
...
void testShutdownZeroFailuresNeverCommitTerminal() {
  ...
  MotionStopBarrier publish_failed_coordinator(true, publish_failed.actions);
...
int main() {
  try {
    ...
    testEndpointHoldPreservesActiveRoute();
    ...
    testTeleopResumeConfirmsZeroWithoutTouchingAutonomyGoal();
    ...
    std::fprintf(stderr, "test_motion_stop_barrier: FAIL: %s\n", exc.what());
    return 1;
  }
  return 0;
}
