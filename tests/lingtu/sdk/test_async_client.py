...
class TestAsyncLingTuClient(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_wait_until_arrived_delegates_task_identity(self) -> None:
        expected = NavigationStatus(task=NavigationTask(state="SUCCESS"))
    ...
    async def test_wait_until_arrived_without_identity_delegates_current_task_wait(self) -> None:
        expected = NavigationStatus(task=NavigationTask(state="SUCCESS"))
        self.client.wait_until_arrived.return_value = expected
        ...
        self.assertIs(result, expected)
        self.client.wait_until_arrived.assert_called_once_with(None, 12.0, 0.1)
