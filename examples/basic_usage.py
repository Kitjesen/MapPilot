...
def main(host: str = "127.0.0.1") -> None:
    ...
    if isinstance(maps, dict):
        for m in maps.get("maps", []):
            print(f"  Map: {m.get('name')}  "
                  f"PCD: {m.get('has_pcd', False)}  "
                  f"Active: {m.get('is_active', False)}")
    elif isinstance(maps, list):
        for m in maps:
            print(f"  Map: {m.get('name', m)}")
    ...
    # ── Hot-swap driver ─────────────────────────────────────────────────
    robot.swap_driver("sim_mujoco")  # switch to simulated driver
