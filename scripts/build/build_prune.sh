...
BUILD_DIR="${LINGTU_PRUNE_BUILD_DIR:-${LINGTU_MAP_CLEANING_BUILD_DIR:-${DEFAULT_BUILD_DIR}}}"
SRC_DIR="${REPO_ROOT}/src/maps/prune/cpp"
JOBS="${LINGTU_BUILD_JOBS:-$(nproc 2>/dev/null || echo 4)}"
BUILD_TARGETS=(prune erasor2_stage)
...
case "${LINGTU_PRUNE_ERASOR2:-${LINGTU_MAP_CLEANING_ERASOR2:-OFF}}" in
  1|ON|on|TRUE|true|YES|yes)
    CMAKE_ARGS+=(-DLINGTU_PRUNE_ERASOR2=ON)
    BUILD_TARGETS+=(erasor2_clean)
    ;;
esac
