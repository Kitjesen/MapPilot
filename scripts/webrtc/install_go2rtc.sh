#!/usr/bin/env bash
# scripts/webrtc/install_go2rtc.sh — install go2rtc sidecar on S100P (aarch64).
#
# Why a sidecar?
...
#   bash scripts/webrtc/install_go2rtc.sh --no-systemd   # just install the binary
#
# Security:
...
Look up the official SHA-256 for ${GO2RTC_VERSION} go2rtc_linux_arm64, then run:
  GO2RTC_SHA256=<expected-sha256> bash scripts/webrtc/install_go2rtc.sh

For local lab testing only, bypass verification explicitly with:
  GO2RTC_SKIP_SHA256=1 bash scripts/webrtc/install_go2rtc.sh
EOF
    exit 1
fi
...
echo "==> Installing go2rtc → ${BIN_PATH}"
