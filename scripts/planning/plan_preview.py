...
trusted local files from another directory.
"""
...
def require_trusted_tomogram_path(path: Path, map_root: Path, allow_external: bool) -> None:
    """Guard pickle loading to the trusted map root unless explicitly overridden."""
    try:
        resolved_path = path.expanduser().resolve(strict=False)
        resolved_root = map_root.expanduser().resolve(strict=False)
        in_map_root = os.path.commonpath([str(resolved_path), str(resolved_root)]) == str(resolved_root)
    ...
    if allow_external:
        print(
            f"WARNING: loading trusted external tomogram pickle: {path}",
            file=sys.stderr,
        )
        return
    raise ValueError(
        "refusing to load tomogram pickle outside the trusted map root; "
        "move it under --map-root or pass --trust-tomogram-pickle for trusted local files"
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--trust-tomogram-pickle", action="store_true", help="Allow loading a trusted local tomogram pickle outside --map-root/NAV_MAP_DIR.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        require_trusted_tomogram_path(
            tomogram_path,
            map_root,
            allow_external=bool(args.trust_tomogram_pickle),
        )
