...
SERVICE_NAME="${LINGTU_NAV_DDS_SERVICE_NAME:-lingtu-nav-dds.service}"

sudo mkdir -p "${CONFIG_DIR}"
sudo cp "${SCRIPT_DIR}/runtime-env.sh" "${CONFIG_DIR}/thunder-runtime-env.sh"
sudo chmod 0644 "${CONFIG_DIR}/thunder-runtime-env.sh"

sudo cp "${SCRIPT_DIR}/lingtu-nav-dds.service" "${SYSTEMD_DIR}/${SERVICE_NAME}"
...
echo "Installed LingTu navigation DDS endpoint service: ${SYSTEMD_DIR}/${SERVICE_NAME}"
