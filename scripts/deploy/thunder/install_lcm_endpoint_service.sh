#!/usr/bin/env bash
# Install the Thunder field LCM endpoint service without ROS compatibility units.
...
SERVICE_NAME="${LINGTU_ENDPOINT_SERVICE_NAME:-lingtu-thunder-lcm-endpoint.service}"

sudo mkdir -p "${CONFIG_DIR}"
sudo cp "${SCRIPT_DIR}/runtime-env.sh" "${CONFIG_DIR}/thunder-runtime-env.sh"
sudo chmod 0644 "${CONFIG_DIR}/thunder-runtime-env.sh"

sudo cp "${SCRIPT_DIR}/lingtu-thunder-lcm-endpoint.service" "${SYSTEMD_DIR}/${SERVICE_NAME}"
...
echo "Installed Thunder LCM endpoint service: ${SYSTEMD_DIR}/${SERVICE_NAME}"
