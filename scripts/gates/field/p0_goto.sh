...
echo "[1/5] Sanity"
HEALTH="$(curl -sf "${AUTH_ARGS[@]}" http://localhost:5050/api/v1/health)"
...
while [[ $SECONDS -lt $DEADLINE ]]; do
  STATUS_JSON="$(curl -sf "${AUTH_ARGS[@]}" http://localhost:5050/api/v1/navigation/status 2>/dev/null || echo '{}')"
  STATUS="$(echo "$STATUS_JSON" | python3 -c 'import json,sys; d=json.load(sys.stdin); print((d.get("task") or {}).get("state","?"))')"
