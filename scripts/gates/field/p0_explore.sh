...
stop_explore() {
  bash "$REPO_ROOT/scripts/lingtu" stop >/dev/null 2>&1 || true
}
...
if [[ -n "$EXPLORE_MAP" ]]; then
  bash "$REPO_ROOT/scripts/lingtu" switch explore --map "$EXPLORE_MAP"
else
  bash "$REPO_ROOT/scripts/lingtu" switch explore
fi
...
bash "$REPO_ROOT/scripts/lingtu" stop
trap - EXIT
