...
HOST_CHECK_ACTIONS = {
    ...
    "gazebo_runtime": (
        "install a Gazebo/Ignition runtime with gz or ign on PATH, then verify with sim/scripts/gazebo_runtime_gate.py"
    ),
    "isolated_ros_domain": (
...
def _categories_for_gate(
    ...
    if not categories and gate_name in missing_or_failed:
        ...
        if any(
            item in blockers
            for item in ("abi", "native runtime", "python_tag", "no runnable pct", "mingw numpy", "numpy")
        ):
            ...
            entry = checks_by_name.setdefault(
                str(check_name),
                {
                    ...
                    "diagnostic_commands": list(
                        check.get("diagnostic_commands") or HOST_CHECK_DIAGNOSTIC_COMMANDS.get(str(check_name), ())
                    ),
                },
            )
        ...
        commands.append(
            {
                ...
                "host_failed_checks": list(row["host_preflight"].get("failed_checks") or []),
                "host_preflight_blockers": list(row["host_preflight"].get("blockers") or []),
                "dependency_blockers": list(row.get("dependency_blockers") or []),
                "dependency_blocker_status": dict(row.get("dependency_blocker_status") or {}),
                "runtime_dataflow_blocker": row.get("runtime_dataflow_blocker", ""),
                "runtime_dataflow_failed_edges": list(row.get("runtime_dataflow_failed_edges") or []),
...
def _host_setup_command_rows(host_setup_plan: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for raw_check in host_setup_plan.get("failed_checks") or []:
        ...
        diagnostic_commands = [str(command) for command in check.get("diagnostic_commands") or [] if str(command)]
        for command in diagnostic_commands:
            add(
                command=command,
                check=check_name,
                gates=[str(gate) for gate in check.get("gates") or [] if str(gate)],
                blockers=[str(blocker) for blocker in check.get("blockers") or [] if str(blocker)],
        ...
        [
            {
                ...
                "gates": gates,
                "recommended_action": ("prove the target host is runnable before launching DimOS gates"),
            },
            {
                "command": RUN_MISSING_COMMAND,
                "check": "run_missing",
                "gates": gates,
                "recommended_action": ("run missing DimOS gates in dependency order with host-blocked gates skipped"),
            },
            {
        ...
        for dependency in GATE_RUN_DEPENDENCIES.get(gate_name, ()):
            ...
            if dependency_row:
                if _mapping(dependency_row.get("host_preflight")).get("ok") is False:
                    ...
                    blocker_status[dependency] = str(dependency_row.get("status") or "failed")
            else:
...
def _runtime_dataflow_gate_brief(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "same_source_provenance": _jsonable(_mapping(value.get("same_source_provenance"))),
        ...
        "child_report_freshness": _jsonable(_mapping(value.get("child_report_freshness"))),
    }
...
def _runtime_dataflow_cross_gate_chains(
    ...
    native = _runtime_dataflow_gate_brief(_mapping(runtime_dataflow_gates.get("native_pct_mujoco")))
    if "policy_nav" in runtime_dataflow_gates:
        policy = _runtime_dataflow_gate_brief(_mapping(runtime_dataflow_gates.get("policy_nav")))
    ...
    fastlio_gates = {
        name: _runtime_dataflow_gate_brief(_mapping(runtime_dataflow_gates.get(name))) for name in fastlio_gate_names
    }
            ...
            fastlio_candidate_rejections.append(
                {
                    ...
                    "primary_blocker": (candidate.get("primary_blocker") or candidate.get("reason") or ""),
    ...
    pct_proven_gate = next(
        (
            ...
            if payload.get("ok") is True and _mapping(payload.get("pct_provenance")).get("ok") is True
        ),
        "",
    )
    same_run_gate = next(
    ...
    any_fastlio_checked = any(payload.get("checked") is True for payload in fastlio_gates.values())
    any_fastlio_ok = bool(first_ok_fastlio_gate)
    native_same_source_ok = _mapping(native.get("same_source_provenance")).get("ok") is True
    ...
    if native.get("checked") is not True:
        ...
        blockers.append("native_pct_mujoco dataflow did not prove same-source map artifacts")
    if not any_fastlio_checked:
        ...
        blockers.append("Fast-LIO live dataflow did not prove PCT planner in the same run")
    elif not fastlio_same_run_proven:
        blockers.append("Fast-LIO live dataflow did not prove same-source map artifacts")
    chains["pct_mujoco_and_fastlio_live"] = {
        ...
        "claim_boundary": (
            "same_run_pct_fastlio_live" if same_run_proven else "cross_gate_evidence_not_single_run_fusion"
        ),
        "blockers": blockers,
    }
    return chains
...
def _execution_plan(
    *,
        ...
        phases.append(
            {
                ...
                "failed_checks": [item.get("check") for item in host_setup_plan.get("failed_checks") or []],
    ...
    unblocked_rows = [
        ...
        if host_preflight_checked and row["host_preflight"]["ok"] is not False and not row.get("dependency_blockers")
    ]
    ...
    blocked_rows = [
        row for row in prioritized_failed_rows if host_preflight_checked and row["host_preflight"]["ok"] is False
    ]
        ...
        phases.append(
            {
                ...
                "status": "target_host",
                "purpose": ("run the full DimOS closure loop on the prepared Linux ROS2/PCT/MuJoCo simulation host"),
    ...
    dependency_blocked_rows = [
        row
        for row in failed_rows
        if host_preflight_checked and row["host_preflight"]["ok"] is not False and row.get("dependency_blockers")
    ]
...
def build_dimos_gap_report(
    ...
    *,
    ...
    host_preflight_source = (
        host_preflight_source or "argument"
    ...
    failing_cross_gate_chains = _mapping(runtime_dataflow_summary.get("failing_cross_gate_chains"))
    runtime_dataflow_ok = (
    ...
    for index, gate_name in enumerate(DIMOS_BENCHMARK_REQUIRED_GATES, start=1):
        ...
        if not host_blockers and gate_run_status == "host_blocked" and str(gate_run.get("error") or ""):
    ...
    for row in failed_rows:
        ...
        if not dependency_blockers:
            continue
        blocker_text = "blocked by unmet prerequisite gate(s): " + ", ".join(dependency_blockers)
        ...
        row["recommended_action"] = "run prerequisite gate reports first: " + ", ".join(dependency_blockers)
    prioritized_failed_rows = sorted(
        failed_rows,
        key=lambda row: (_priority_rank(str(row["priority"])), int(row["order"])),
    )
    actionable_failed_rows = [row for row in prioritized_failed_rows if not row.get("dependency_blockers")]
    ...
    if summary_is_stale:
        ...
        stop_condition = "do not claim DimOS-style algorithm health until cross-gate dataflow passes: " + ", ".join(
            cross_gate_failures
        )
    else:
        stop_condition = (
