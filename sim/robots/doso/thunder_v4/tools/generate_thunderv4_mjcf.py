...
ROBOT_VISUAL_MATERIALS = {
    # MJCF/Unreal PBR colors are linear values. These correspond to the
    # intended sRGB swatches #121416, #181A1C, #141618, #0C0E10, and #060708.
    "body_shell_black": "0.006049 0.006995 0.008023 1",
    "joint_graphite": "0.009134 0.010330 0.011612 1",
    "limb_graphite": "0.006995 0.008023 0.009134 1",
    "sensor_black": "0.003677 0.004391 0.005182 1",
    "wheel_black": "0.001821 0.002125 0.002428 1",
}
...
# Match the policy_1119 nominal joint reference so reset and policy warmup start
# from the same pose.
...
def _visual_material_name(canonical_link_name: str) -> str:
    if canonical_link_name.endswith("_foot"):
        return "wheel_black"
    if canonical_link_name.endswith("_hip"):
        return "joint_graphite"
    if canonical_link_name.endswith(("_thigh", "_calf")):
        return "limb_graphite"
    if canonical_link_name.startswith(("camera", "lidar")):
        return "sensor_black"
    return "body_shell_black"
...
def _visual_meshes(links: dict[str, ET.Element]) -> tuple[ET.Element, dict[str, list[ET.Element]]]:
    ...
    for material_name, rgba in ROBOT_VISUAL_MATERIALS.items():
        ET.SubElement(asset, "material", {"name": material_name, "rgba": rgba})
    ET.SubElement(
        asset,
        "texture",
        {
    ...
    for source_name, link in links.items():
        ...
        for index, visual in enumerate(link.findall("visual")):
            ...
            attrs = {
                ...
                "group": "1",
                "density": "0",
                "material": _visual_material_name(canonical),
            }
            if origin is not None:
...
def _build_model(urdf_path: Path, scene: str = "flat", show_collisions: bool = False) -> ET.ElementTree:
    ...
    mujoco_root.append(ET.Comment(" Attachable RobotPackages inherit session-owned MuJoCo options. "))
    ...
    ET.SubElement(base, "site", {"name": "payload_top", "size": "0.01", "pos": "0 0 0.14", "quat": "1 0 0 0"})
    camera_mount = _fixed_joint_pose(joints["camera1_joint"])
    ET.SubElement(
        base,
        "camera",
        {"name": "front_camera", "pos": camera_mount["pos"], "xyaxes": "0 -1 0 0 0 1", "fovy": "60"},
    )
