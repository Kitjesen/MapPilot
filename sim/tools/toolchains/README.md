...
```powershell
pwsh -NoProfile -File .\sim\tools\toolchains\detect_windows_toolchain.ps1 | Out-File .\sim\tools\toolchains\windows.detected.json -Encoding utf8
pwsh -NoProfile -File .\sim\tools\toolchains\detect_windows_toolchain.ps1 -LockFile .\sim\tools\toolchains\windows.lock.json
```
...
`sim/tools/toolchains/ue_build.py`. The same runner also has an explicit direct UBT
...
```powershell
& .\.venv\Scripts\python.exe .\sim\tools\toolchains\ue_build.py `
