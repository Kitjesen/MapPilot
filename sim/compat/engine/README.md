# Compatibility Python Simulation Engine

Status: development engine used by direct simulation scripts and gates.

`sim/compat/engine` contains Python simulation utilities used by development gates.
...
| `worlds/` | `WorldRegistry` scene XML lookup, alias registration, empty-world fallback. |

Gazebo compatibility adapters live under `sim/adapters/gazebo/`.
...
from sim.compat.engine.core import SimEngine, WorldConfig, RobotConfig
from sim.compat.engine.mujoco import MuJoCoEngine
from sim.compat.engine.worlds import WorldRegistry
```
...
src/drivers/sim/mujoco/driver.py
  ...
  -> sim/compat/engine/mujoco/engine.py
  -> MuJoCoLidar.scan()
  -> sim/compat/engine/mujoco/lidar.py
...
require the canonical `sim/packages/sensors/livox/mid360/assets/mid360.npy` scan pattern; startup
...
simulation engine and sensor implementation stay in `sim/compat/engine` so driver
...
Generated reports belong under `artifacts/`, not under `sim/compat/engine`. Simulation
