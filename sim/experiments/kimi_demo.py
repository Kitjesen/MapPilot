    ...
    "rooms": [
        {
            "id": "room_0",
            "name": "corridor",
            "object_labels": ["door", "fire extinguisher", "exit sign", "trash can", "umbrella stand"],
            "object_ids": [1, 2, 3, 4, 15],
        },
        {
            "id": "room_1",
            "name": "office",
            ...
            "object_ids": [5, 13, 6, 7, 8, 9, 10, 11, 12, 14],
        },
    ],
    "groups": [
...
TESTS = [
    ...
    (
        ...
        "\u8d70\u5eca\u91cc\u7684\u51fa\u53e3\u6807\u5fd7\u5728\u54ea\u91cc",
        "zh",
        "L3: ZH room-level reasoning",
    ),
    (
        "ZH-L3-complex",
        "\u529e\u516c\u5ba4\u91cc\u9760\u7a97\u7684\u90a3\u628a\u7ea2\u8272\u6905\u5b50",
        "zh",
        "L3: ZH complex room+attr+spatial",
    ),
...
def print_slow_result(name, desc, result, elapsed, raw_llm=""):
    print_separator()
    print(f"  [{name}] {desc}")
    print("  Path: SLOW PATH (Kimi-k2.5 LLM)")
    print(f"  Latency: {elapsed:.0f} ms")
    print_separator("-")
    print(f"  Action:     {result.action}")
    if result.action == "navigate":
        print(
            f"  Target:     {result.target_label}  ({result.target_x:.1f}, {result.target_y:.1f}, {result.target_z:.1f})"
        )
...
async def run_demo():
    ...
    print("  Scene: 2 rooms, 15 objects, 10 relations")
    print("  Robot position: (3.0, 1.5, 0.0)")
    print_separator("=")
    print()
            ...
            all_output.append(
                {
                    ...
                    "action": fast.action,
                    "target_label": fast.target_label,
