...
SCENE_GRAPH = {
    ...
    "topology_edges": [
        ...
        {"from_room": 0, "to_room": 3, "type": "passage", "mediator": "corridor", "distance": 5.0},
        {"from_room": 0, "to_room": 4, "type": "passage", "mediator": "corridor", "distance": 5.0},
    ...
    "groups": [
        {"group_id": 0, "room_id": 0, "name": "structure", "center": {"x": 0.0, "y": 0.0}, "object_ids": [1, 3]},
        {"group_id": 1, "room_id": 0, "name": "safety", "center": {"x": 0.5, "y": 0.5}, "object_ids": [2, 4]},
        {
            "group_id": 2,
            "room_id": 1,
            "name": "workstation",
        ...
        {"group_id": 4, "room_id": 3, "name": "utility", "center": {"x": -4.0, "y": 3.0}, "object_ids": [12, 13, 14]},
        {"group_id": 5, "room_id": 4, "name": "structure", "center": {"x": -4.0, "y": -3.0}, "object_ids": [15, 16]},
    ],
    "objects": [
        {
            "id": 1,
...
async def test_goal_resolution_with_topology(client, resolver):
    ...
    tests = [
        ...
        ("find the printer in office B", "en", "EN+topology: LLM should identify office_B and select printer"),
        (
            "\u53bb\u53a8\u623f\u62ff\u676f\u6c34",
            "zh",
    ...
    for instruction, lang, desc in tests:
        ...
        # Slow Path -- LLM
        print("  Path: SLOW (calling Kimi-k2.5...)", end="", flush=True)
...
async def test_tsg_vs_llm_exploration(client, resolver):
    ...
    explore_tests = [
        ...
        ("\u627e\u5230\u5496\u5561\u673a", "zh", "target absent -- kitchen most likely has coffee machine"),
        ("find the fire extinguisher on the 2nd floor", "en", "stairwell most likely leads to 2nd floor"),
        (
            "\u627e\u5230\u4f1a\u8bae\u5ba4\u7684\u6295\u5f71\u4eea",
            "zh",
    ...
    for instruction, lang, desc in explore_tests:
        ...
        for i, t in enumerate(targets):
            print(
                f"  #{i + 1}: {t.node_name} (score={t.score:.3f}, IG={t.information_gain:.3f}, "
...
async def test_traversal_memory(client):
    ...
    points = [np.array([f["position"]["x"], f["position"]["y"]]) for f in SCENE_GRAPH.get("frontier_nodes", [])]
    ...
    print("  Calling Kimi-k2.5...", end="", flush=True)
    t0 = time.perf_counter()
    try:
        raw = await client.chat(messages)
    except Exception as e:
...
async def main():
    ...
    print("  Scene: 5 rooms, 20 objects, 6 topology edges, 2 frontiers")
    print("  LLM: Kimi-k2.5 (Moonshot)")
    sep("=")
