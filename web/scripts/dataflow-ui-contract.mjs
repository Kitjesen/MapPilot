...
ensure(!has(app, '<InspectionAcceptanceView'), failures, 'App must keep InspectionAcceptanceView as an internal diagnostic instead of the product run entry')
...
ensure(has(view, 'runtimeProduct(next'), failures, 'RuntimeDataflowView must derive the current Product from runtime identity')
ensure(has(view, 'current_product: currentProduct'), failures, 'RuntimeDataflowView must pass the current Product into switch-plan')
ensure(has(view, 'target_product: currentProduct'), failures, 'RuntimeDataflowView must pass an explicit Gateway Product into switch-plan')
ensure(has(view, 'runtime_product_unknown'), failures, 'RuntimeDataflowView must block switch-plan when Product identity is absent')
...
ensure(has(smoke, 'runtime_switch_plan must not switch Env'), failures, 'Gateway smoke must verify fixed-Env Product switching')
ensure(has(smoke, 'resolved target RunPlan'), failures, 'Gateway smoke must verify resolved RunPlan identity')
