...
The directories below are vendored source snapshots kept with their upstream
license files. They are for offline calibration workstations only and must not
be linked into or bundled with the LingTu product runtime/release.
...
`direct_visual_lidar_calibration/` is MIT-licensed, but its optional
`find_matches_superglue.py` path requires a separate SuperGlue dependency with
a non-commercial license. That dependency is not approved for commercial or
product use; use manual initialization unless separate licensing is cleared.
