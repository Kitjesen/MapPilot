...
```bash
cd catkin_ws/src
ln -s /path/to/lingtu/tools/calibration/lidar_imu/LiDAR_IMU_Init .
cd ..
catkin_make
...
# Result:
#   tools/calibration/lidar_imu/LiDAR_IMU_Init/result/Initialization_result.txt
```
...
LingTu's `tools/calibration/lidar_imu/config/mid360.yaml` differs from upstream:
...
- Product acceptance still requires `python tools/calibration/verify.py` and a short SLAM sanity run after applying the result.
