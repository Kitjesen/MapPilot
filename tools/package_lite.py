#!/usr/bin/env python3
"""Build a manifest-driven lite Profile runtime package."""
...
DEFAULT_OUTPUT_DIR = ROOT_DIR / "artifacts" / "lite-package"
SUMMARY_FILENAME = "LITE_PACKAGE.json"

if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

from tools.validate import validate_lite_package as contract  # noqa: E402


class PackageBuildError(RuntimeError):
    """Raised when the lite Profile package cannot be built safely."""
...
def build_package(
    *,
    ...
    """Build the lite Profile package and return a machine-readable summary."""

    validation = contract.validate(manifest_path)
    if not validation["ok"]:
        raise PackageBuildError(
            "lite Profile package contract failed: " + "; ".join(str(item) for item in validation["blockers"])
        )

    manifest = _load_manifest(manifest_path)
    package_cfg = manifest.get("package") or {}
    include_paths = tuple(contract._normalize_manifest_path(path) for path in package_cfg.get("include_paths") or ())
    exclude_paths = tuple(contract._normalize_manifest_path(path) for path in package_cfg.get("exclude_paths") or ())
    omit_patterns = tuple(contract._normalize_manifest_path(path) for path in package_cfg.get("omit_paths") or ())
    ...
    summary = {
        "schema_version": "lingtu.lite_package.build.v1",
...
def _prepare_output_dir(output_dir: Path, *, force: bool) -> None:
    ...
    if output_dir.exists():
        if not force and any(output_dir.iterdir()):
            raise PackageBuildError(f"output directory is not empty: {output_dir}. Use --force to replace it.")
...
def _is_excluded(rel_path: str, exclude_paths: tuple[str, ...]) -> bool:
    ...
    candidates = [rel_path]
    candidates.extend(
        contract._normalize_manifest_path(parent) for parent in PurePosixPath(rel_path).parents if str(parent) != "."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the lite Profile package builder CLI."""

    parser = argparse.ArgumentParser(description="Build a lite Profile runtime package")
    parser.add_argument(
        "--manifest",
    ...
    try:
        ...
        if args.json:
            ...
            print(f"lite Profile package build: FAIL\n  ERROR: {exc}")
        return 1
    ...
    if args.json:
        ...
        print(
            f"lite Profile package {mode}: {summary['output_dir']} "
