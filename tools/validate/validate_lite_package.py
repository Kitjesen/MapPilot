#!/usr/bin/env python3
"""Validate the lite Profile package and deployment contract."""
...
MANIFEST_PATH = ROOT_DIR / "config" / "lite_package.yaml"
PYPROJECT_PATH = ROOT_DIR / "pyproject.toml"
RUNTIME_ENV_PATH = ROOT_DIR / "scripts" / "deploy" / "thunder" / "runtime-env.sh"
FORBIDDEN_LITE_MODULE_PREFIXES = {
...
REQUIRED_PACKAGE_INCLUDE_PATHS = (
    ...
    "config/driver_backends/thunder.yaml",
    ...
    "src/nav/services/plan/mapless/__init__.py",
    "src/nav/services/plan/mapless/direct.py",
    "src/nav/services/plan/mapless/direct_path.py",
...
def _lite_excluded_module_prefixes(manifest: dict[str, Any]) -> dict[str, str]:
    ...
    for rel_path in package_cfg.get("exclude_paths") or ():
        ...
        if prefix:
            prefixes[prefix] = f"package excluded from the lite Profile: {rel_path}"
    return prefixes
...
def _validate_package_boundary(
    ...
    if missing_includes:
        blockers.append(
            "package.include_paths missing required lite Profile runtime paths: " + ", ".join(missing_includes)
        )

    missing_excludes = sorted(required_excludes - exclude_set)
    if missing_excludes:
        blockers.append(
            "package.exclude_paths missing required lite Profile exclusions: " + ", ".join(missing_excludes)
        )

    missing_omits = sorted(required_omits - omit_set)
    if missing_omits:
        blockers.append("package.omit_paths missing required lite Profile omissions: " + ", ".join(missing_omits))
...
def _validate_python_dependencies(manifest: dict[str, Any], blockers: list[str], checked_files: list[str]) -> None:
    ...
    if leaked_base:
        blockers.append(
            "pyproject.toml: core dependencies include packages forbidden "
            f"by the lite Profile: {', '.join(leaked_base)}"
        )
    ...
    if leaked_lite:
        blockers.append(
            f"pyproject.toml: {lite_extra!r} extra includes packages forbidden "
            f"by the lite Profile: {', '.join(leaked_lite)}"
        )
    ...
    if lite_requirements:
        blockers.append(
            "python.lite_requirements is deprecated; pyproject.toml base dependencies "
            "and the lite extra are the sole source of truth"
        )

    required_lite = {_dependency_name(dep) for dep in python_cfg.get("required_lite_dependencies") or ()}
    all_lite_names = base_names | lite_names
    missing_lite = sorted(required_lite - all_lite_names)
    if missing_lite:
        blockers.append(
            f"pyproject.toml: missing required lite Profile dependencies in base/lite: {', '.join(missing_lite)}"
        )
...
def _validate_runtime_contract(
    ...
    profile = str(manifest.get("profile") or "lite")
...
def _validate_lite_runtime_defaults(
    ...
    if config.get("enable_native") is not False:
        blockers.append(f"profile config {profile}: enable_native must be false for the lite Profile runtime")
...
def validate(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    """Validate the lite Profile manifest and return a structured result."""
    ...
    if manifest.get("schema_version") != "lingtu.lite_package.v1":
        blockers.append("schema_version must be lingtu.lite_package.v1")
    if manifest.get("profile") != "lite":
        blockers.append("profile must be lite")
...
def main(argv: list[str] | None = None) -> int:
    """Run the lite Profile package validator CLI."""

    parser = argparse.ArgumentParser(description="Validate lite Profile packaging boundaries")
    parser.add_argument("--json", action="store_true", help="Print machine-readable JSON")
    parser.add_argument(
        "--manifest",
        default=str(MANIFEST_PATH),
        help="Path to config/lite_package.yaml",
    )
    ...
    if args.json:
        ...
        print(f"lite Profile package check: OK ({checked})")
    else:
        print("lite Profile package check: FAIL")
