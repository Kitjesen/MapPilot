#!/usr/bin/env python3
"""validate_config.py — Validate the structure and value ranges of robot_config.yaml.

Usage:
    python tools/validate/validate_config.py            # run from project root
    python tools/validate/validate_config.py --strict   # treat WARNINGs as errors

Exit codes:
    0  all checks passed
    1  ERROR found (in --strict mode, WARNINGs also count)
"""
...
# ── Schema definition ────────────────────────────────────────────
# Expected type and optional value range (min, max) for each section/key.
SCHEMA: dict[str, dict[str, dict]] = {
    ...
    "gateway": {
        ...
        "mcp_port": {"type": int, "required": False, "min": 1, "max": 65535},
    },
    "brainstem": {
...
def _find_config() -> str:
    """Locate config/robot_config.yaml by searching up from the script directory."""
    candidates = [
...
def validate(cfg: dict, strict: bool = False) -> tuple[int, int]:
    """Validate cfg and return (errors, warnings) counts."""
    errors = 0
    warnings = 0
    ...
    # 2. Validate each key against schema
    for section_name, keys in SCHEMA.items():
        ...
        # Check for unknown keys within this section
        for key in section:
            if key not in keys:
